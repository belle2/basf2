...
void NtupleClusterTool::setupTree()
{
  ...
  m_nHits  = new float[nDecayProducts];
  m_ClusterHasPSD  = new int[nDecayProducts];
  m_PulseShapeDiscriminationMVA = new float[nDecayProducts];
  ...
  for (int iProduct = 0; iProduct < nDecayProducts; iProduct++) {
    ...
    m_tree->Branch((strNames[iProduct] + "_clusterNHits").c_str(),      &m_nHits[iProduct],
                   (strNames[iProduct] + "_clusterNHits/F").c_str());
    m_tree->Branch((strNames[iProduct] + "_clusterHasPSD").c_str(),      &m_ClusterHasPSD[iProduct],
                   (strNames[iProduct] + "_clusterHasPSD/I").c_str());
    m_tree->Branch((strNames[iProduct] + "_PulseShapeDiscriminationMVA").c_str(),      &m_PulseShapeDiscriminationMVA[iProduct],
                   (strNames[iProduct] + "_PulseShapeDiscriminationMVA/F").c_str());
...
void NtupleClusterTool::deallocateMemory()
{
  ...
  delete [] m_PulseShapeDiscriminationMVA;
...
void NtupleClusterTool::eval(const Particle* particle)
{
  ...
  for (int iProduct = 0; iProduct < nDecayProducts; iProduct++) {
    ...
    m_nHits[iProduct]   = float(Variable::eclClusterNHits(selparticles[iProduct]));
    ...
    m_PulseShapeDiscriminationMVA[iProduct]  = Variable::eclPulseShapeDiscriminationMVA(selparticles[iProduct]);
