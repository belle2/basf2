...
void NtupleDalitzTool::setupTree()
{
  ...
  m_tree->Branch((strNames[0] + "_m12").c_str(), &m_m12, (strNames[0] + "_m12/F").c_str());
  m_tree->Branch((strNames[0] + "_m13").c_str(), &m_m13, (strNames[0] + "_m13/F").c_str());
  m_tree->Branch((strNames[0] + "_m23").c_str(), &m_m23, (strNames[0] + "_m23/F").c_str());
...
{
  if (!particle) {
    B2ERROR("NtupleDalitzTool::eval - ERROR, no Particle found!\n");
    return;
  }
  ...
  if (selparticles.size() != 4) {
    B2ERROR("NtupleDalitzTool::eval - ERROR, you must select exactly 4 particles in the decay (mother, daughter1, daughter2, daughter3)!\n");
    return;
  }
