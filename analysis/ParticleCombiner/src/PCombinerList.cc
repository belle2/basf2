...
void PCombinerList::addParticle(const Particle* particle)
{
  if (particle->getArrayName() != m_particleStore) {
    B2ERROR("PCombinerList::addParticle particle is from different store array, not added");
    return;
  }
  int iparticle = particle->getArrayIndex();
  if (iparticle < 0) {
    B2ERROR("PCombinerList::addParticle particle is not in a store array, not added")
    return;
  }
...
void PCombinerList::setPDG(int pdg)
{
  auto* particlePDG = TDatabasePDG::Instance()->GetParticle(pdg);
  if (not particlePDG) {
    B2WARNING("PCombinerList::setPDG Couldn't find given PDG Code in TDatabasePDG: " << pdg)
    return;
  }
...
bool PCombinerList::hasAntiParticle(int pdg)
{
...
void PCombinerList::addParticle(unsigned iparticle, int pdg, unsigned type)
{
  if (abs(pdg) != abs(getPDG())) {
    B2ERROR("PCombinerList::addParticle PDG codes do not match, not added")
    return;
  }
...
void PCombinerList::removeParticles(const std::vector<unsigned int>& toRemove)
{
...
Particle* PCombinerList::getParticle(unsigned i) const
{
...
void PCombinerList::print() const
{
  std::cout << "PCombinerList:";
  std::cout << " " << m_particleStore;
  std::cout << " PDGCode=" << getPDG() << " [antiPDGCode=" << getPDGbar() << "]";
