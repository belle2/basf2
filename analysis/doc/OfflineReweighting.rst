...
Majority of the physics analyses at Belle II are relying on simulation for setting up the selection cuts for the process of interest. 
The selection on simulated data (MC) will result in different efficiency if the same selection would be applied on the real experimental data, 
...
The offline PID tables are provided as files in CSV format and they can be easily loaded as a DataFrame in Jupyter Notebook or a python script 
via ``pandas.from_csv()`` function. 
Each charged final state particle can have a correspoding efficiency and fake rate correction tables. 
...
table is ``(11, 11)`` and pion to electron fake rate is ``(11, 211)``.
...
The following code snipppet provides an example of reweighting particles with prefixes ``lepminus_`` and ``lepplus_`` in the ``user_df`` DataFrame:
...
One can switch off the weight variations by providing the ``generate_variations=False`` flag to ``reweight()`` function, 
...
Because of the high cross-feed rates between the FEI modes, a covariance matrix must be provided ether as a separate ``.npy`` file or 
