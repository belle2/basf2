...
Rave('D0:kpi', 0.0, fit_type='massvertex', path=my_path)

# perform D0 single track fit (production vertex)
# D0 vertex and covariance matrix must be defined
Rave('D0:st', 0.0, path=my_path)
# keep candidates only passing C.L. value of the fit > 0.0 (no cut)
Rave('D0:st', 0.0, decay_string='^D0 -> K- pi+', constraint='ipprofile', path=my_path)

# perform D0 vertex fit updating daughters
# keep candidates only passing C.L. value of the fit > 0.0 (no cut)
Rave('D0:du', 0.0, daughtersUpdate=True, path=my_path)
