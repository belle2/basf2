...
variables_TL = dict()
variables_TL['Electron'] = ['p_CMS', 'pt_CMS', 'p', 'pt', 'eid', 'eid_dEdx', 'eid_TOP', 'eid_ARICH', 'eid_ECL']
variables_TL['Muon'] = ['p_CMS', 'pt_CMS', 'p', 'pt', 'muid', 'muid_dEdx', 'muid_TOP', 'muid_ARICH']
variables_TL['Kaon'] = ['p_CMS', 'pt_CMS', 'cosTheta', 'pt', 'Kid', 'Kid_dEdx', 'Kid_TOP', 'Kid_ARICH', 'charge', 'NumberOfKShortinRemainingROEKaon', 'ptTracksRoe', 'distance']
variables_TL['SlowPion'] = ['p_CMS', 'pt_CMS', 'cosTheta', 'p', 'pt', 'piid', 'piid_dEdx', 'piid_TOP', 'piid_ARICH', 'pi_vs_edEdxid', 'cosTPTO', 'charge', 'Kid', 'eid']
variables_TL['Lambda'] = ['lambdaFlavor', 'NumberOfKShortinRemainingROELambda', 'M', 'cosAngleBetweenMomentumAndVertexVector', 'lambdaZError', 'MomentumOfSecondDaughter', 'MomentumOfSecondDaughter_CMS', 'p_CMS', 'p', 'chiProb', 'distance']
...
variables_EL['Kaon'] = ['bestQrOf(K+:ROE , IsFromB(Kaon))', 'p_CMS_Kaon', 'chargeTimesKaonLiklihood']  # TODO More Event Level Variables
variables_EL['SlowPion'] = ['bestQrOf(pi+:ROE, IsFromB(SlowPion))', 'p_CMS_SlowPion']  # TODO More Event Level Variables

if eventLevelReady:
    for (symbol, category) in eventLevelParticles:
        methodPrefix_EL = 'TMVA_' + category + '_EL'
        # Using MetaVariable
        targetVariable = 'IsRightClass(' + category + ', ' + symbol + ':ROE' + ', ' + 'IsFromB(' + category + '))'
        print 'This is the targetVariable: ' + targetVariable
...
if combinerLevelReady:
    variables = ['QrOf(e+:ROE, IsRightClass(Electron, e+:ROE, IsFromB(Electron)), IsFromB(Electron))',
...
if combinerLevelReady:
    variablesToNTuple('B0', ['getExtraInfo(qr_Combined)', 'getExtraInfo(qr_MC)'],
                      'TaggingInformation', workingDirectory + '/B0_B0bar_final.root', path=main)
