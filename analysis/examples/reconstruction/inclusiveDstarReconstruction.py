...
mA.addInclusiveDstarReconstruction("D*+:Dstcharged_slowPicharged_MC -> pi+:from_mc", "[useCMSFrame(p) < 0.2]", main_path)

mA.fillParticleListFromMC("pi0:from_mc", "", addDaughters=True, path=main_path)
mA.addInclusiveDstarReconstruction("D*+:Dstcharged_slowPizero_MC -> pi0:from_mc", "[useCMSFrame(p) < 0.2]", main_path)
mA.addInclusiveDstarReconstruction("D*0:Dstzero_slowPizero_MC -> pi0:from_mc", "[useCMSFrame(p) < 0.2]", main_path)
...
mA.addInclusiveDstarReconstruction("D*+:Dstcharged_slowPicharged -> pi+:slow ...", "[useCMSFrame(p) < 0.2]", main_path)
mA.matchMCTruth("D*+:Dstcharged_slowPicharged", main_path)
mA.addInclusiveDstarReconstruction("D*-:Dstcharged_slowPicharged_negative -> pi-:slow ...", "[useCMSFrame(p) < 0.2]", main_path)
mA.matchMCTruth("D*+:Dstcharged_slowPicharged_negative", main_path)

stdPi0s.stdPi0s("eff60", main_path)
mA.matchMCTruth("pi0:eff60", main_path)
mA.addInclusiveDstarReconstruction("D*+:Dstcharged_slowPizero -> pi0:eff60 ...", "[useCMSFrame(p) < 0.2]", main_path)
mA.matchMCTruth("D*+:Dstcharged_slowPizero", main_path)
mA.addInclusiveDstarReconstruction("D*0:Dstzero_slowPizero -> pi0:eff60 ...", "[useCMSFrame(p) < 0.2]", main_path)
