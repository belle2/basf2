...
Depending on the hardware used for the training some of the parameters found in the basf2/analysis/data/{configurationfile}.yaml file might need to be adjusted.\
...
3. **Prepare training data**
   ...
   - All of this is done by running the `merge_samples_to_parquet.py` script:
    ...
    - IMPORTANT! If you change the 'chunk_size' parameter this script has to be re-run as the chunking is done here.

4. **Training**
    - To launch the training use the `trainer.py` script:
    ```bash
   python3 trainer.py --train_input /path/to/output_parquet/{uniqueIdentifier}_training_samples.parquet --val_input /path/to/output_parquet/{uniqueIdentifier}_validation_samples.parquet --uniqueIdentifier {uniqueIdentifier}
    ```
    - Should the training crash at any point it can be restarted from the latest checkpoint like this:
    ```bash
   python3 trainer.py --train_input /path/to/output_parquet/{uniqueIdentifier}_training_samples.parquet --val_input /path/to/output_parquet/{uniqueIdentifier}_validation_samples.parquet --uniqueIdentifier {uniqueIdentifier} --warmstart
    ```
