...
namespace TreeFitter {
  ...
  ErrCode KalmanCalculator::init(const CLHEP::HepVector& residuals, const CLHEP::HepMatrix& G,
                                 const FitParams* fitparams, const CLHEP::HepSymMatrix* V, int weight)
  {
    ErrCode status;
    m_nconstraints = residuals.num_row();  // dimension of the constraint
    m_nparameters  = fitparams->dim(); // dimension of the state

#ifndef SLOWBUTSAFE
    int valdim  = residuals.num_row(); // dimension of the constraint
    ...
    if (m_ierr) {
      ...
      B2ERROR("Error inverting matrix. Vertex fit fails.");
    }
...
//     HepSymMatrix Rs(residuals.numrow()) ;
...
//     VtkSparseMatrix Ks = CGT*Rs ;
  ...
  {
    //fitparams->par() -= fitparams->cov() * (G.T() * (R * residuals) ) ;
    if (vtxverbose >= 8) {
      std::cout << "KalmanCalculator::updatePar - updating parameters:" << std::endl;
      std::cout << "                              parameter dimension = " << fitparams->dim() << std::endl;
      std::cout << "                              constraint dimension = " << m_residuals->num_row() << std::endl;
      for (int i = 0; i < fitparams->dim(); i++) {
        if ((m_matrixK * (*m_residuals))[i]) {
          std::cout << fitparams->par()[i] << " -= ";
          for (int j = 0; j < m_residuals->num_row(); j++) {
            std::cout << "    +(" << m_matrixK[i][j] << "  *  " << (*m_residuals)[j] << ")" << std::endl;
          }
        }
      }
    }

    fitparams->par() -= m_matrixK * (*m_residuals);
    m_chisq = m_matrixRinv.similarity(*m_residuals);
  }

  void KalmanCalculator::updatePar(const CLHEP::HepVector& pred, FitParams* fitparams)
  {
    // this is still very, very slow !
    CLHEP::HepVector residualsprime = (*m_residuals) + (*m_matrixG) * (pred - fitparams->par());
    fitparams->par() = pred - m_matrixK * residualsprime;
    m_chisq = m_matrixRinv.similarity(residualsprime);
  }
...
//      fitparams->cov().fast(row,col) += tmp * _matrixK(col,k) ;
    ...
    for (int col = 1; col <= m_nconstraints; ++col) {
      for (int k = 1; k <= m_nparameters; ++k) {
        if ((*m_matrixG)(col, k) != 0) {
          ++(fitparams->nConstraintsVec(k)) ;
        }
      }
    }
  }
