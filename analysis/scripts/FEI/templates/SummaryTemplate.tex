...
\begin{abstract}
This report contains key performance indicators and control plots of the Full Event Interpretation algorithm.
The user-, pre-, and post-cuts as well as trained multivariate selection methods are described.
Furthermore the resulting purities and efficiencies are stated. In ${overallSignalEfficiencyInPercent}\,\% of the events
...
\clearpage
\section{Summary}
For each final-state particle a multivariate selection method was trained without any previous cuts
on the candidates. Afterwards, the signal probability calculated by the method was used to apply
...
\end{center}
\FloatBarrier
For each decay channel of each intermediate particle a multivariate selection method was trained after applying a fast pre-cut
on the candidates. Afterwards the signal probability calculated by the method was used to apply
