...
def test_expert_jpsi(working_dir, file_names, prefix, environmentType='MC5', max_events=0):
    ...
    Rave('K_S0:pipi', 0., path=main, silence_warning=True)
    ...
    Rave('J/psi:mumu', 0., fit_type='massvertex', path=main, silence_warning=True)

    # reconstruct B0 -> J/psi Ks decay
    reconstructDecay('B0:jpsiks -> J/psi:mumu K_S0:pipi', '5.2 <= M <= 5.4', path=main)

    # Fit the B0 Vertex
    Rave('B0:jpsiks', 0., 'vertex', 'B0 -> [J/psi -> ^mu+ ^mu-] K_S0', '', path=main, silence_warning=True)
