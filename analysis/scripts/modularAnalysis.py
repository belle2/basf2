...
def MCGenTopoToTree(
    decayString,
    treename='MCGenTopo',
    filename='MCGenTopo.root',
    nMCGenMax=200,
    path=analysis_main,
):
    """
    Creates and fills a tree for the topology information of MC generated particles.
    To be specific, the information involves the following three variables:
        ...
        MCGenMothIndex (array of the mother indexes of the MC generated particles in a given event).

    Along with the variables, the event meta data, namely __experiment__, __run__, __event__, are always stored.
    In case a decayString is provided, __candidate__ and __ncandidates__ will be stored as well.

    MCGenPDG and MCGenMothIndex are not scalar variables but array variables with variable length equal to nMCGen.
    Hence, they can not be stored to flat ntuples with the module VariablesToNtuple.
    That's the reason why the module MCGenTopoToTree is developed.
    Note that the module is referred to as MCGenTopoToTree instead of MCGenTopoToNtuple,
    in order to avoid mistaking the tree as a flat ntuple with only scalar values stored.

    To keep consistent with the case of the module VariablesToNtuple:
        if a decayString is provided, one entry per candidate will be filled to the tree;
        otherwise, one entry per event will be filled to the tree.

    Parameters:
        decayString (str): specifies type of Particles and determines the name of the ParticleList
        treename (str): name of the tree
        filename (str): which is used to store the variables
        nMCGenMax (int): Maximum permissible number of MC generated particles in a given event
        path (basf2.Path): the basf2 path where the analysis is processed
    """

    output = register_module('MCGenTopoToTree')
    output.set_name('MCGenTopoToTree_' + decayString)
    output.param('particleList', decayString)
    output.param('treeName', treename)
    output.param('fileName', filename)
    output.param('nMCGenMax', nMCGenMax)
    path.add_module(output)


def variablesToNtuple_MCGenTopoToTree(
    decayString,
    variables,
    treename='variables',
    filename='ntuple.root',
    ...
    """
    This function is a combination of the following two functions: variablesToNtuple and MCGenTopoToTree.
    The first function creates and fills a flat ntuple with the specified variables from the VariableManager.
    The second function creates and fills a tree for the topology information of MC generated particles.
    For more details, please refer to the descriptions of the two functions.
    ...
    variablesToNtuple(decayString, variables, treename, filename, path)
    MCGenTopoToTree(decayString, treename + '_MCGenTopo', filename, nMCGenMax, path)
