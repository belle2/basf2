...
def loadStdXi(fitter='kfitter', path=analysis_main):
    ...
    cutAndCopyList(
        ...
        True, path=path)
    # pi-:all list is filled in mergedLambdas()
    # stdXi-
    if fitter == 'kfitter':
...
def loadStdXi0(gammatype='eff40', path=analysis_main):
    mergedLambdas(path=path)
    ...
    if gammatype == 'eff60':
        ...
        reconstructDecay(
            'pi0:std -> gamma:pi0eff60 gamma:pi0eff60',
            '0.08377 < M < 0.189577',
            True,
            path=path)
        ...
        reconstructDecay(
            'pi0:std -> gamma:pi0eff50 gamma:pi0eff50',
            '0.08377 < M < 0.189577',
            True,
            path=path)
        ...
        reconstructDecay(
            'pi0:std -> gamma:pi0eff40 gamma:pi0eff40',
            '0.08377 < M < 0.189577',
            True,
            path=path)
        ...
        reconstructDecay(
            'pi0:std -> gamma:pi0eff30 gamma:pi0eff30',
            '0.08377 < M < 0.189577',
            True,
            path=path)
        ...
        reconstructDecay(
            'pi0:std -> gamma:pi0eff20 gamma:pi0eff20',
            '0.08377 < M < 0.189577',
            True,
            path=path)
    ...
    reconstructDecay(
        'Xi0:prelim -> Lambda0:reco pi0:std',
        '1.225 < M < 1.405',
        path=path,
...
def loadBelleStdXi(fitter='kfitter', path=analysis_main):
    # Define the 2D cos(Alpha) variables for Xi and Lambda candidates with respect to the reconstructed Xi
    # Vertex vectors are defined from IP to Xi and Xi to Lambda
    stdPi('all', path=path)
...
def loadBelleStdXi0(massConst=False, path=analysis_main):
   ...
   [ daughter(0,atcPIDBelle(4,2)) > 0.2 ]',
    ...
    if not massConst:
        ...
        applyCuts('Xi0:prelim', '1.30986 < M < 1.31986', path=path)
        vertexTree(
            'Xi0:prelim',
            conf_level=0.0,
            massConstraint=[
