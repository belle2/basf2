...
def add_isSignalTags():
    """
    The function decode the mcErrors codes into human readable tags like `isSignalAcceptMissingGamma`.
    After release-05-00-00 all isSignalSomething tags would be implemented in here instead of
    analysis/variables/src/MCTruthVariables.cc.

    Example:

        Call this function before using isSignalSomething
        >>> variables.utils.add_isSignalTags()
    ...
    _variablemanager.addAlias("isExtendedSignal", "passesCut(mcErrors == " +
                              "%d)" % (c_MisID | c_AddedWrongParticle))
    _variablemanager.addAlias("isSignalAcceptWrongFSPs", "passesCut(mcErrors == " +
                              "%d)" % (c_MisID | c_AddedWrongParticle))
    ...
    _variablemanager.addAlias("isSignalAcceptBremsPhotons", "passesCut(mcErrors == " +
                              "%d)" % (c_AddedRecoBremsPhoton))
    _variablemanager.addAlias("isSignalAcceptMissingNeutrino", "passesCut(mcErrors == " +
                              "%d)" % (c_MissNeutrino))
    _variablemanager.addAlias("isSignalAcceptMissingMassive", "passesCut(mcErrors == " +
                              "%d)" % (c_MissMassiveParticle | c_MissKlong))
    _variablemanager.addAlias("isSignalAcceptMissingGamma", "passesCut(mcErrors == " +
                              "%d)" % (c_MissGamma))
    _variablemanager.addAlias("isSignalAcceptMissing", "passesCut(mcErrors == " +
                              "%d)" % (c_MissGamma | c_MissMassiveParticle | c_MissKlong | c_MissKlong))
