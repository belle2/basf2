...
def add_isSignalAcceptFlags():
    """
    The function decodes the mcErrors codes into human readable tags like `isSignalAcceptMissingGamma`,
    using meta function unmask(). After release-05-00-00 all isSignalSomething tags would be implemented
    in here instead of analysis/variables/src/MCTruthVariables.cc.
    ...
    # are there any way to read the error bit definition directly from MCTruthVariables.h to python? which would be
    # much safer when modifying the error bit definition
    ...
    _variablemanager.addAlias("isExtendedSignal", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MisID | c_AddedWrongParticle, c_Correct))
    _variablemanager.addAlias("isSignalAcceptWrongFSPs", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MisID | c_AddedWrongParticle, c_Correct))
    # the isPrimarySignal is implemented by
    # if (isSignal(part) > 0.5 and particleMCPrimaryParticle(part) > 0.5)
    # and I don't know whether it is equivalent to if (isSignal == 1 and mcParticleStatus % 2 == 0 )
    # _variablemanager.addAlias("isPrimarySignal", "passesCut(isSignal == 1 and mcParticleStatus % 2 == 0)")
    _variablemanager.addAlias("isSignalAcceptBremsPhotons", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_AddedRecoBremsPhoton, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissingNeutrino", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MissNeutrino, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissingMassive", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MissMassiveParticle | c_MissKlong, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissingGamma", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MissGamma, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissingGammaAndDecayInFlight", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MissGamma | c_DecayInFlight, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissingNeutrinoAndWrongFSP", "passesCut(unmask(mcErrors," +
                              "%d) == %d)" % (c_MissNeutrino | c_MisID | c_AddedWrongParticle, c_Correct))
    _variablemanager.addAlias("isSignalAcceptMissing", "passesCut(unmask(mcErrors," +
