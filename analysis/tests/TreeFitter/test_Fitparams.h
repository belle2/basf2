...
namespace {
  ...
  TEST_F(TreeFitterFitparTest, Constructor)
  {
    ...
    EXPECT_TRUE(fitParDim3.getStateVector().array().all() == 0.0) << "statevector not initialised to 0,0,...";

    /** all covariance matrix elemtns initialised to 0 */
    EXPECT_TRUE(fitParDim3.getCovariance().array().all() == 0.0) << "covariance not initialised to 0";
  }
  ...
  TEST_F(TreeFitterFitparTest, Functions)
  {
    ...
    EXPECT_TRUE(fitParDim3.getStateVector().array().all() == 0.0) << "fitParDim3.resetStateVector() didn't work";

    // cppcheck-suppress constStatement
    fitParDim3.getCovariance() << 1, 2, 3, 4, 5, 6, 7, 8, 9;
    fitParDim3.resetCovariance();
    EXPECT_TRUE(fitParDim3.getCovariance().array().all() == 0.0) << "fitParDim3.resetCovariance() didn't work";
