...
class MergingChecker(basf2.Module):
    ...
    def initialize(self):
        ...
        self.all = Belle2.PyStoreObj('e-:all')
...
ma.fillParticleListFromMC("e-:all", "", path=path)
ma.variablesToExtraInfo('e-:all', {'formula(0)': 'identifier', 'random': 'quality'}, path=path)
# load electrons with px below 2
ma.fillParticleListFromMC("e-:below2", "px < 2", path=path)
ma.variablesToExtraInfo('e-:below2', {'formula(2)': 'identifier', 'formula(1 + random)': 'quality'}, path=path)
# load electrons with px above 3
ma.fillParticleListFromMC("e-:above3", "px > 3", path=path)
ma.variablesToExtraInfo('e-:above3', {'formula(3)': 'identifier', 'formula(2 + random)': 'quality'}, path=path)
