...
# loose cut on energy
electrons = ('e+:FSP', 'eid > 0.1')
fillParticleLists([electrons], path=main)


# a) save signal dr using PreCutHistMaker
hist_params = (100, 0.0, 10.0)
histmaker = register_module('PreCutHistMaker')
histmaker.param('decayString', 'e+ -> e+:FSP')
histmaker.param('fileName', 'test_fspHist.root')
histmaker.param('histParams', hist_params)
histmaker.param('variable', 'dr')
histmaker.param('target', 'isSignal')
histmaker.param('cut', '')
...
ntupler.param('fileName', 'test_fspntuple.root')
ntupler.param('variables', ['dr', 'isSignal'])
ntupler.param('particleList', 'e+:FSP')
...
def checkHistograms(name, selection):
    ...
    ntuple.Project("ntuplehist", "dr", selection)
...
checkHistograms("alle+", "")
checkHistograms("signale+", "isSignal > 0.5")


print("Test passed, cleaning up.")
# cleanup
os.remove('test_fspntuple.root')
os.remove('test_fspHist.root')
