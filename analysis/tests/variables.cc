...
namespace {
  ...
  TEST_F(MetaVariableTest, basicMathTest)
  {
    ...
    ASSERT_NE(var, nullptr);
    EXPECT_FLOAT_EQ(var->function(&p), 1.5707963267948966); // 90

    var = Manager::Instance().getVariable("cos(1.0471975511965976)"); // cos 60
    ASSERT_NE(var, nullptr);
    EXPECT_FLOAT_EQ(var->function(&p), 0.5);
    ...
    var = Manager::Instance().getVariable("tan(0.7853981633974483)"); // tan 45
    ASSERT_NE(var, nullptr);
    EXPECT_FLOAT_EQ(var->function(&p), 1.0);

    var = Manager::Instance().getVariable("atan(1.0)");
    ASSERT_NE(var, nullptr);
    EXPECT_FLOAT_EQ(var->function(&p), 0.7853981633974483);
  }
