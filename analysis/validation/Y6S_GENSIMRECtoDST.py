...
"""
<header>
  <output>Y6S_genbb.dst.root</output>
...
evtgen.param('userDECFile',
             Belle2.FileSystem.findFile('generators/evtgen/decayfiles/Y6S_nonbsbs.dec'))
main.add_module(evtgen)
print_params(evtgen)

pdg.load(Belle2.FileSystem.findFile('generators/evtgen/decayfiles/Y6S.pdl'))  # to load a different file

# remove VXD from background mixer (Torben's suggestion)
# import glob
# bgfolder = '/sw/belle2/bkg'
# bg = glob.glob(bgfolder + '/*.root')
# add_simulation(main, bkgfiles=bg, bkgscale=0.025)
add_simulation(main)
# for m in main.modules():
#    if m.type() == "BeamBkgMixer":
#        m.param('scaleFactors', [('Coulomb_HER', 0.1), ('Coulomb_LER', 0.1), ('Touschek_HER', 0.1), ('Touschek_LER', 0.1)])
#        m.param('components', ['CDC', 'TOP', 'ARICH', 'ECL', 'BKLM', 'EKLM'])
...
output.param('outputFileName', '../Y6S_genbb.dst.root')
