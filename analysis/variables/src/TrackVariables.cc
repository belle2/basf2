...
namespace Belle2 {
  namespace Variable {
    ...
    double trackNVTXHits(const Particle* part)
    {
      return trackNHits(part, Const::EDetector::VTX);
    }

    double trackNVXDHits(const Particle* part)
    {
      return trackNPXDHits(part) + trackNSVDHits(part) + trackNVTXHits(part);
    }
...
track-based particle.
    ...
    REGISTER_VARIABLE("nVTXHits", trackNVTXHits,
                      "The number of VTX hits associated to the track. Returns NaN if called for something other than a track-based particle.");
    REGISTER_VARIABLE("nVXDHits", trackNVXDHits,
                      "The number of PXD, SVD, and VTX hits associated to the track. Returns NaN if called for something other than a track-based particle.");
...
mdst files processed with basf2 versions older than ``release-05-01``.
    ...
    REGISTER_VARIABLE("firstVTXLayer", trackFirstVTXLayer,
                      "The first activated VTX layer associated to the track. Returns NaN if called for something other than a track-based particle.");
