//
// $Id: FixMdst_ecl.cc 11174 2010-09-29 06:29:44Z hitoshi $
//
// $Log$
//
...
//double pi0resol(double p, double theta, char* side, bool mcdata, int exp )
  double FixMdstModule::pi0resol(double p, double theta, char* side, bool mcdata,
                                 int exp, int option)
  {
...
//  dout(Debugout::INFO,"FixMdst_ecl") <<" iside="<<iside<<std::endl;
//----------------------------------------------------
//        20 <= theta  <= 45
//----------------------------------------------------
//   0.15 < p  < 0.8
...
//--
    ...
    //     dout(Debugout::INFO,"FixMdst_ecl") <<" theta="<<theta<<" p="<<pbuf
    //     <<" para="<< para[0]<<", "<< para[1]<<", "
    //     << para[2]<<", "<<para[3]
...
// Implemented into FixMdst_ecl in 2002/11/30 by KM
//==================================================
//*******************************
// pi0resol: pi0 resolution function
//
...
//======================================================
  void FixMdstModule::correct_ecl(int option, int version)
  {
    ...
    //dout(Debugout::INFO,"FixMdst_ecl") << "This is version1.2.1" << std::endl;
    // dout(Debugout::INFO,"FixMdst_ecl") << "entering correct_ecl()" << std::endl;
    ...
    for (std::vector<Mdst_ecl>::iterator itecl = eclmgr.begin();
      ...
      if (expmc == 1 && m_reprocess_version == 0 && Eno == 45) {
        ...
        // dout(Debugout::DDEBUG,"FixMdst_ecl")<<"Exp45 fix idecl="<<idecl<<" cellID="<<cellID
        ...
        for (std::vector<Mdst_gamma>::iterator itgam45 = gammamgr.begin();
          ...
          if (gamma45.ecl().get_ID() == shower.get_ID()) {
            ...
            //if( factor45!=1.0 )
            //{
            // dout(Debugout::DDEBUG,"FixMdst_ecl")<< "Exp45 fix idgamma="<<idgam<<std::endl;
            //}
          }
        }
      }
        ...
        case 0: // Option set as default.
          if (expmc == 2) { // This event record is MC data.
            ...
            if (Eno == 15 && i_previous == 1) {
              // dout(Debugout::INFO,"FixMdst_ecl") << "Exp.15 version=1 applied case!" << std::endl;
              //dout(Debugout::INFO,"FixMdst_ecl") << "ecl factor=" << factor << " " ;
              ...
              //dout(Debugout::INFO,"FixMdst_ecl") << "factor(rel)=" << factor << " ";
            }
          }
          break;
        case 1:
      ...
      switch (option) {
        case 0: // Option as default.
          if (expmc == 2) { // This event record is MC data.
            ...
            if (Eno == 15 && i_previous == 1) {
              // dout(Debugout::INFO,"FixMdst_ecl") << "Exp.15 version=1 applied case!" << std::endl;
              //dout(Debugout::INFO,"FixMdst_ecl") << "factor=" << factor << " " ;
              // Original scaling is done by Exp.13 curve.
              //Original definition.
              //factor13
              // = ecl_adhoc_corr( 13, 1,
              ...
              factor13
                = ecl_adhoc_corr(13, 1, ecl_threshold,
                                 gamma_energy, gamma_cos);
              factor = factor / factor13;
              //dout(Debugout::INFO,"FixMdst_ecl") << "gamma factor(rel)=" << factor << " " << std::endl;
            }
          }
          break;
        case 1:
...
//*********************************************************
  void FixMdstModule::make_pi0(int option, double low_limit, double up_limit)
  {
    //---------------------
    ...
    switch (option) {
      ...
      case 2: // pi0 cand. are selected by -Xsigma < Mgg-Mpi0 < +Xsigma.
        // dout(Debugout::DDEBUG,"FixMdst_ecl") << "option=2 was selected." << std::endl;
        //dbg dout(Debugout::INFO,"FixMdst_ecl") << "mass window is " << low_limit << " & ";
        //dbg dout(Debugout::INFO,"FixMdst_ecl") << up_limit << std::endl;
    ...
    for (std::vector<Mdst_gamma>::iterator itgamma = gammamgr.begin();
      ...
      for (std::vector<Mdst_gamma>::iterator jtgamma = itgamma + 1;
        ...
        if (low_limit < mass_ctrl && mass_ctrl < up_limit) {
          // dout(Debugout::DDEBUG,"FixMdst_ecl")<<"mass="<<mass_before;
          // dout(Debugout::DDEBUG,"FixMdst_ecl")<<" p="<<gamgam_lv.vect().mag()<<" theta=";
          // dout(Debugout::DDEBUG,"FixMdst_ecl")<<gamgam_lv.theta()<<" mcdata="<<mcdata;
          // dout(Debugout::DDEBUG,"FixMdst_ecl")<<" higher="<<
          //   pi0resol(gamgam_lv.vect().mag(),
          //    gamgam_lv.theta()*180./M_PI,
          //  "higher",mcdata, Eno, option )<<" or ";
          // dout(Debugout::DDEBUG,"FixMdst_ecl")<<" lower="<<
          //   pi0resol(gamgam_lv.vect().mag(),
          //    gamgam_lv.theta()*180./M_PI,
  ...
  void FixMdstModule::make_pi0_primary_vertex(int option, double low_limit, double up_limit,
                                              ...
                                              const HepSymMatrix& epvtx_err)
  {
    ...
    switch (option) {
      ...
      case 1:   // option=1 case, check given mass window.
        if (mpi0_pdg < low_limit || up_limit < mpi0_pdg) {
          // If mass window is not correct, do nothing.
          dout(Debugout::ERR, "FixMdst_ecl") << "Invalid mass window between " << low_limit;
          dout(Debugout::ERR, "FixMdst_ecl") << " and " << up_limit << std::endl;
          return;
        }
        break;
      default: // Otherwise, invalid option, do nothing.
        dout(Debugout::ERR, "FixMdst_ecl") << "Invalid option=" << option << std::endl;
        return;
    }
  ...
  void FixMdstModule::correct_ecl_primary_vertex(const HepPoint3D& epvtx,
                                                 const HepSymMatrix& epvtx_err)
  {
