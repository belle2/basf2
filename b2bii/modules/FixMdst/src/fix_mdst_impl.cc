//
// $Id: FixMdst_impl.cc 11331 2011-07-25 11:58:12Z hitoshi $
//
// $Log$
//
...
#include "panther/BeamEnergy.h"

#include "panther/mdst.h"
#include "panther/belletdf.h"
#include "panther/evtcls.h"
#include "panther/dEdxCalib.h"
#include "panther/level4.h"
...
#define NOMINAL_ENERGY 5.290024915
  ...
  double FixMdstModule::s_benergy_value = NOMINAL_ENERGY;

  FixMdstModule::FixMdstModule() : Module()
  {
  ...
  void FixMdstModule::initialize()
  {
    Muid_init();
  }

  void FixMdstModule::terminate()
  {
    Muid_term();
  }

  void FixMdstModule::event()
  {

    if ((BsCouTab(BELLE_EVENT) == 0) || (BsCouTab(MDST_EVENT_ADD) == 0)) {
      B2INFO("Warning from FixMdst: No Belle_event or Mdst_event_add table; no correction for event");
      setReturnValue(-1);
      return;
    }
    ...
    if (m_good_event == 1) {
      if (good_event()) {
        ...
        setReturnValue(-1);
        B2INFO("FixMdst: Not a good event");
        return;
      }
    } else {
      setReturnValue(0);
    ...
    if (m_table_size == 1) {
      ...
      if ((m_reprocess_version == 0 && bevt.count() > 0 && bevt[0].ExpNo() >= 39) ||
        ...
        if (Mdst_ecl_trk_Manager::get_manager().count() > m_limit_mdst_ecl_trk) {
          setReturnValue(-1);
          B2INFO("FixMdst: " <<  Mdst_ecl_trk_Manager::get_manager().count() << " " << m_limit_mdst_ecl_trk);
          return;
        }
        if (Mdst_klm_cluster_hit_Manager::get_manager().count()
            > m_limit_mdst_klm_cluster_hit) {
          setReturnValue(-1);
          B2INFO("FixMdst: " <<  Mdst_klm_cluster_hit_Manager::get_manager().count() << " " << m_limit_mdst_klm_cluster_hit);
          return;
        }
      }
    }
  ...
  void FixMdstModule::beginRun()
  {
    ...
    if (belleevt != evtmgr.end() && (*belleevt)) {
      ...
      B2ERROR("[FixMdst] Error: Cannot read \"Belle_RunHead\".");
    }
  }

  int
  FixMdstModule::get_reprocess_version()
  {
...
//#include BELLETDF_H
  ...
  double FixMdstModule::Benergy(int expnum, int runnum)
  {
