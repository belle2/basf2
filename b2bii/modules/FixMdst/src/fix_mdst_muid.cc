...
// Created:     31-Jan-2001
//
// $Id: FixMdst_muid.cc 9944 2006-11-29 07:36:07Z katayama $
//
// Revision history
//
// $Log$
...
// muid_user is absorped in FixMdst (by Piilonen).
//
...
// Start of muid routines in FixMdst __________________________________

  void FixMdstModule::Muid_init(void)
  {
    ...
    if (m_mapped_expno > 0) {
      ...
      B2INFO("%FixMdst muid is initialized by basf with"
      ...
      B2INFO("%FixMdst muid is disabled by user (ExpNo = "
             << m_expno << " is negative)");
    }
  ...
  void FixMdstModule::Muid_term(void)
  {
  ...
  void FixMdstModule::Muid_begin_run(int expmc, int exp, int run)
  {
  ...
  void FixMdstModule::Muid_end_run(void)
  {
...
// End of muid routines in FixMdst ____________________________________
  ...
  void MuidProb::readDB(const char* dbtemplate, int& expNo)
  {
    ...
    if (pathname == "") {
      B2ERROR("%MuidProb: Failed to open database file."
              << "Module FixMdst muid will do nothing.");
      expNo = 0;
      return;
    }
