//
// $Id: FixMdst_trk.cc 11331 2011-07-25 11:58:12Z hitoshi $
//
// $Log$
//
...
//==========================================================
  double FixMdstModule::vee_mass_nofit(const Mdst_vee2& vee2, float scale)
  {
//==========================================================
...
//in case where decay vtx is outside of beam pipe
    ...
    if (vee2.daut()) {
      ...
      Helix  helixm(vtx, a);
      // dout(Debugout::DDEBUG,"FixMdst_trk") << "outside bp" << std::endl;
...
//in case where decay vtx is inside of beam pipe
      ...
      a[2] = a[2] / scale;
      helixm.a(a);
      // dout(Debugout::DDEBUG,"FixMdst_trk") << "inside bp" << std::endl;
      HepLorentzVector v0 = helixp.momentum(0., massp) + helixm.momentum(0., massm);
      return v0.mag();
...
//===============================================================
  void FixMdstModule::scale_momenta(float scale_data, float scale_mc, int mode)
  {
//===============================================================
...
//======================================================
    ...
    // dout(Debugout::DDEBUG,"FixMdst_trk") << "sacel momenta is called" << std::endl;

//Check existence of belle_event
    Belle_event_Manager& evtmgr = Belle_event_Manager::get_manager();
    // dout(Debugout::DDEBUG,"FixMdst_trk") << evtmgr.count() << std::endl;
    if (0 == evtmgr.count()) return; //do nothing if not exist
    // dout(Debugout::DDEBUG,"FixMdst_trk") << "after check" << std::endl;
...
//set scale factor
    ...
    // dout(Debugout::DDEBUG,"FixMdst_trk") << scale << std::endl;
...
//Turn on flag, save scale factor
    ...
    // dout(Debugout::DDEBUG,"FixMdst_trk") << mevtmgr[0].flag_scale() << std::endl;
    // dout(Debugout::DDEBUG,"FixMdst_trk") << mevtmgr[0].scale() << std::endl;
...
//=====================================================================
  void FixMdstModule::scale_momenta_set(const int mode, const int expno,
...
//=====================================================================
  void FixMdstModule::scale_momenta_set_v1(const int mode, const int expno,
                                           const int runno, double& scale)
  {
//=====================================================================
...
//=====================================================================
  void FixMdstModule::scale_momenta_set_v2(const int mode, const int expno,
                                           const int runno, double& scale)
  {
//=====================================================================
...
//
  ...
  /**
   * The implementation of FixMdstModule::scale_error()
  ...
  void FixMdstModule::scale_error(const int message_level)
  {
...
////////////////////////////////////////////////////////////////////////
  ...
  int FixMdstModule::set_primary_vertex(HepPoint3D& epvtx,
...
//=======================================================
  ...
  int
  FixMdstModule::add_extra_trk_vee2()
  {
    Mdst_event_add_Manager& mevtmgr = Mdst_event_add_Manager::get_manager();
    ...
    for (std::vector<Mdst_trk_extra>::const_iterator i = teMgr.begin();
      ...
      if (!(trk.quality() & 15u)) {
        ...
        B2ERROR("Warning from FixMdst: strange track in Mdst_trk_extra: quality="
                << trk.quality());
      }
    }
    ...
    sheMgr.remove();
    veMgr.remove();
    if (i_ch & 0xffffff00) B2ERROR("Warning from FixMdst: overflow! i_ch = " << i_ch);
    if (i_trk & 0xffffff00) B2ERROR("Warning from FixMdst: overflow! i_trk = " << i_trk);
    if (i_vee2 & 0xffffff00) B2ERROR("Warning from FixMdst: overflow! i_vee2 = " << i_vee2);
    flag = i_ch & 0xff;
    flag |= ((i_trk & 0xff) << 8);
    flag |= ((i_vee2 & 0xff) << 16);
...
#if 0
  int
  FixMdstModule::remove_extra_trk_vee2()
  {
    Mdst_event_add_Manager& mevtmgr = Mdst_event_add_Manager::get_manager();
  ...
  FixMdstModule::remove_extra_trk_vee2()
  {
    ...
    if (i_trk) {
      ...
      for (std::vector<Mdst_trk>::iterator i = tMgr.begin();
        ...
        if (!((*i).quality() & 16u)) {
          B2ERROR("Warning from FixMdst: inconsistency between Mdst_trk and Mdst_evet_add"
                 );
        }
      ...
      if (i_vee2) {
        ...
        for (std::vector<Mdst_vee2>::iterator i = veeMgr.begin();
          ...
          if (!((*i).chgd(0).trk().quality() & 16u) && !((*i).chgd(1).trk().quality() & 16u)) {
            B2ERROR("Warning from FixMdst: inconsistency between Mdst_vee2 and Mdst_evet_add"
                   );
          }
          ...
          if (extra_id1) {
            ...
            if (vee.chgd_ID()) {
              B2ERROR("Warning from FixMdst: both tracks of Mdst_vee2_extra are good track"
                     );
            }
            vee.chgd((*i).chgd(1));
          }
