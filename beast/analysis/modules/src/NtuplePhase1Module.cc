...
namespace Belle2 {
  ...
  void NtuplePhase1Module::event()
  {
    ...
    const float P_HER = m_skb.VAHCCG_HER_PRES_AVG;
    const float P_LER = m_skb.VALCCG_LER_PRES_AVG;
    ...
    */
    ...
    float coefBG_LER = I_LER * P_LER / (m_input_I_LER[0] * m_input_P_LER[0]);
    float coefBG_HER = I_HER * P_HER / (m_input_I_HER[0] * m_input_P_HER[0]);
    float coefTo_LER = TMath::Power(I_LER / m_input_I_LER[0],
                                    2) / (bunch_nb_LER / m_input_bunchNb_LER[0]) / (sigma_y_LER / m_input_sigma_LER[0]);
    float coefTo_HER = TMath::Power(I_HER / m_input_I_HER[0],
