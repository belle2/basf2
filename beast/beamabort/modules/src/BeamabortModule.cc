...
#include <beast/beamabort/modules/BeamabortModule.h>
...
using namespace Belle2::beamabort;
...
//XxxModule
REG_MODULE(Beamabort);


BeamabortModule::BeamabortModule() : Module(), m_intParameter(0), m_doubleParameter(0), m_stringParameter("")
{
  setDescription("Creates BEAMABORT crystals - sub-detector of BEASTII");
...
void BeamabortModule::initialize()
{
  B2INFO("Beamabort: Initialize");
...
void BeamabortModule::beginRun()
{
  B2INFO("Beamabort: Begin of new run");
  //Here comes the initialisation specific to each run
}

void BeamabortModule::event()
{
  B2INFO("Beamabort: Event is being processed");
  ...
  RelationIndex<MCParticle, BeamabortSimHit> relMCSimHit(mcParticles, simHits);

  //Lets loop over all created BeamabortSimHits:
  //int nSimHits = simHits.getEntries();
  //for (int i = 0; i < nSimHits; ++i) {
  //BeamabortSimHit& hit = *simHits[i];
  //Find all MCParticles which point to that SimHit and the corresponding weight
  //RelationIndex<MCParticle, BeamabortSimHit>::range_to range = relMCSimHit.getElementsTo(hit);
  //for (; range.first != range.second; ++range.first) {
  //And Print something about the relation
  //const RelationIndex<MCParticle, BeamabortSimHit>::Element& relation = *range.to;
  //B2INFO("BeamabortSimHit #" << i << " has an energy deposition of " << hit.getEnergyDep()
  ...
  for (int i = 0; i < nMCParticles; ++i) {
    ...
    //Find all BeamabortSimHits which point from that MCParticle using a typedef and BOOST_FOREACH
    //The typedef is needed as BOOST_FOREACH is a macro and cannot handle anything including a comma
    typedef RelationIndex<MCParticle, BeamabortSimHit>::Element relMCSimHit_Element;
...
void BeamabortModule::endRun()
{
  B2INFO("Beamabort: End of run");
  //Here cleanup after each run
}


void BeamabortModule::terminate()
{
  B2INFO("Beamabort: Terminate");
  //Here final cleanup
}
