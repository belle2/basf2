...
#include <beast/claws/modules/CLAWSModule.h>
...
CLAWSModule::CLAWSModule() : Module(), m_intParameter(0), m_doubleParameter(0), m_stringParameter("")
{
  setDescription("Creates CLAWS crystals - sub-detector of BEASTII");
...
void CLAWSModule::beginRun()
{
  B2INFO("CLAWS: Begin of new run");
  //Here comes the initialisation specific to each run
}

void CLAWSModule::event()
{
  B2INFO("CLAWS: Event is being processed");
  ...
  RelationIndex<MCParticle, CLAWSSimHit> relMCSimHit(mcParticles, simHits);

  //Lets loop over all created CLAWSSimHits:
  //int nSimHits = simHits.getEntries();
  //for (int i = 0; i < nSimHits; ++i) {
  //CLAWSSimHit& hit = *simHits[i];
  //Find all MCParticles which point to that SimHit and the corresponding weight
  //RelationIndex<MCParticle, CLAWSSimHit>::range_to range = relMCSimHit.getElementsTo(hit);
  //for (; range.first != range.second; ++range.first) {
  //And Print something about the relation
  //const RelationIndex<MCParticle, CLAWSSimHit>::Element& relation = *range.to;
  //B2INFO("CLAWSSimHit #" << i << " has an energy deposition of " << hit.getEnergyDep()
  ...
  for (int i = 0; i < nMCParticles; ++i) {
    ...
    //Find all CLAWSSimHits which point from that MCParticle using a typedef and BOOST_FOREACH
    //The typedef is needed as BOOST_FOREACH is a macro and cannot handle anything including a comma
    typedef RelationIndex<MCParticle, CLAWSSimHit>::Element relMCSimHit_Element;
...
void CLAWSModule::endRun()
{
  B2INFO("CLAWS: End of run");
  //Here cleanup after each run
}


void CLAWSModule::terminate()
{
  B2INFO("CLAWS: Terminate");
  //Here final cleanup
}
