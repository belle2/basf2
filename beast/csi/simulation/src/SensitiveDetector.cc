...
namespace Belle2 {
  ...
  namespace csi {
      ...
      Simulation::SensitiveDetectorBase("CsiSensitiveDetector", Const::invalidDetector)
    {
      ...
      // CsiHitIndex: array of 12 (Nb. crystals) x 80 (max time index (tof [ns]/100))
      for (int i = 0; i < 12; i++) {for (int j = 0; j < 80; j++) { CsiHitIndex[i][j] = 0;}}
      ...
      G4ThreeVector VecCell)
    {
      ...
      */
      ...
      if (firstcall == 0 || m_currentEvnetNumber != m_oldEvnetNumber) {
        ...
        for (int iCSICell = 0; iCSICell < 12; iCSICell++) {
