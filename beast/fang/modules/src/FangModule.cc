...
#include <beast/fang/modules/FangModule.h>
...
namespace Belle2 {
  ...
  namespace fang {
    ...
    FangModule::FangModule() : Module(), m_intParameter(0), m_doubleParameter(0), m_stringParameter("")
    {
      setDescription("Creates FANG crystals - sub-detector of BEASTII");
    ...
    void FangModule::initialize()
    {
      B2INFO("Fang: Initialize")
    ...
    void FangModule::beginRun()
    {
      B2INFO("Fang: Begin of new run");
      //Here comes the initialisation specific to each run
    }

    void FangModule::event()
    {
      B2INFO("Fang: Event is being processed");
      ...
      RelationIndex<MCParticle, FangSimHit> relMCSimHit(mcParticles, simHits);

      //Lets loop over all created FangSimHits:
      //int nSimHits = simHits.getEntries();
      //for (int i = 0; i < nSimHits; ++i) {
      //FangSimHit& hit = *simHits[i];
      //Find all MCParticles which point to that SimHit and the corresponding weight
      //RelationIndex<MCParticle, FangSimHit>::range_to range = relMCSimHit.getElementsTo(hit);
      //for (; range.first != range.second; ++range.first) {
      //And Print something about the relation
      //const RelationIndex<MCParticle, FangSimHit>::Element& relation = *range.to;
      //B2INFO("FangSimHit #" << i << " has an energy deposition of " << hit.getEnergyDep()
      ...
      for (int i = 0; i < nMCParticles; ++i) {
        ...
        //Find all FangSimHits which point from that MCParticle using a typedef and BOOST_FOREACH
        //The typedef is needed as BOOST_FOREACH is a macro and cannot handle anything including a comma
        typedef RelationIndex<MCParticle, FangSimHit>::Element relMCSimHit_Element;
    ...
    void FangModule::endRun()
    {
      B2INFO("Fang: End of run");
      //Here cleanup after each run
    }


    void FangModule::terminate()
    {
      B2INFO("Fang: Terminate");
      //Here final cleanup
    }
