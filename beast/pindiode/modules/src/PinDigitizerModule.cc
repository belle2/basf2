...
void PinDigitizerModule::event()
{
  ...
  //auto columnArray = new vector<int>[nPIN](); //column
  //auto rowArray = new vector<int>[nPIN](); //row
  //auto bcidArray = new vector<int>[nPIN](); //BCID
  //auto totArray = new vector<int>[nPIN](); //TOT
  /** Initialize 3D array */
  ...
  //Determine T0 for each PIN
  int nentries = PinSimHits.getEntries();

  double T0[nPIN];
  bool PixelFired[nPIN];
  for (int i = 0; i < nPIN; i++) {
    T0[i] = 1000000.;
    PixelFired[i] = false;
  }
  for (int i = 0; i < nentries; i++) {
    ...
    //double T = posn.Z() / 100. + (PINCenter[detNb].Z() + 10.) + m_z_DG;
    double T = posn.Z() / 100. - PINCenter[detNb].Z() + m_z_DG;
  ...
  //int oldPID=0;
  //loop on all entries to store in 3D the ionization for each PIN
  for (int i = 0; i < nentries; i++) {
    PindiodeSimHit* aHit = PinSimHits[i];
    ...
    double xpos = position.X() / 100. - PINCenter[detNb].X();
    double ypos = position.Y() / 100. - PINCenter[detNb].Y();
    //double zpos = position.Z() / 100. + (PINCenter[detNb].Z() + 10) + m_z_DG;
    double zpos = position.Z() / 100. - PINCenter[detNb].Z() + m_z_DG;
          ...
          for (int ig1 = 0; ig1 < (int)GEM_gain1; ig1++) {
            ...
            for (int ig2 = 0; ig2 < (int)GEM_gain2; ig2++) {
              ...
              if ((0 <= col && col < m_ChipColumnNb) &&
                ...
                //store info into 3D array for each PINs
...
void PinDigitizerModule::Drift(double x1, double y1, double z1, double& x2, double& y2, double& z2, double& t2, double st,
                               double sl, double vd)
{
  //check if
  if (z1 > 0.) {
...
void PinDigitizerModule::GEMGeo1(double x1, double y1, double& x2, double& y2)
{
...
void PinDigitizerModule::GEMGeo2(double x1, double y1, double& x2, double& y2)
{
...
//Make pixel hit
bool PinDigitizerModule::Pixelization(int detNb)
{
  ...
  if (bci.size() > 0) {
    ...
    for (int i = 0; i < (int)bci.size(); i++) {
      ...
      //create PindiodeHit
      //PinHits.appendNew(PindiodeHit(col[i], row[i], bci[i] - t0[0], ToT[i], detNb, pid[i]));
      PinHits.appendNew(PindiodeHit(col[i], row[i], bci[i] - t0[0], ToT[i], detNb));
    }
