...
namespace Belle2 {
   ...
   *  Each sector [octant] contains BKLMModules.  The local geometry of a
  ...
  class BKLMSector {
    ...
    //! Output operator
    //friend std::ostream& operator<<( std::ostream& out, const BKLMSector& sector );
  ...
  public:
    ...
    BKLMSector(const BKLMSector& s);

    //! Destructor
    ~BKLMSector();

    //! Assignment operator (includes list of contained modules)
    BKLMSector& operator=(const BKLMSector& s);

    //! Comparison operator (ordering)
    bool operator<(const BKLMSector&) const;

    //! Comparison operator (equality)
    bool operator==(const BKLMSector& s) const { return isSameSector(s); }

    //! Add a module to this sector
    void addModule(BKLMModule*);

    //! Determine if two sectors are identical
    const bool isSameSector(int frontBack, int sector) const;

    //! Determine if two sectors are identical
    const bool isSameSector(const BKLMSector& s) const;

    //! Determine if two sectors are identical
    const bool isSameSector(const BKLMModule& m) const;
    ...
    const BKLMModule* findModule(int module) const;
    ...
    const BKLMRect localToGlobal(const BKLMRect& r) const;

    //! Transform space-points of bounding rectangle from global to local coordinates
    const BKLMRect globalToLocal(const BKLMRect& r) const;
  ...
  private:
    ...
    std::vector<BKLMModule*> m_Modules;
