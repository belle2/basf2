...
BKLMModule::BKLMModule()
{
}

BKLMModule::BKLMModule(int               frontBack,
                       int               sector,
                       int               layer,
...
BKLMModule::BKLMModule(const BKLMModule& m) :
...
BKLMModule::BKLMModule& operator=( const BKLMModule& m ) {
  if ( this != &m ) {
    m_FrontBack = m.m_FrontBack;
    m_Sector = m.m_Sector;
    m_Layer = m.m_Layer;
...
bool BKLMModule::operator<(const BKLMModule& m) const
{
...
const bool BKLMModule::isSameModule(int frontBack, int sector, int layer) const
{
...
const bool BKLMModule::isSameModule(const BKLMModule& m) const
{
...
const double BKLMModule::getLocalCoordinate(double stripAve, char direction) const
{
  if (direction == 'P') {
...
const CLHEP::Hep3Vector BKLMModule::getLocalPosition(double phiStripAve, double zStripAve) const
{
  return CLHEP::Hep3Vector(m_LocalX,
                           getLocalCoordinate(phiStripAve, 'P'),
                           getLocalCoordinate(zStripAve, 'Z')) + m_Shift;
}

const CLHEP::HepMatrix BKLMModule::getLocalError(int phiStripMultiplicity, int zStripMultiplicity) const
{
  CLHEP::HepMatrix error(3, 3, 0);

  // Quick calculation... should be replaced with measured values
...
const BKLMRect BKLMModule::getStripRectLocal(double stripAve, char direction) const
{
  BKLMRect rect;
...
const BKLMRect BKLMModule::getModuleRectLocal(void) const
{
  BKLMRect rect;
...
const bool BKLMModule::isInActiveArea(const CLHEP::Hep3Vector& pos, int& phiStrip, int& zStrip) const
{
  CLHEP::Hep3Vector posShifted = pos - m_Shift;
...
const void BKLMModule::getStripDivisions(const CLHEP::Hep3Vector& pos, double& phiStripDiv, double& zStripDiv) const
{
  CLHEP::Hep3Vector posShifted = pos - m_Shift;
...
void BKLMModule::printTree() const
{
  B2INFO("BKLMModule: BKLM-"   << (m_FrontBack == 0 ? 'F' : 'B')
         << "-S"   << m_Sector
...
/*
std::ostream& operator<<( std::ostream& out, const BKLMModule& m )
{
  out << "BKLM-" << (m.m_FrontBack == 0 ? 'F' : 'B')
      << "-S"    << m.m_Sector
      << (m_Layer < 10 ? "-L0" : "-L") << m.m_Layer
      << "    "  << m.m_Shift
      << " x: "  << m.m_LocalX;
  return out;
}
