...
void BKLMUnpackerModule::event()
{
  ...
  B2DEBUG(101, "BKLMUnpackerModule:: there are " << m_rawKLMs.getEntries() << " RawKLM entries");
  for (int i = 0; i < m_rawKLMs.getEntries(); i++) {

    if (m_rawKLMs[i]->GetNumEvents() != 1) {
      B2DEBUG(1, "BKLMUnpackerModule:: RawKLM has more than one entry"
              << LogVar("RawKLM.Index", i)
              << LogVar("RawKLM.Entries", m_rawKLMs[i]->GetNumEvents()));
      continue;
    }

    B2DEBUG(101, "BKLMUnpackerModule:: events in buffer: " << m_rawKLMs[i]->GetNumEvents() << " ; number of nodes (copper boards): " <<
    ...
    for (int j = 0; j < m_rawKLMs[i]->GetNumEntries(); j++) {
      ...
      for (int finesse_num = 0; finesse_num < 4; finesse_num++) {
        ...
        for (int k = 0; k < numDetNwords; k++) {
          ...
          char buffer[200] = "";
          sprintf(buffer, "%.8x\n", item);
          B2DEBUG(101, "BKLMUnpackerModule:: " << buffer);
        ...
        if (numDetNwords % hitLength != 1 && numDetNwords != 0) {
          if (!m_keepEvenPackages) {
            B2DEBUG(1, "BKLMUnpackerModule:: word count incorrect: "
                    << LogVar("WordCount", numDetNwords));
            continue;
          }
        }
        B2DEBUG(101, "BKLMUnpackerModule:: this finesse has " << numHits << " hits");

        if (numDetNwords > 0)
          B2DEBUG(101, "BKLMUnpackerModule:: counter is: " << (buf_slot[numDetNwords - 1] & 0xFFFF));
        ...
        for (int iHit = 0; iHit < numHits; iHit++) {
          ...
          B2DEBUG(101, "BKLMUnpackerModule:: unpacking first word: " << buf_slot[iHit * hitLength + 0] << ", second: " << buf_slot[iHit *
                  hitLength + 1]);
          ...
          B2DEBUG(101, "BKLMUnpackerModule:: unpacking " << bword1 << ", " << bword2 << ", " << bword3 << ", " << bword4);
          ...
          B2DEBUG(101, "BKLMUnpackerModule:: unpacked info: channel: " << channel << ", axis: " << axis << " lane: " << lane << " ctime: " <<
                  ctime << " tdc: " << tdc << " charge: " << charge);

          B2DEBUG(101, "BKLMUnpackerModule:: copper: " << copperId << " finesse: " << finesse_num);
          ...
          if (m_electIdToModuleId.find(electId) == m_electIdToModuleId.end()) {
            if (!m_useDefaultModuleId) {
              B2DEBUG(1, "BKLMUnpackerModule:: could not find in mapping"
            ...
            B2DEBUG(101, "BKLMUnpackerModule:: electId: " << electId << " module: " << moduleId);
          ...
          if (layer > 14) {
            B2DEBUG(1, "BKLMUnpackerModule:: strange that the layer number is larger than 14 "
                    << LogVar("Layer", layer));
            continue;
          }
          ...
          if (outRange) {
            ...
            m_rejectedCount++;
            B2DEBUG(2, "BKLMUnpackerModule:: channel number is out of range"
          ...
          B2DEBUG(101, "BKLMUnpackerModule:: digit after Unpacker: sector: " << bklmDigit->getSector() << " isForward: " <<
                  bklmDigit->isForward()
                  << " layer: " << bklmDigit->getLayer() << " strip: " << bklmDigit->getStrip() << " isPhi: " << bklmDigit->isPhiReadout());
          B2DEBUG(101, "BKLMUnpackerModule:: charge " << bklmDigit->getCharge() << " tdc" << bklmDigit->getTime() << " ctime " <<
                  bklmDigit->getCTime() <<
...
void BKLMUnpackerModule::terminate()
{
  for (const auto& message : m_rejected) {
    B2DEBUG(1, "BKLMUnpackerModule:: " << message.first << " (occured " << message.second << " times)");
  }
}

void BKLMUnpackerModule::loadMapFromDB()
{
  B2DEBUG(101, "BKLMUnpackerModule:: reading from database...");

  DBArray<BKLMElectronicMapping> elements;
  for (const auto& element : elements) {
    B2DEBUG(101, "BKLMUnpackerModule:: version = " << element.getBKLMElectronictMappingVersion() << ", copperId = " <<
            element.getCopperId() <<
    ...
    B2DEBUG(101, "BKLMUnpackerModule:: electId: " << elecId << " moduleId: " << moduleId);
  }
}
