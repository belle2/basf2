...
void BKLMSimulationPar::read(const GearDir& content, unsigned int randomSeed, bool doBackgroundStudy)
{

  m_RandomSeed = randomSeed;
  m_DoBackgroundStudy = doBackgroundStudy;
  m_HitTimeMax = content.getParamNumValue("/HitTimeMax");    // ns
  ...
  for (div = 0; div < nDiv; ++div) {
    ...
    for (j = 1; j <= nWeight; ++j) {
      ...
      weight[j] = divContent.getParamNumValue(name) + weight[j-1];
      weight[0] = weight[j];
    }
