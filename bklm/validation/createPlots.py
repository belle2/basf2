...
def draw_bklmhists(file_chain):
    ...
    forward.GetListOfFunctions().Add(TNamed('Description', 'Flag indicating if muon hit is at z < 47 cm (0) or z > 47 cm (1)'))
    forward.GetListOfFunctions().Add(TNamed('Check', 'Somewhat more at backward end'))
    forward.GetListOfFunctions().Add(TNamed('Contact', 'piilonen@vt.edu'))
    forward.GetListOfFunctions().Add(TNamed('MetaOptions', 'pvalue-warn=0.99,pvalue-error=0.90'))
