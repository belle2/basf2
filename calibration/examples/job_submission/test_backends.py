...
test_script = Path(find_file("calibration/examples/job_submission/test_script.sh"))
test_basf2_script = Path(find_file("calibration/examples/job_submission/basic_basf2.py"))
test_data = Path(find_file("calibration/examples/job_submission/test_data"))


def test_path_exists(path):
    if not path.exists():
        raise FileNotFoundError(f"The expected file {test_script.as_posix()} does not exist.")
...
def create_jobs(args):
    ...
    # Output directory, where the stdout and stderr will be sent. We have set this to be the same as the working dir
