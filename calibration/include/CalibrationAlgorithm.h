  ...
  namespace calibration {
    ...
    public:
      ...
      struct MyDBQuery {
         ...
         */
        MyDBQuery(const std::string& aPackage = "", const std::string& aModule = "", TObject* aObject = 0,
       ...
       */
      std::string getCollectorName() {return getPrefix();}
      ...
      E_Result execute(std::vector<ExpRun> runs);

      /// Run calibration over list of exp.run given as string separated by comma
      E_Result execute(std::string expRunList)
      {
        return execute(string2RunList(expRunList));
      }

      /// Get constants (in TObjects) for database update from last calibration
      std::list<Database::DBQuery> getPayloads()
      {
    ...
    protected:
      ...
      virtual E_Result calibrate() = 0;

      // Helpers ---------------- Data retrieval -------

      /// Get the list of runs for which calibration is called
      std::vector<ExpRun> getRunList() {return m_runs;}

      /// Get calibration data object by name and list of runs
      template<class T>
      T& getObject(string name, vector< CalibrationAlgorithm::ExpRun > runlist)
      {
      ...
      template<class T>
      T& getObject(string name, CalibrationAlgorithm::ExpRun run)
      {
    ...
    private:
      ...
      std::string runList2String(ExpRun run);
      /// Get string repr. of list((exp,run) for CalibRootObj
      std::string runList2String(std::vector<ExpRun>& list);
      /// Get list((exp,run)) from string repr. in CalibRootObj
      std::vector<ExpRun> string2RunList(std::string list);

      /// Description of the algorithm
      std::string m_description = "Add short description of your algorithm...";

      /// The prefix of datastore objects
      std::string m_prefix = "";
      /// Runs for which the calibration has been last requested
      std::vector<ExpRun> m_runs = {};
      /// Payloads generated by last calibration
      std::list<MyDBQuery> m_payloads = {};
