...
namespace Belle2 {
   ...
   */
  class CalibrationAlgorithm_OLD {
  public:
     ...
     */
    explicit CalibrationAlgorithm_OLD(const std::string& collectorModuleName) : m_prefix(collectorModuleName) {}

    /// Virtual destructor (base class)
    virtual ~CalibrationAlgorithm_OLD() {}
  ...
  protected:
    ...
    template<class T>
    T& getObject(const std::string& name, std::vector<CalibrationAlgorithm_OLD::ExpRun> runlist) const
    {
    ...
    template<class T>
    T& getObject(std::string name, CalibrationAlgorithm_OLD::ExpRun run) const
    {
  ...
  private:
    ...
    ClassDef(CalibrationAlgorithm_OLD, 1); /**< Abstract base class for calibration algorithms */
  };
} // namespace Belle2
