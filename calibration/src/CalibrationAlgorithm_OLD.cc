...
const std::string CalibrationAlgorithm_OLD::RUN_RANGE_OBJ_NAME = "__ca_data_range";

CalibrationAlgorithm_OLD::EResult CalibrationAlgorithm_OLD::execute(vector< Belle2::CalibrationAlgorithm_OLD::ExpRun > runs,
    int iteration)
{
...
vector< CalibrationAlgorithm_OLD::ExpRun > CalibrationAlgorithm_OLD::string2RunList(string list) const
{
...
string CalibrationAlgorithm_OLD::runList2String(vector< CalibrationAlgorithm_OLD::ExpRun >& list) const
{
  std::string str("");
  for (auto run : list) {
    if (str != "")
...
string CalibrationAlgorithm_OLD::runList2String(CalibrationAlgorithm_OLD::ExpRun run) const
{
...
IntervalOfValidity CalibrationAlgorithm_OLD::getIovFromData()
{
  auto& range = getObject<RunRange>(RUN_RANGE_OBJ_NAME);
  return range.getIntervalOfValidity();
}

void CalibrationAlgorithm_OLD::saveCalibration(TObject* data, const string& name, const IntervalOfValidity& iov)
{
  m_payloads.emplace_back(name, data, iov);
}

void CalibrationAlgorithm_OLD::saveCalibration(TClonesArray* data, const string& name, const IntervalOfValidity& iov)
{
  m_payloads.emplace_back(name, data, iov);
}

void CalibrationAlgorithm_OLD::saveCalibration(TObject* data, const string& name)
{
  if (m_runs.empty())
    return;
...
void CalibrationAlgorithm_OLD::saveCalibration(TClonesArray* data, const string& name)
{
  if (m_runs.empty())
    return;
...
bool CalibrationAlgorithm_OLD::commit()
{
  if (m_payloads.empty())
    return false;
...
bool CalibrationAlgorithm_OLD::commit(std::list<Database::DBQuery> payloads)
{
  if (payloads.empty())
    return false;
  return Database::Instance().storeData(payloads);
...
vector< CalibrationAlgorithm_OLD::ExpRun > CalibrationAlgorithm_OLD::getRunListFromAllData()
{
