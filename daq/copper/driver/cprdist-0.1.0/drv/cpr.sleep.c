...
static char *build_id = "egawa Wed Feb 15 13:56:01 JST 2012";
...
//static unsigned int debug_flag = DEBUG_YAMADA;
...
static void config_event_fifo_depth(void)
{
    ...
    writel(1, COPPERREG(LEF_WA_FF));
    writel(1, COPPERREG(LEF_WB_FF));
    writel(1, COPPERREG(LEF_WC_FF));
    writel(1, COPPERREG(LEF_WD_FF));

    writel(511, COPPERREG(LEF_WA_AF));
    writel(511, COPPERREG(LEF_WB_AF));
    writel(511, COPPERREG(LEF_WC_AF));
    writel(511, COPPERREG(LEF_WD_AF));
}
...
actual_interrupt_handler(struct work_struct * w)
{
	...
	if (plx9054.lost_dma_intr < 10 &&
	    ...
	    printk("<1>maybe lost DMA done interrupt (dmac=%d)\n",
		   run_state.dma_count);
	...
	if (plx9054.lost_dma_intr) {
	    printk("<1>found lost DMA done interrupt (dmac=%d)\n",
		   run_state.dma_count);
	    plx9054.lost_dma_intr = 0;
	}
	...
	if (plx9054.dma_active) {
	    printk("<1>"
		   "found local input active(%x) but DMA is active\n",
		   status.val);
	}
			 ...
			 unsigned long input_length, void *data)
{
    ...
    writel(val, COPPERREG(FINESSE_STA));
...
static int start_dma()
{
	...
	printk("<1>start_dma is called, but DMA is active\n");

	dmacsr.val = readl(PLX9054REG(DMACSR0));
	if (dmacsr.bit.dma0_done) {
	    printk("<1>DMACSR0 shows DMA was done\n");
	}
	...
	return -1;
      ...
      if( size > 10000 || sleep_cnt > 10 ) break;
      if( debug_flag & DEBUG_YAMADA ){
	getnstimeofday( &tv);  
	printk("<1>" "sleep Start %d %d %d\n", 
	       tv.tv_sec, tv.tv_nsec, sleep_cnt);
      }
	msleep_interruptible(1);
      if( debug_flag & DEBUG_YAMADA ){
	getnstimeofday( &tv);  
	printk("<1>" "sleep Stopp %d %d %d\n", 
	       tv.tv_sec, tv.tv_nsec, sleep_cnt);
      }
      sleep_cnt++;
}
...
static void allocate_buffer(void)
{
    ...
    for (i = 0; i < NDMABUF; i++) {
	p = (unsigned char
	     *) (__get_free_pages(GFP_KERNEL | GFP_DMA, DMA_PAGES));
	plx9054.dmabuf[i].in_use = 0;
	plx9054.dmabuf[i].wpos = 0;
