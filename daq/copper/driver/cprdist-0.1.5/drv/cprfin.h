...
#define COPPER_PLX9054_BUS        1
#define COPPER_PLX9054_DEVFUN  0x40
...
cprfin_find_plx9054(void)
{
  ...
  pdev = pci_find_slot(COPPER_PLX9054_BUS, COPPER_PLX9054_DEVFUN);
  if (!pdev) {
    printk("cprfin: cannot find slot: bus=%04x devfun=%04x\n", COPPER_PLX9054_BUS, COPPER_PLX9054_DEVFUN);
    return NULL;
  }

  if (pdev->vendor != PCI_VENDOR_ID_PLX || pdev->device != PCI_DEVICE_ID_9054) {
    printk("cprfin: cannot find device: vendor=%04x device=%04x\n", PCI_VENDOR_ID_PLX, PCI_DEVICE_ID_9054);
    return NULL;
  }
...
cprfin_finesse_mmap(struct pci_dev* pdev)
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    map_addr = (unsigned long)ioremap(prg_addr & PCI_BASE_ADDRESS_MEM_MASK, size);
...
#define FNIOGETGENSLOT     _IOR(CPRFIN_MAGIC, 0x01, sizeof(unsigned int))

#define FNIOGETGENCSR      _IOR(CPRFIN_MAGIC, 0xf1, sizeof(unsigned int))
#define FNIOGETGENFIFOEMU  _IOR(CPRFIN_MAGIC, 0xf2, sizeof(unsigned int))
#define FNIOGETGENFPGAVER  _IOR(CPRFIN_MAGIC, 0xf3, sizeof(unsigned int))
#define FNIOGETGENSERIAL   _IOR(CPRFIN_MAGIC, 0xf4, sizeof(unsigned int))
#define FNIOGETGENTYPE     _IOR(CPRFIN_MAGIC, 0xf5, sizeof(unsigned int))
#define FNIOGETGENRESERVED _IOR(CPRFIN_MAGIC, 0xf6, sizeof(unsigned int))
