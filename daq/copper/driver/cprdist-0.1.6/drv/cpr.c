...
#define DECL_COPPER_WRITER(n) \
static int \
write_copper_##n(struct file *pfile, const char *buf, \
			 unsigned long input_length, void *data) \
{ return write_copper_register( ((volatile unsigned int *)(copper.reg + COPPER_##n)), \
		pfile, buf, input_length, data); }

#define ADD_COPPER_WRITER(n) \
if (pentry_file) pentry_file->write_proc = write_copper_##n

#define ADD_COPPER_READER_WRITER(n) \
pentry_file = create_proc_entry(#n, 0, copper.pentry);  \
if (pentry_file) { \
	pentry_file->read_proc = read_copper_##n; \
	pentry_file->write_proc = write_copper_##n; \
}

DECL_COPPER_WRITER(FINESSE_STA)

DECL_COPPER_WRITER(LEF_WA_FF)
DECL_COPPER_WRITER(LEF_WB_FF)
DECL_COPPER_WRITER(LEF_WC_FF)
DECL_COPPER_WRITER(LEF_WD_FF)

DECL_COPPER_WRITER(LEF_WA_AF)
DECL_COPPER_WRITER(LEF_WB_AF)
DECL_COPPER_WRITER(LEF_WC_AF)
DECL_COPPER_WRITER(LEF_WD_AF)

DECL_COPPER_WRITER(CONF_WA_FF)
DECL_COPPER_WRITER(CONF_WB_FF)
DECL_COPPER_WRITER(CONF_WC_FF)
DECL_COPPER_WRITER(CONF_WD_FF)

DECL_COPPER_WRITER(CONF_WA_AF)
DECL_COPPER_WRITER(CONF_WB_AF)
DECL_COPPER_WRITER(CONF_WC_AF)
DECL_COPPER_WRITER(CONF_WD_AF)
...
static void create_copper_proc(void)
{
    ...
    ADD_COPPER_READER_WRITER(CONF_WA_AF);
    ADD_COPPER_READER_WRITER(CONF_WB_AF);
    ADD_COPPER_READER_WRITER(CONF_WC_AF);
    ADD_COPPER_READER_WRITER(CONF_WD_AF);

    ADD_COPPER_READER_WRITER(CONF_WA_FF);
    ADD_COPPER_READER_WRITER(CONF_WB_FF);
    ADD_COPPER_READER_WRITER(CONF_WC_FF);
    ADD_COPPER_READER_WRITER(CONF_WD_FF);

    ADD_COPPER_READER_WRITER(LEF_WA_AF);
    ADD_COPPER_READER_WRITER(LEF_WB_AF);
    ADD_COPPER_READER_WRITER(LEF_WC_AF);
    ADD_COPPER_READER_WRITER(LEF_WD_AF);

    ADD_COPPER_READER_WRITER(LEF_WA_FF);
    ADD_COPPER_READER_WRITER(LEF_WB_FF);
    ADD_COPPER_READER_WRITER(LEF_WC_FF);
    ADD_COPPER_READER_WRITER(LEF_WD_FF);
...
static int find_chips(void)
{
	...
	if (pdev == NULL) {
	    ...
	    if (bus_no == 1) {
		printk("<1>Processor Module is treadted as EPC6315\n");
		is_EPC6315 = 1;
	    }
	    found = 1;
	    break;
	...
	int result = request_irq(pdev->irq, plx9054_intr,
				(is_EPC6315 ? (IRQF_DISABLED |IRQF_SHARED) : IRQF_SHARED) ,
				  "plx9054", &plx9054);
	if (result < 0) {
