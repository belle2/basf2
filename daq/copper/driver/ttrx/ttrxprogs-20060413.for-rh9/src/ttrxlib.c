...
get_ttrx(unsigned int *base_plx_p, unsigned int *base_rx_p, int rxid)
{
  ...
  if (! fp_pci) {
    perror("cannot fopen /proc/pci");
    exit(1);
  }

  memset(buf, 0, sizeof(buf));
  while (fgets(buf, 255, fp_pci)) {
    if (strncmp(buf, "  Bus", 5) == 0) {
      sscanf(buf, "%*s %d%*s %*s %d%*s %*s %d", &bus, &dev, &func);
      plx = 0;
    } else {
      continue;
    }

    sprintf(buf, "/proc/bus/pci/%02x/%02x.%1x", bus, dev, func);

    if (! (fp_bus = fopen(buf, "r"))) {
      fprintf(stderr, "cannot fopen %s", buf);
      perror("");
      exit(1);
    }
