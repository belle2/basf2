...
{   
    ...
    int istart;
    int card_test[4];// by zjz and lza Dec 10 2011
...
/*        printf("                         after redo.\n"); */
	...
	do {
        ...
        for( iii=0;iii<4;iii++) card_test[iii]=0; // by zjz and lza
	...
	struct copper_footer * footer =
	 ...
	 int t;
        ...
        if(buffer[ 9]) card_test[0] = 1;
        if(buffer[10]) card_test[1] = 1;
        if(buffer[11]) card_test[2] = 1;
        if(buffer[12]) card_test[3] = 1; // by zjz and lza
        
        istart = 13;
        for (iii=0;iii<4;iii+) {
         if (iii==0 & card_test(iii) == 0) istart = istart + buffer[9 + iii-1] ;
         
         if(card_test[0] & iii==0) fprintf(fpw,"Board A: %08x /*** Data length=%08x, Trigger#=%08x ***/\n", buffer[istart + 1], buffer[istart + 2], buffer[istart + 3]);
         if(card_test[1] & iii==1) fprintf(fpw,"Board B: %08x /*** Data length=%08x, Trigger#=%08x ***/\n", buffer[istart + 1], buffer[istart + 2], buffer[istart + 3]);
         if(card_test[2] & iii==2) fprintf(fpw,"Board C: %08x /*** Data length=%08x, Trigger#=%08x ***/\n", buffer[istart + 1], buffer[istart + 2], buffer[istart + 3]);
         if(card_test[3] & iii==3) fprintf(fpw,"Board D: %08x /*** Data length=%08x, Trigger#=%08x ***/\n", buffer[istart + 1], buffer[istart + 2], buffer[istart + 3]);

         for (t= istart + 4; t<(ret/4 - sizeof(struct copper_footer)/4 - 2); t= t + 1)
          {
