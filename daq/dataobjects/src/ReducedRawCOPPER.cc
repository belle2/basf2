...
ClassImp(ReducedRawCOPPER);

ReducedRawCOPPER::ReducedRawCOPPER()
{
}
...
// ReducedRawCOPPER::ReducedRawCOPPER( PreRawCOPPER* raw_cpr ){
//    int nwords_buf_to = calcReducedNwords( raw_cpr );
//    int* buf_to = new int[ nwords_buf_to ];
//    int *buf_from = NULL;
//    int cur_pos_to = 0;
//    int copy_nwords = 0;

//    for( int i = 0; i < raw_cpr->GetNumEntries(); i++ ){
//      //Header copy
//      buf_from = raw_cpr->GetBuffer( i );
//      copy_nwords = raw_cpr->tmp_header.GetNwords();
//      copyData( &cur_pos_to, copy_nwords, buf_from, buf_to, nwords_buf_to );

//      // copy FINESSE buffer
//      for( int j = 0; j < 4; j++ ){
//        // copy body
//        buf_from =
//   raw_cpr->GetFINESSEBuffer( i, j )
//   + raw_cpr->SIZE_B2LHSLB_HEADER
//   + raw_cpr->POS_B2L_CTIME;
//        copy_nwords =
//   raw_cpr->GetFINESSENwords( i, j )
//   - raw_cpr->SIZE_B2LHSLB_HEADER
//   - raw_cpr->POS_B2L_CTIME
//   - raw_cpr->SIZE_B2LFEE_TRAILER
//   - raw_cpr->SIZE_B2LHSLB_TRAILER;
//        copyData( &cur_pos_to, copy_nwords, buf_from, buf_to, nwords_buf_to );
//      }

//      // copy RawCOPPER trailer
//      buf_from = raw_cpr->GetBuffer( i )
//        + raw_cpr->GetBlockNwords( i )
//        - raw_cpr->tmp_trailer.GetTrlNwords();
//      copy_nwords = raw_cpr->tmp_trailer.GetTrlNwords();
//      copyData( &cur_pos_to, copy_nwords, buf_from, buf_to, nwords_buf_to );
//    }

//    // length check
//    if( cur_pos_to != nwords_buf_to ){
//      perror("Buffer overflow. Exiting...");
//      exit(1);
//    }

//    // copy to m_buffer
//    if( m_buffer != NULL && m_use_prealloc_buf != 0 ){
//      delete[] m_buffer;
//    }
//    m_buffer = buf_to;
...
// void ReducedRawCOPPER::copyData( int* cur_pos_to, const int copy_nwords, const int* buf_from,
//          int* buf_to, const int nwords_buf_to ){
//   if( *cur_pos_to + copy_nwords > nwords_buf_to ){
//     perror("Buffer overflow. Exiting...");
//     exit(1);
//   }
//   memcpy( buf_to + *cur_pos_to, buf_from, sizeof( copy_nwords ) );
//   *cur_pos_to += copy_nwords;
//   return;
// }


int ReducedRawCOPPER::GetBufferPos(int n)
{
...
unsigned int ReducedRawCOPPER::CalcDriverChkSum(int n)
{
...
int ReducedRawCOPPER::GetNumFINESSEBlock(int n)
{
  int cnt = 0;
  for (int i = 0; i < 4; i++) {
...
int ReducedRawCOPPER::GetDetectorNwords(int n, int finesse_num)
{
  int nwords = 0;
...
int ReducedRawCOPPER::GetFINESSENwords(int n, int finesse_num)
{
...
int ReducedRawCOPPER::GetOffsetFINESSE(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int* ReducedRawCOPPER::GetFINESSEBuffer(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int* ReducedRawCOPPER::GetDetectorBuffer(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
unsigned int ReducedRawCOPPER::GetB2LFEE32bitEventNumber(int n)
{
  exit(1); return 0;
...
unsigned int  ReducedRawCOPPER::CalcXORChecksum(int* buf, int nwords)
{
...
void ReducedRawCOPPER::CheckData(int n,
                                 ...
                                 int prev_runsubrun_no, int* cur_runsubrun_no)
{
  ...
  *cur_evenum_rawcprhdr = GetEveNo(n);
  ...
  if (*cur_evenum_rawcprhdr != evenum_feehdr) {
    sprintf(err_buf, "CORRUPTED DATA: Event # in ReducedRawCOPPER header and FEE header is different : cprhdr 0x%x feehdr 0x%x : Exiting...\n%s %s %d\n",
  ...
  *cur_copper_ctr = GetCOPPERCounter(n);
  ...
  if (rawtrl.GetChksum() !=
    ...
    sprintf(err_buf, "CORRUPTED DATA: ReducedRawCOPPER checksum error : block %d : length %d eve 0x%x : Trailer chksum 0x%.8x : calcd. now 0x%.8x\n %s %s %d\n",
...
bool ReducedRawCOPPER::CheckCOPPERMagic(int n)
{
  exit(1);
...
void ReducedRawCOPPER::CheckUtimeCtimeTRGType(int n)
{
  exit(1); return;
//   // Check if every FINESSE has the same value of utime and ctime
...
double ReducedRawCOPPER::GetEventUnixTime(int n)
{
...
unsigned int ReducedRawCOPPER::GetB2LHeaderWord(int n, int finesse_buffer_pos)
{
  unsigned int word[4];
...
// unsigned int ReducedRawCOPPER::FillTopBlockRawHeader(unsigned int m_node_id, unsigned int m_data_type,
...
//     char err_buf[500];
//     sprintf(err_buf,
//             "This function should be used for ReducedRawCOPPER containing only one datablock, while. this object has num_nodes of %d and num_events of %d\n %s %s %d\n",
...
//   if (copper_buf[ CPR_POS_CH_A_DATA_LENGTH ] == 0 &&
//       copper_buf[ CPR_POS_CH_B_DATA_LENGTH ] == 0 &&
//       copper_buf[ CPR_POS_CH_C_DATA_LENGTH ] == 0 &&
//       copper_buf[ CPR_POS_CH_D_DATA_LENGTH ] == 0) {
//     char err_buf[500];
//     sprintf(err_buf,
...
//     (copper_buf[ CPR_POS_DATA_LENGTH ]
//      + SIZE_COPPER_DRIVER_HEADER
//      + SIZE_COPPER_DRIVER_TRAILER)
...
//   int offset_2nd_finesse = offset_1st_finesse + copper_buf[ CPR_POS_CH_A_DATA_LENGTH ];
//   int offset_3rd_finesse = offset_2nd_finesse + copper_buf[ CPR_POS_CH_B_DATA_LENGTH ];
//   int offset_4th_finesse = offset_3rd_finesse + copper_buf[ CPR_POS_CH_C_DATA_LENGTH ];
...
//   m_buffer[ ReducedRawHeader::POS_EXP_RUN_NO ] =
//     finesse_buf[ SIZE_B2LHSLB_HEADER + B2LFEE_POS_EXP_RUN ];


//   //
//   // Fill event #
//   //
//   unsigned int cur_ftsw_eve32 =  finesse_buf[ SIZE_B2LHSLB_HEADER + B2LFEE_POS_TT_TAG ];
//   m_buffer[ ReducedRawHeader::POS_EVE_NO ] = cur_ftsw_eve32;

//   //
//   // Copy FTSW words from B2LFEE header
//   //
//   m_buffer[ ReducedRawHeader::POS_TTCTIME_TRGTYPE ] =
//     finesse_buf[ SIZE_B2LHSLB_HEADER + B2LFEE_POS_TT_CTIME_TYPE ];
//   m_buffer[ ReducedRawHeader::POS_TTUTIME ] =
//     finesse_buf[ SIZE_B2LHSLB_HEADER + B2LFEE_POS_TT_UTIME ];
...
//   int err_flag = 0;
//   if ((unsigned int)(copper_buf[ CPR_POS_MAGIC_COPPER_1 ]) != CPR_COPPER_MAGIC_DRIVER_HEADER) {
//     err_flag = 1;
//   } else if ((unsigned int)(copper_buf[ CPR_POS_MAGIC_COPPER_2 ]) != CPR_COPPER_MAGIC_FPGA_HEADER) {
//     err_flag = 1;
//   } else if ((unsigned int)(*fpga_trailer_magic) != CPR_COPPER_MAGIC_FPGA_TRAILER) {
//     err_flag = 1;
//   } else if ((unsigned int)(*driver_trailer_magic) != CPR_COPPER_MAGIC_DRIVER_TRAILER) {
//     err_flag = 1;
//   }
//   if (err_flag == 1) {
//     char err_buf[500];
...
// #ifdef USE_B2LFEE_FORMAT_BOTH_VER1_AND_2
// void ReducedRawCOPPER::CheckB2LFEEHeaderVersion(int n)
// {
//   int flag = 0;
//   int* temp_buf;
...
// #ifdef TEMP
//       char err_buf[500];
//       sprintf(err_buf, "ReducedRawCOPPER contains no FINESSE data. Exiting...\n %s %s %d\n",
//               __FILE__, __PRETTY_FUNCTION__, __LINE__);
//       string err_str = err_buf; throw (err_str);
//       sleep(12345678);
//       exit(-1);
