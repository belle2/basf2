...
#include "daq/slc/apps/eb1txd/eb1rx_status.h"
...
void EB1TXCallback::initialize(const DBObject& obj) throw(RCHandlerException)
{
  ...
  //add(new NSMVHandlerFloat(vname + "evtrate", true, false, 0));
  add(new NSMVHandlerFloat(vname + "flowrate", true, false, 0));
  std::string upname = std::string("/dev/shm/") + getNode().getName() + "_eb1tx_up";
  std::string downname = std::string("/dev/shm/") + getNode().getName() + "_eb1tx_down";
...
/*
   ...
   obj         : Contains parameters in configuration from the database.
*/
void EB1TXCallback::configure(const DBObject& /*obj*/) throw(RCHandlerException)
{
}
...
void EB1TXCallback::load(const DBObject& obj) throw(RCHandlerException)
{
  ...
  try {
    ...
    int txport = obj.getInt("txport");
    std::string host = obj.getText("host");
...
*/
void EB1TXCallback::start(int expno, int runno) throw(RCHandlerException)
{
...
*/
void EB1TXCallback::stop() throw(RCHandlerException)
{
...
*/
bool EB1TXCallback::resume(int subno) throw(RCHandlerException)
{
...
/*
   ...
   returns true on success or false on failed
*/
bool EB1TXCallback::pause() throw(RCHandlerException)
{
...
*/
void EB1TXCallback::recover(const DBObject& obj) throw(RCHandlerException)
{
...
*/
void EB1TXCallback::abort() throw(RCHandlerException)
{
  m_con.abort();
...
*/
void EB1TXCallback::monitor() throw(RCHandlerException)
{
  double t1 = Time().get();
  double dt = t1 - m_t0;
