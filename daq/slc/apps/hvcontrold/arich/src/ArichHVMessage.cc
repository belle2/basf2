...
std::string ArichHVMessage::toString()
{
  ...
  if (m_type == GET) {
    ...
    switch (m_command) {
      case ALL: ss << Belle2::form("SET %04X,%d,%04X,%04X,%04X", getVoltageDemand(),
      ...
      case RAMPUP_SPEED:   ss << Belle2::form("RVU %04X", getRampUpSpeed()); break;
      case RAMPDOWN_SPEED: ss << Belle2::form("RVD %04X", getRampDownSpeed()); break;
      case VOLTAGE_DEMAND: ss << Belle2::form("CH5 %04X", getVoltageDemand()); break;
      case VOLTAGE_LIMIT:  ss << Belle2::form("CH2 %04X", getVoltageLimit()); break;
      case CURRENT_LIMIT:  ss << Belle2::form("CH7 %04X", getCurrentLimit()); break;
      case STORE:          ss << Belle2::form("STORE %d", getStoreId()); break;
      case RECALL:         ss << Belle2::form("RECALL %d", getStoreId()); break;
      default: break;
    }
  }
  return ss.str();
