...
int COPPER::read(int request) throw(IOException)
{
  int ret;
  int val;
...
copper_info& COPPER::monitor() throw(IOException)
{
  if (m_fd < 0) {
    throw (IOException("Copper file descriptor not ready"));
  }
  ...
  try {
    ...
    */
    ...
    ::close(m_fd);
    m_fd = -1;
    throw (IOException("IO error : %s", e.what()));
  }
  return m_info;
}

void COPPER::print() throw()
{
  printf("%x\n", m_info.ff_sta);
  printf("%x\n", m_info.conf_w_ae[0]);
  printf("%x\n", m_info.conf_w_ae[1]);
...
bool COPPER::isFifoEmpty() throw()
{
  if (m_fd <= 0) return false;
  return (m_info.ff_sta & 0xff) == 0x23;
}

bool COPPER::isLengthFifoFull() throw()
{
  if (m_fd <= 0) return false;
