...
{
  try {
    if (expno <= 0) expno = getExpNumber(config);
    if (expno < 0) {
      return RunNumber();
    }
    if (runno <= 0) runno = getRunNumber(expno, config);
    if (runno < 0) {
      return RunNumber();
    }
  ...
  try {
    if (config.size() == 0) {
      m_db.execute("select max(cast(split_part(label,'.',4) as int))+1 "
                   ...
                   "where label like '_%%.s.%03d._%%' limit 1;", expno);
    } else {
      m_db.execute("select max(cast(split_part(label,'.',4) as int))+1 "
                   "as runno from runnumber "
  ...
  try {
    if (config.size() == 0) {
      m_db.execute("select max(cast(split_part(label,'.',3) as int))"
                   ...
                   "where label like '_%%.s._%%._%%' limit 1;");
    } else {
      m_db.execute("select max(cast(split_part(label,'.',3) as int))"
                   " as expno from runnumber "
