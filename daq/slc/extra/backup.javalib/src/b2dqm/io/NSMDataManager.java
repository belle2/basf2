				...
				if (_histo != null) {
					...
					if (_shape instanceof GText) {
						...
						if (_info_v.size() > 0) {
							...
							int i = 0;
							switch (data.getParam(info._paraname).type) {
							case NSMData.CHAR:  i = data.getChar(info._paraname, info.index); break;
							case NSMData.INT16: i = data.getShort(info._paraname, info.index); break;
							case NSMData.INT32: i = data.getInt(info._paraname, info.index); break;
							case NSMData.INT64: i = (int)data.getLong(info._paraname, info.index); break;
							case NSMData.BYTE8: i = data.getChar(info._paraname, info.index); break;
							case NSMData.UINT16:i = data.getShort(info._paraname, info.index); break;
							case NSMData.UINT32:i = data.getInt(info._paraname, info.index); break;
							case NSMData.UINT64:i = (int)data.getLong(info._paraname, info.index); break;
							case NSMData.FLOAT: i = (int)data.getFloat(info._paraname, info.index); break;
							case NSMData.DOUBLE:i = (int)data.getDouble(info._paraname, info.index); break;
							}
