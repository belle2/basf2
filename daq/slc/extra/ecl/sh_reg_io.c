...
int sh_reg_io(const char* ip_addr, const char* opt, 
	      int sh_num, int reg_num, int reg_wdata, 
	      int* sh_reg_data, char* msg)
{
  ...
  if((sh_mask & reg_data) != 0) {
    sprintf(msg, "WARNING: deser buffer is not empty: DL_READ_PEND=%04X, shaper mask=%04X Flushing...\n", reg_data, sh_mask);
  ...
  if(strcmp(opt, "r") == 0) {
	...
	if((read_ok_mask_i & 1) != 0) {
	  sprintf(msg, "ShaperDSP#%d[%04X] => %04X\n", sh_num_i+1, reg_num, sh_reg_data[sh_num_i]);
	} else {
	  sprintf(msg, "ShaperDSP#%d[%04X] ...\n", sh_num_i+1, reg_num);
	}
      }
    }
