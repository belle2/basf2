...
static Belle2::PXDHVControlCallback* g_callback = NULL;

using namespace Belle2;

void init_pxdhv(const char* confname)
{
  ...
  g_callback = new PXDHVControlCallback(NSMNode(nodename), rcname);
...
void eventCallback(struct event_handler_args eha)
{
  ...
  if(eha.status != ECA_NORMAL) {
	...
	if (val == "off") {
	  ...
	  LogFile::info(g_callback->getRCState().getLabel());
