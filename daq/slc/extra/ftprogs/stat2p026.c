...
\* ---------------------------------------------------------------------- */
void
regs2p026(ftsw_t *ftsw, struct timeval *tvp, fast_t *f, slow_t *s)
{
  int i;
...
\* ---------------------------------------------------------------------- */
void
summary2p026(struct timeval *tvp, fast_t *f, slow_t *s)
{
  int clkfreq;
  ...
  sprintf(g_ftstat.err, "%s%s%s",
	  ...
	  B(f->oerrin,30) ? " errin" : "",
	  B(f->oerrin,29) ? " fifo" : "");
  g_ftstat.errport[0] = 0;
  g_ftstat.errportsrc = D(f->errsrc,4,0);
  char ss[100];
  if (D(f->errsrc,4,0)) {
    sprintf(ss, "%s%s%s%s%s",
	    Bs(f->errsrc,4," errin"),
	    Bs(f->errsrc,3," b2ldn"),
	    Bs(f->errsrc,2," clkerr"),
	    Bs(f->errsrc,1," tshort"),
  ...
  sprintf(g_ftstat.errport, "%s%s%s%s%s%s%s%s%s%s%s%s",
	  ss,
...
#if !defined(TTMASTER) && !defined(POCKET_TTD)
void
stat2p026(ftsw_t *ftsw, int ftswid, int showcolor)
{
  struct timeval tv;
  ft2p_t p;
