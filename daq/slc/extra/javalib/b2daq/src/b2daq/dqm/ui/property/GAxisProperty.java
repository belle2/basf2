...
public class GAxisProperty extends GShapeProperty {
    ...
    public GAxisProperty(HistogramCanvas canvas) {
        super(canvas);
    }
    ...
    public void put(String key, String value) {
        ...
        super.put(key, value);
        if (key.matches("log-scale")) {
            _axis.setLogScale(value.matches("on"));
        }
        if (key.matches("time-format")) {
        ...
        if (key.matches("label-font-size")) {
            _axis.setLabelFontSize(Double.parseDouble(value));
        }
        if (key.matches("label-font-color")) {
            _axis.getLabelsFont().setColor(Color.web(value));
        }
        ...
        if (key.matches("label")) {
            ...
            ObservableList<GValuedText> glabel_v = FXCollections.observableArrayList();
        ...
        if (key.matches("tick-color")) {
            _axis.setLineColor(Color.web(value));
        }
