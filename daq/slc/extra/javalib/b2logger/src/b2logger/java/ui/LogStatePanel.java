...
public class LogStatePanel extends JavaGCanvasPanel {
	...
	public void update(LogMessage message) {
		SystemLogLevel level = message.getLogLevel();
		if ( level.hiegher(SystemLogLevel.ERROR) ) {
			...
			_g_state_text.setFontColor(HtmlColor.WHITE);
			_g_state_rect.setFillColor(HtmlColor.RUNNING_GREEN);
			_g_state_rect.setLineColor(HtmlColor.GREEN);
			_is_stable = true;
		}
		_g_update_time.setText(message.getTime().toDateString());
		repaint();
	}
