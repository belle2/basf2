...
import org.belle2.daq.dqm.graphics.GHisto2;
...
public abstract class Histo2 extends Histo {
	...
	public double getMeanX() {
		...
		for (int nx = 0; nx < axisX.getNbins(); nx++) {
			...
			mean += bin * ((nx + 0.5) * (axisX.getMax() - axisX.getMin()) / axisX.getNbins() + axisX.getMin());
			entries += bin;
		}
		return mean / entries;
	}
	...
	public double getMeanY() {
		...
		for (int ny = 0; ny < axisY.getNbins(); ny++) {
			...
			mean += bin * ((ny + 0.5) * (axisY.getMax() - axisY.getMin()) / axisY.getNbins() + axisY.getMin());
			entries += bin;
		}
		return mean / entries;
	}
	...
	protected String getXML() {
		String str = "title=\"" + getTitle() + "\" data=\"" + data.toString() + "\" >\n";
	...
	public void add(Histo h, double scale) {
		if (h.getAxisY().getNbins() == getAxisY().getNbins() && h.getAxisY().getMin() == getAxisY().getMin()
				&& h.getAxisY().getMax() == getAxisY().getMax() && h.getAxisX().getNbins() == getAxisX().getNbins()
				&& h.getAxisX().getMin() == getAxisX().getMin() && h.getAxisX().getMax() == getAxisX().getMax()) {
	...
	public void setNbinsxy(int nbinsx, int nbinsy) {
		axisX.setNbins(nbinsx);
		axisY.setNbins(nbinsy);
		data.resize((nbinsx + 2) * (nbinsy + 2));
	}
