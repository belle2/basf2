...
import org.belle2.daq.dqm.Histo1;
...
public final class GHisto1 extends GHisto {
	...
	public void update() {
		...
		double dbinx = (histo.getAxisX().getMax() - histo.getAxisX().getMin()) / histo.getAxisX().getNbins();
		...
		int nin = 0;
		int n_digit = 1;
		if (histo.getDataType().matches(".*I") || histo.getDataType().matches(".*C") || histo.getDataType().matches(".*S")) {
			n_digit = 0;
		}
		...
		for (int n = 1; n <= histo.getAxisX().getNbins(); n++) {
			if (x < pointX[2 * n]) {
				if (y >= pointY[2 * n] && y < canvas.getPad().getY() + canvas.getPad().getHeight()) {
					return "histo = " + getObject().getName() + "\n" + "bin = " + n + "\n" + "value = "
							+ NumberString.toString(((Histo) getObject()).getBinContent(n - 1), 2) + "";
				}
			}
		}
		return "";
