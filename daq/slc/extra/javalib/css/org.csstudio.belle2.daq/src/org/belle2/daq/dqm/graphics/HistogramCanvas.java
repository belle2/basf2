...
import org.belle2.daq.dqm.TimedGraph1;
...
public class HistogramCanvas extends GRect {
	...
	private final ArrayList<Histo> histograms = new ArrayList<>();
	private final ArrayList<GShape> shapes = new ArrayList<>();
	...
	public HistogramCanvas(String name, String title, double width, double height, boolean use_stat) {
		...
		setName(name);
		setFont(new FontProperty(HtmlColor.BLACK, "Arial", 1.0, FontProperty.WEIGHT_BOLD));
		setTitle(title);
	...
	public HistogramCanvas(ArrayList<Histo> histos) {
		this("", "", 100, 100);
	...
	public final void resetPadding() {
		...
		axisx.set(pad.getX(), pad.getY() + pad.getHeight(), pad.getWidth(), 0, 1 - (pad.getY() + pad.getHeight()));
		axisy.set(pad.getX(), pad.getY() + pad.getHeight(), pad.getHeight(), pad.getX(), 0);
		if (colorAxis != null) {
			colorAxis.set(pad.getX() + pad.getWidth() + (1 - pad.getX() - pad.getWidth()) * 0.28,
					pad.getY() + pad.getHeight(), pad.getHeight(), (1 - pad.getX() - pad.getWidth()) * 0.25, 0);
		}
		if (axisy2 != null) {
			axisy2.set(pad.getX() + pad.getWidth(), pad.getY() + pad.getHeight(), pad.getHeight(),
					1 - pad.getX() - pad.getWidth(), 0);
		}
	}
	...
	public void setHistograms(ArrayList<Histo> histograms) {
	...
	public void setShapes(ArrayList<GShape> shapes) {
		this.shapes.clear();
	...
	public void addHisto(Histo h) {
		...
		if (!_use_pad) {
			...
			try {
				...
				if (xmax < 10 && xmax > 5 && xmin > -10 && xmin < -5) {
	...
	public void update() {
		...
		if (stat && _stat_histo != null) {
			if (_stat_rect == null) {
				...
				_stat_line = new GLine(0.71, 0.06, 0.99, 0.06, HtmlColor.BLACK);
