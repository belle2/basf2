...
public class ConfigObject extends DBObject {
	...
	private HashMap<String, String> m_text_m = new HashMap<String, String>();
	private HashMap<String, ArrayList<ConfigObject>> m_obj_v_m = 
		new HashMap<String, ArrayList<ConfigObject>>();
	
	public void reset() {
		super.reset();
		m_value_m = new HashMap<String, Object>();
		m_text_m = new HashMap<String, String>();
		m_obj_v_m = new HashMap<String, ArrayList<ConfigObject>>();
	}
	...
	public void readObject(Reader reader) throws IOException {
		...
		for (int i = 0; i < npar; i++) {
			...
			switch (type) {
			case FieldInfo.BOOL:   addBool(name, reader.readBoolean()); break;
			...
			case FieldInfo.OBJECT: {
				ArrayList<ConfigObject> obj_v = new ArrayList<ConfigObject>();
	...
	public void writeObject(Writer writer) throws IOException {
		...
		for (int ii = 0; ii < name_v.size(); ii++) {
			String name = name_v.get(ii);
			int type = getProperty(name).getType();
			writer.writeString(name);
			writer.writeInt(type);
			switch (type) {
			case FieldInfo.BOOL:   writer.writeBoolean(getBool(name)); break;
	...
	@Override
	public DBObject getObject(String name, int i) { return m_obj_v_m.get(name).get(i);	}

	@Override
	public int getNObjects(String name) { return m_obj_v_m.get(name).size(); }
	...
	public ArrayList<ConfigObject> getObjects(String name) { return m_obj_v_m.get(name); }
	public void addObject(String name, ConfigObject obj) {
		if (!hasField(name)) {
			add(name, new FieldInfo.Property(FieldInfo.OBJECT, 0));
			m_obj_v_m.put(name, new ArrayList<ConfigObject>());
		}
