...
public class NSMData extends DBObject {
    ...
    public String getFormat() { return getTable(); }
    public void setFormat(String format) { setTable(format); }
		...
		for (int n = 0; n < npars; n++) {
			...
			if (type == FieldInfo.NSM_OBJECT) {
				...
				for (int i = 0; i < length; i++) {
					...
					case FieldInfo.NSM_UINT64: addValue(name, type, reader.readLong()); break;
					case FieldInfo.NSM_INT32: 
					case FieldInfo.NSM_UINT32: addValue(name, type, reader.readInt()); break;
					case FieldInfo.NSM_INT16:
					case FieldInfo.NSM_UINT16: addValue(name, type, reader.readShort()); break;
					case FieldInfo.NSM_CHAR: 
					case FieldInfo.NSM_BYTE8: addValue(name, type, reader.readChar()); break;
					case FieldInfo.NSM_FLOAT: addValue(name, type, reader.readFloat()); break;
					case FieldInfo.NSM_DOUBLE: addValue(name, type, reader.readDouble()); break;
					default: break;
					}
				}
			}
	...
	public void writeObject(Writer writer) throws IOException {
		...
		for (String name : name_v) {
			...
			if (pro.getType() == FieldInfo.NSM_OBJECT) {
				...
				for (int i = 0; i < length; i++) {
					switch (pro.getType()) {
					case FieldInfo.NSM_CHAR:   writer.writeChar((Character)buf.get(i)); break;
					case FieldInfo.NSM_INT16:  writer.writeShort((Short)buf.get(i)); break;
					case FieldInfo.NSM_INT64:  writer.writeLong((Long)buf.get(i)); break;
					case FieldInfo.NSM_INT32:  writer.writeInt((Integer)buf.get(i)); break;
					case FieldInfo.NSM_BYTE8:  writer.writeChar((Character)buf.get(i)); break;
					case FieldInfo.NSM_UINT16: writer.writeShort((Short)buf.get(i)); break;
					case FieldInfo.NSM_UINT32: writer.writeInt((Integer)buf.get(i)); break;
					case FieldInfo.NSM_UINT64: writer.writeLong((Long)buf.get(i)); break;
					case FieldInfo.NSM_FLOAT:  writer.writeFloat((Float)buf.get(i)); break;
					case FieldInfo.NSM_DOUBLE: writer.writeDouble((Double)buf.get(i)); break;
					default : break;
					}
				}
			}
    ...
    public String getEnum(String name) { return ""; }
