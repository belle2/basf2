...
import b2dqm.core.TimedGraph1F;
...
import b2dqm.io.CanvasProperty;
import b2dqm.io.NSMDataManager;
import b2dqm.io.GHistoProperty;
import b2dqm.io.GTextProperty;
	...
	public Component create(String label) {
		...
		if (str_v.length > 0) {
			...
			if (type.matches("TablePanel")) {
				...
				if (_config.hasKey(label+".list")) {
					str_v = _config.getString(label+".list").split(",");
					for (String str : str_v) {
