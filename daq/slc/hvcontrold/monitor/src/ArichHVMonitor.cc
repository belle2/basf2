...
void ArichHVMonitor::update(NSMData* data) throw()
{
  try {
    ...
    //return;
    _gr_voltage->addPoint(data->getInt32("voltage_mon_0"));
    _gr_current->addPoint(data->getInt32("current_mon_0"));
    _label_state->setFontColor(MonColor::WHITE);
    int s = data->getByte("status_0");
    if (s == 1) {
      ...
      _label_state->setText("ERROR");
    }
    _label_voltage_mon->setText(Belle2::form("%d [digit]", data->getInt32("voltage_mon_0")));
    _label_current_mon->setText(Belle2::form("%d [digit]", data->getInt32("current_mon_0")));
