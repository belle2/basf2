...
public class SystemButtonPanel extends JPanel implements Updatable {
	...
	public SystemButtonPanel(RCNodeSystem system) {
		_system = system;
	...
	public void update() {
		...
		_button_boot.setEnabled(_system.getVersion() >= 0 && 
				_boot_command.available(state)>=RCCommand.ENABLED);
		_button_load.setEnabled(_system.getVersion() >= 0 && 
					_load_command.available(state)>=RCCommand.ENABLED);
		_button_start.setEnabled((_system.getVersion() >= 0 || 
