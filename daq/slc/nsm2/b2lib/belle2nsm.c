...
/* -- b2nsm_openmem_str -------------------------------------------------- */
void *
b2nsm_openmem_str(const char *filebuf, const char *dat, const char *fmt, int rev)
{
  char dat_uprcase[NSMSYS_NAME_SIZ+1];
  void *ptr;
  if (! nsm) return 0;
  xuprcpy(dat_uprcase, dat, NSMSYS_NAME_SIZ+1);
  ptr = nsmlib_openmem_str(nsm, filebuf, dat, fmt, rev);
...
/* -- b2nsm_allocmem_str ------------------------------------------------- */
void *
b2nsm_allocmem_str(const char *filebuf, const char *dat, 
		   const char *fmt, int rev, float cycle)
{
  char dat_uprcase[NSMSYS_NAME_SIZ+1];
  void *ptr;
  if (! nsm) return 0;
  xuprcpy(dat_uprcase, dat, NSMSYS_NAME_SIZ+1);
  ptr = nsmlib_allocmem_str(nsm, filebuf, dat_uprcase, fmt, rev, cycle);
