...
static NSMcmdtbl_t nsmd_cmdtbl[] = {
  ...
  { NSMCMD_NEWREQ,      0, "NEWREQ",      nsmd_do_newreq },
...
#define ADDR_IP(a)    ((uint32)((a).sin_addr.s_addr)) // network-byte-order
...
#define NodeIsValid(i)  (nsmd_sysp && i >= 0 && i < NSMSYS_MAX_NOD)
...
time10ms()
{
  struct timeval tv;
  gettimeofday(&tv, 0);
  return (uint64)tv.tv_sec * 100 + tv.tv_usec / 10000; // in 10msec unit
}
// -- time1ms -----------------------------------------------------------
static uint64
time1ms()
...
nsmd_destroy(int code = NSMD_EUNEXPCT)
{
  ...
  switch (code) {
    ...
    case NSMD_ESYSERR:  strcpy(str, " (syscall)");    exit_code = -code; break;
    case NSMD_EINUSE:   strcpy(str, " (in-use)");     exit_code = -code; break;
    case NSMD_EUNEXPCT: strcpy(str, " (unexpected)"); exit_code = -code; break;
    case NSMD_ESHMGET:  strcpy(str, " (shmget)");     exit_code = -code; break;
    case NSMD_ESHMPERM: strcpy(str, " (shmperm)");    exit_code = -code; break;
    case NSMD_ENONETIF: strcpy(str, " (no-net-if)");  exit_code = -code; break;
    case NSMD_ETIME:    strcpy(str, " (wrong-time)"); exit_code = -code; break;
      // signals
    case SIGTERM:      strcpy(str, " (SIGTERM)"); break;
    case SIGINT:       strcpy(str, " (SIGINT)");  break;
    case SIGCHLD:      strcpy(str, " (SIGCHLD)"); break;
    case SIGPIPE:      strcpy(str, " (SIGPIPE)"); break;
    default:           sprintf(str, " (sig=%d)", code);
  }
...
nsmd_shmclean()
{
  ...
  int headlen = sizeof(int32) * 2 + sizeof(int64) * 2;
  ...
  if ((retsys == 0 || retsys == ENOENT) &&
    ...
    if (sys.pid == mem.pid && sys.ipaddr == mem.ipaddr &&
      ...
      if (kill(sys.pid, 0) < 0 && errno == ESRCH) { // unexpected
        WARN("%s: pid=%d, t=%d/%d",
             "shared memory without process, client maybe still running",
             sys.pid, sys.timstart, sys.timevent);
        WARN("NSMD should be able to kill clients HERE, to be implemented.");
      } else {
...
// npar!=0 to use bufp->dat to send int32*npar
             ...
             NSMudpbuf* bufp = 0, int len = 0, int npar = 0, uint32* pars = 0)
{
  ...
  (byte8)bufp->dat[0], (byte8)bufp->dat[1],
  (byte8)bufp->dat[2], (byte8)bufp->dat[3]);
  }
  */
...
nsmd_udpsend_not(SOCKAD_IN* sockadp, NSMdmsg& dmsg)
{
  ...
  // check length
  int len = dmsg.npar * sizeof(int32) + dmsg.len;
...
nsmd_tcpwriteq()
{
  ...
  int headparlen = sizeof(NSMtcphead) + q->npar * sizeof(int32);
  ...
  if (ret == writelen && q->datap) {
    nsmd_free("tcpwriteq", q->datap - q->npar * sizeof(int32));
  }
  ...
  DBG("writeq before f=%08x(%08x) l=%08x q=%08x",
  ...
  DBG("writeq after f=%08x(%08x) l=%08x q=%08x",
      nsmd_tcpqfirst, nsmd_tcpqfirst ? nsmd_tcpqfirst->prevp : 0, nsmd_tcpqlast, q);
  if (q->msglen > NSM_TCPTHRESHOLD)
    nsmd_free("tcpwriteq", (char*)q);
...
nsmd_tcpsend(NSMcon& con, NSMdmsg& dmsg, NSMDtcpq* qptr, int beforeafter)
{
  ...
  if ((req == NSMCMD_SYSCPYMEM || req == NSMCMD_REQCPYMEM) &&
      (!DBGBIT(4) || !sys.ready || len == NSMD_TCPSENDSIZ)) dispflag = 0;
  ...
  q->req   = dmsg.req;
  q->npar  = dmsg.npar;
  q->msglen = sizeof(NSMtcphead) + dmsg.npar * sizeof(int32) + dmsg.len;
  q->pos   = 0;
  ...
  // setup parameters and data buffer or pointer
  // dmsg.datap が sys のどこかを差してたらサイズにかかわらず datap にすべし
...
nsmd_newconn(int conid, int32 ip, const char* contype)
{
  ...
  con.ready  = sys.ready == -1 ? 0 : sys.ready;
  con.timevent = time(0);
  sys.dirty  = 1;
  return sock;
}
...
nsmd_destroyconn(NSMcon& con)
{
  ...
  if (itslocal) {
    ...
    if (NoMaster()) {
      ASRT("destroyconn: master is missing");
      ...
      // disappeared one is not a MASTER/DEPUTY
    }
  }

  // long procedure here...

  if (sock_save == con.sock) { // if not equal, it might be already removed
    shutdown(con.sock, 2);
    close(con.sock);
    nsmd_delconn(con);
  }
...
nsmd_sch_initbcast(int16 conid, int32 opt)
{
  ...
  if (sys.master >= NSMCON_TCP) {
    ...
    if (sys.ready > 0) {
      ...
      nsmd_init_count--;
      next = 100; // 1 sec
      LOG("standing for new daemon (%d)", nsmd_init_count);
    }
  } else if (sys.master < 0) { // nsmd_init_count == 0
    LOG("select myself as MASTER");
    nsmd_init_count = -1;
    sys.con[NSMCON_TCP].timstart = sys.con[NSMCON_UDP].timstart;
    sys.con[NSMCON_UDP].timevent = time(0);
    sys.master = NSMCON_TCP;
    sys.ready  = 3;  // what does 3 mean?
    sys.generation++;
    return -1;
            ...
            int* ichunkp = 0)
{
  ...
  static char sizc[] = "1248";
  int loc = 0;
  const int chunksiz = NSM_UDPDATSIZ; // = 1484
  ...
  DBG("fmtcpy datid %d buftodat %d pos %d len %d fmt %s %02x %02x %02x %02x",
      ...
      (byte8)src[0], (byte8)src[1], (byte8)src[2], (byte8)src[3]);
  */

  if (len == 0) len = chunksiz;
  if (pos == pos_cache && datid == dat_cache) {
    fmt      = fmt_cache;
    ichunk   = ichunk_cache;
  ...
  while (*fmt) {
    ...
    if (siz == 0) {
      ASRT("fmtcpy siz=%d datid=%d fmt_cache=%p fmt=%s %p %p", siz, datid, fmt_cache, dat.dtfmt, dat.dtfmt, fmt);
    }
...
// -- nsmd_fmtcpy_old -------------------------------------------------------
// If I receive NSMD_MAXINVFMT (32) times wrong data in NSMD_TIMINVFMT
// (3600 = 1 hour), I must be a wrong node, so I exit.
#if 0
int
nsmd_fmtcpy_old(void* dst, void* src, int siz)
{
  NSMmem& mem = *nsmd_memp;
  NSMsys& sys = *nsmd_sysp;
  NSMdat* datp;
  ...
  int memid, sdiff, cnt;
  char* fmt;
  static int ninvfmt = 0;
  static int tinvfmt[NSMD_MAXINVFMT] = {0,};

  DBG("fmtcpy: (spos=%d dpos=%d siz=%d)", spos, dpos, siz);

  // dst is inside the memory
  if (dpos >= 0 && dpos + siz <= NSM_MEMSIZ) {
    if ((memid = nsmd_checkmem(0, dpos)) < 0) {
      WARN("fmtcpy: checkmem error=%d dpos=%d", memid, dpos);
      return memid;
    }
    datp = sys.dat + memid;
    sdiff = dpos - ntohl(datp->dtpos);

    // src is inside the memory
  } else if (spos >= 0 && spos + siz <= NSM2_MEMSIZ) {
    if (spos % 8) {
      WARN("fmtcpy: wrong src memory alignment");
      return NSMEMEMALIGN;
    }
    if ((memid = nsmd_checkmem(0, spos)) < 0) {
      WARN("fmtcpy: checkmem error=%d spos=%d", memid, spos);
      return memid;
    }
    datp = sys.dat + memid;
    sdiff = spos - ntohl(datp->dtpos);
  } else {
    WARN("fmtcpy: wrong src/dest spos=%d dpos=%d", spos, dpos);
    return NSMEINVPTR;
  }

  if (siz == 0)
    siz = ntohl(datp->dtsiz);

  if (sdiff + siz > ntohs(datp->dtsiz)) {
    int now = time(0);
    int i, first = 0, last = 0;

    for (last = 0; last < NSMD_MAXINVFMT; last++) {
      if (! tinvfmt[last]) break;
      if (tinvfmt[i] < now - NSMD_TIMINVFMT) first = last + 1;
    }
    if (first) {
      for (i = 0; i < NSMD_MAXINVFMT - first; i++)
        tinvfmt[i] = tinvfmt[i + first];
      for (; i < NSMD_MAXINVFMT; i++)
        tinvfmt[i] = 0;
      last -= first;
    }
    if (last >= NSMD_MAXINVFMT) {
      ASRT("fmtcpy: wrong siz=%d sdiff=%d dtsiz=%d(s=%d d=%d did=%d)",
           siz, sdiff, ntohl(datp->dtsiz), spos, dpos, memid);
    }
    tinvfmt[last] = now;
    WARN("fmtcpy: wrong siz=%d sdiff=%d dtsiz=%d(s=%d d=%d did=%d)",
         siz, sdiff, ntohl(datp->dtsiz), spos, dpos, memid);
    return NSMEINVPTR;
  }
  fmt = datp->dtfmt;
  //datp->dtcnt = htonl(ntohl(datp->dtcnt) + 1);

  while (*fmt) {
    static char fmtc[] = "aAccsSiIfFlLdD";
    static char lenc[] = "11112244448888";
    char* p = strchr(fmtc, *fmt++);
    int len;

    if (! p) continue;

    len = lenc[p - fmtc] - '0';
    cnt = strtoul(fmt, &p, 10);
    fmt = p;
    ...
    while (cnt-- > 0) {
      if (sdiff > 0) {
        ...
        src = (char*)src + len;
        dst = (char*)dst + len;
      } else if (siz == 0 && sdiff == 0) {
        return 0;
      } else {
        WARN("fmtcpy: wrong siz or sdiff %d/%d", siz, sdiff);
        return NSMEINVPTR;
      }
    }
  }
  if (siz >= 0 && siz < MEMALIGN(1) && sdiff == 0) return 0;
...
nsmd_setup_daemon(NSMcon& con)
{
  ...
  dmsg.pars[6] = hlltohl(tcpcon.timstart, 0);
  dmsg.pars[7] = hlltohl(tcpcon.timstart, 1);
  dmsg.pars[8] = sys.generation;

  if (&con < sys.con || &con >= sys.con + NSMSYS_MAX_CON) {
    NSMudpbuf udpbuf;
    memcpy(udpbuf.dat, dmsg.pars, sizeof(int32) * 9);
...
nsmd_touchsys(NSMcon& con, int pos, int siz)
{
  ...
  // loop over tcpq
  int off = sizeof(NSMtcphead) + 2 * sizeof(int32); // always 2 params
  NSMDtcpq* q = nsmd_tcpqfirst;
...
nsmd_setup_client(NSMcon& con, uid_t uid, pid_t pid, const char* name)
{
  ...
  for (nodp = sys.nod; nodp < nodlastp; nodp++) {
    if (strcmp(nodp->name, name) == 0) {
      if (nodp->nodpid == 0) { // everything else may stay the same
        break; // unused name is there
      } else {
        LOG("NEWCLIENT: nodename %s is in use", name);
        return NSMENODEEXIST;
      }
  ...
  nsmd_touchsys(con, SYSPOS(&sys.nodhash[hash]), sizeof(int32));
...
nsmd_setup_req(NSMcon& con, const char* name)
{
  ...
  nsmd_touchsys(con, SYSPOS(&sys.reqhash[hash]), sizeof(int32));
...
nsmd_delete_req(NSMcon& con, int reqcode)
{
  ...
  if (hash + nshift > NSMSYS_MAX_HASH) {
    ...
    nsmd_touchsys(con, SYSPOS(&sys.reqhash[hash]), sizeof(int32) * a);
    nsmd_touchsys(con, SYSPOS(&sys.reqhash[0]),    sizeof(int32) * b);
  } else {
    nsmd_touchsys(con, SYSPOS(&sys.reqhash[hash]), sizeof(int32) * nshift);
  }
...
#define IsInvalidSysSrcDest(src, dest, con) \
  ((NodeIsValid(src)   && dest != (uint16)-1) ||  \
   (NodeIsValid(dest)  && src  != (uint16)-1) || \
   (ConIsLocal(con)    && dest != (uint16)-1) || \
   (ConIsMaster(con)   && src  != (uint16)-1) || \
   (! ConIsMaster(con) && dest != (uint16)-1))
...
nsmd_do_delclient(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  if (dmsg.src != -1) ASRT("do_delclient invalid src=%d", dmsg.src);
...
nsmd_do_usrcpymem(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  nsmd_fmtcpy((char*)dmsg.datap, dmsg.pars[0], 1, offset, dmsg.len);
}
// -- nsmd_do_ackdaemon -------------------------------------------------
//
// [for non-master]
...
nsmd_do_ackdaemon(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  // connect to master
  sys.ready = sys.generation > 0 ? -1 : 0;
  ...
  if (sys.master == -1) {
    ...
    sys.con[NSMCON_TCP].timstart = hl2tohll(dmsg.pars[6], dmsg.pars[7]);
...
nsmd_do_syscpymem(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  int opt = dmsg.opt;
  static int seq = 0;
  const int tcpdatmax = NSM_TCPDATSIZ - 4 * sizeof(int32); // 64k - 16byte
  static int datid = -1;
...
nsmd_do_reqcpymem(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  if (opt == 2) {
    datid = pos;
    siz = (int16)ntohs(sys.dat[datid].dtsiz); // 0 if removed, but don't care
    pos = (int16)ntohs(sys.dat[datid].dtpos);
  }
...
nsmd_do_ready(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  if (opt == 1 && IamDeputy()) { // ready after NSMCMD_SYSCPYMEM
    ...
    if (prio > nsmd_priority ||
      ...
      sys.deputy = conid;
      DBG("new deputy, conid=%d ip=%s", conid, ADDR_STR(con.sockad));
    } else {
      return;
    }
  }
...
nsmd_do_newmaster(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  if (newm == oldd && newd == oldm) {
    ...
    // no new connection
    if (newm == nsmd_myip) LOG("newmaster: converted to MASTER");
    if (newd == nsmd_myip) LOG("newmaster: converted to DEPUTY");
    // still has to send back CMD_READY if it's me
  } else if (newm == oldm || newm == oldd) {
    LOG("newmaster: new DEPUTY");
    newtyp  = "DEPUTY";
    chgidp  = &sys.deputy;
    keepidp = &sys.master;
    newip   = newd;
    newtim  = hl2tohll(dmsg.pars[4], dmsg.pars[5]);
  } else if (newd == oldd || newd == oldm) {
    LOG("newmaster: new MASTER");
    newtyp  = "MASTER";
    chgidp  = &sys.master;
    keepidp = &sys.deputy;
    newip   = newm;
    newtim  = hl2tohll(dmsg.pars[2], dmsg.pars[3]);
  } else {
    ASRT("do_newmaster: should not come here");
  }

  // disconnect and/or connect:
  NSMcon* conp = 0; // where to connect and/or to send CMD_READY
  int newopt = 0;   // and opt = 0 unless connection failed
  if (itsme) {
    LOG("newmaster: it's me");
    conp = &con;
    *chgidp = NSMCON_TCP;
  } else {
    // - do nothing if I was a member and I'm a new MASTER/DEPUTY

    if (itwasme) {
      LOG("newmaster: it was me");

      // - disconnect all but new MASTER/DEPUTY if I was a MASTER/DEPUTY
      //   and I'm not a new MASTER/DEPUTY
      // - newip should be there
      if (! chgidp)
        ASRT("do_newmaster: it's not me but new %s is unknown", newtyp);

      *chgidp = NSMCON_NON;
      for (int conid = NSMCON_OUT; conid < sys.ncon; conid++) {
        if (conid == *keepidp)   continue;
        if (ConidIsLocal(conid)) continue;
        if (AddrConid(conid) == newip) {
          if (*chgidp != NSMCON_NON)
            ASRT("do_newmaster: more than one connection to newip");
          *chgidp = conid;
          continue;
        }
      ...
      //   old DEPUTY
      if (*chgidp == NSMCON_NON) {
        LOG("newmaster: new %s is not connected to me before", newtyp);
        *chgidp = sys.ncon;
      }

    } else if (chgidp) {
      // - disconnect the old master/deputy
      //   -- if the connection is still there,
      //   -- if I was and will be a MEMBER, and
      //   -- if it's not just swapping MASTER/DEPUTY
      ...
      *chgidp = sys.ncon;
      for (int conid = NSMCON_OUT; conid < sys.ncon; conid++) {
        if (AddrConid(conid) == newip) *chgidp = conid;
      }
      if (*chgidp != sys.ncon)
        ASRT("do_newmaster: new %s is already connected", newtyp);
    }

    // - connect to new master/deputy if I'm a MEMBER and it is not
    //   connected yet
    if (*chgidp == sys.ncon) {
      if (nsmd_newconn(*chgidp, newip, newtyp) < 0) {
        // but if it failed to connect
        *chgidp = NSMCON_NON;
...
// -- nsmd_fmtcheck -----------------------------------------------------
int
nsmd_fmtcheck(const char* fmtname)
{
  int fmtsize = 0;
  for (const char* p = fmtname; *p; p++) {
    int siz = 0;
    switch (p[0]) {
      case 'a': siz = 1; break;
      case 's': siz = 2; break;
...
nsmd_do_allocmem(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  // otherwise datap should have two strings
  ...
  // check the format
  char fmtname[NSMSYS_DNAM_SIZ + 1];
  strncpy(fmtname, p + 1, dmsg.len - len - 1);
  fmtname[dmsg.len - len - 1] = 0;
  int fmtsize = nsmd_fmtcheck(fmtname);
  ...
  while (datp) {
    ...
    if (strcmp(datname, datp->dtnam) == 0) {
      if (datp->owner != -1) {
         ...
         */
        ASRT("need to write a new code.");

      } else if (strcmp(fmtname, datp->dtfmt) != 0) {
        nsmd_tcpreply(con, dmsg, dmsg.src, NSMEMEMEXIST);
        return;
      }
      ...
      *nprevp = htons(datp - sys.dat);
      nsmd_touchsys(con, SYSPOS(nprevp), sizeof(int16));
    ...
    if (ntohs(datp->anext) == (uint16) - 1) break;
  ...
  if (! existed) {
    ...
    if (datp_aprev) {
      ...
      nsmd_touchsys(con, SYSPOS(&datp_aprev->anext), sizeof(int16));
    } else {
      sys.afirst  = ntohs(datp - sys.dat);
      datp->anext = ntohs(-1);
      nsmd_touchsys(con, SYSPOS(&sys.afirst), sizeof(int16));
    }
  }
...
nsmd_command(NSMcon& con, NSMdmsg& dmsg)
{
  ...
  if (dmsg.req >= NSMCMD_FIRST && dmsg.req <= NSMCMD_LAST) {
    ...
    if (cmd.printable && dmsg.datap) {
      LOG("recv %s(%d)<=%s  len=%d npar=%d data=%s",
          cmd.name, dmsg.seq,
          ADDR_STR(con.sockad), dmsg.len, dmsg.npar, dmsg.datap);
    } else {
...
nsmd_udprecv()
{
  ...
  dmsg.len = recvlen - headlen - recvbuf.npar * sizeof(int32);
  dmsg.datap = dmsg.len ? &recvbuf.dat[recvbuf.npar * sizeof(int32)] : 0;
  dmsg.src = dmsg.dest = -1;
  dmsg.from = fromaddr;
...
nsmd_tcprecv(NSMcon& con)
{
  ...
  int datlen = head.npar * sizeof(int32) + ntohs(head.len);
  ...
  // just return if not fully received
  if (recvlen < recvsiz ||
      (bsiz + recvlen == sizeof(NSMtcphead) &&
...

  {
    ...
    int req  = ntohs(head.req);
    int len  = ntohs(head.len);
    DBG("tcprecv req=%x len=%d npar=%d p=[%d,%d] coni=%d",
        req, len, npar, ntohl(p[0]), ntohl(p[1]), coni);
  }


  //DBG("tcprecv req=%04x(%d) %x=>%x p=%d/%d len=%d",
  //  req, seq, src, dest, p0, p1, len);
  con.icnt++;
  con.isiz += bsiz;
  con.timevent = time(0);
  bsiz = 0;
  ...
  // if data pointer is not cleared by the command handler,
  // clear it here
  if (dmsg.datap == nsmd_longbuf) {
    nsmd_longused = 0;
  } else if (dmsg.datap && msglen > NSM_TCPTHRESHOLD) {
...
disconnect_return:
  ...
  if (datap == nsmd_longbuf) {
    nsmd_longused = 0;
  } else if (datap && msglen > NSM_TCPTHRESHOLD) {
    nsmd_free("tcprecv", datap);
  }
...
// ----------------------------------------------------------------------
int
nsmd_select(fd_set& fdset, int wait)
{
  timeval tv;
  tv.tv_sec  = wait; // no wait if zero
  tv.tv_usec = 0;
...
main(int argc, char** argv)
{
  ...
  DBG("sizeof(uint32)=%d", sizeof(uint32));
  DBG("sizeof(uint64)=%d", sizeof(uint64));
  DBG("sizeof(int16)=%d",  sizeof(int16));
  DBG("sizeof(int32)=%d",  sizeof(int32));
  DBG("sizeof(int64)=%d",  sizeof(int64));
