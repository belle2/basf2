...
std::string Histo::toXML() const throw()
{
  ...
  ss << ">" << std::endl
     << "<title><![CDATA["
     << B2DAQ::replace(getTitle(), "\n", "<br/>")
     << "]]></title>" << std::endl
     << "</histo>" << std::endl;
  return ss.str();
}

void Histo::readObject(B2DAQ::Reader& reader) throw(B2DAQ::IOException)
{
  readConfig(reader);
  readContents(reader);
}

void Histo::readConfig(B2DAQ::Reader& reader) throw(B2DAQ::IOException)
{
  MonObject::readConfig(reader);
  _title = reader.readString();
}

void Histo::writeObject(B2DAQ::Writer& writer) const throw(B2DAQ::IOException)
{
  writeConfig(writer);
  writeContents(writer);
}

void Histo::writeConfig(B2DAQ::Writer& writer) const throw(B2DAQ::IOException)
{
  MonObject::writeConfig(writer);
  writer.writeString(_title);
}

void Histo::readContents(B2DAQ::Reader& reader) throw(B2DAQ::IOException)
{
  _data->readObject(reader);
}

void Histo::writeContents(B2DAQ::Writer& writer) const throw(B2DAQ::IOException)
{
