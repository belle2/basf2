...
#define PCI_DEVICE_FILE ("/proc/bus/pci/devices")
  ...
  *paths_p_ret = paths_p;
  ...
  while( fgets(buf,sizeof(buf),fp) ){
    ...
    sprintf(paths_p[nfound++], "%s/%02x/%02x.%1x", 
	    PCI_PATH, pci_bus, pci_slot, pci_func);
...
char* get_copper_plx9054_path(void) {
  ...
  case 2:
    /* 
       both COPPER and TTRX has PLX9054 
       -> assume COPPER's PLX9054 resides on smaller id bus
    */
