...
bool HVControlMaster::save(const std::string& nodename)
{
  ...
  if (nodename.size() == 0) {
    for (HVCrateInfoMap::iterator it = _crate_m.begin();
         it != _crate_m.end(); it++) {
      if (!save_local(db, it->first)) return false;
    }
  } else {
    if (!save_local(db, nodename)) return false;
  }
  db.close();
  return true;
}
...
bool HVControlMaster::recall(const std::string& str)
{
  ...
  for (size_t i = 0; i < nodename_v.size(); i++) {
    ...
    if (confno <= 0) {
      try {
        db.execute(Belle2::form("select confno from hv_database where node = '%s' order by confno desc limit 1;",
                                nodename.c_str()));
...
bool HVControlMaster::remove(const std::string& str)
{
  ...
  db.connect(config.get("HV_DATABASE_HOST"), config.get("HV_DATABASE_NAME"),
             ...
             config.getInt("HV_DATABASE_PORT"));
  try {
    db.execute(Belle2::form("delete from hv_database %s;", opt.c_str()));
...
bool HVControlMaster::list(const std::string& str)
{
  ...
  db.connect(config.get("HV_DATABASE_HOST"), config.get("HV_DATABASE_NAME"),
             ...
             config.getInt("HV_DATABASE_PORT"));
  try {
    db.execute(Belle2::form("select * from hv_database where slot = 1 and channel = 1 %s order by node;",
                            cmd.c_str()));
    std::vector<DBRecord>& record_v(db.loadRecords());
...
bool HVControlMaster::save_local(DBInterface& db, const std::string& nodename)
{
  HVCrateInfo* crate = _crate_m[nodename];
  try {
