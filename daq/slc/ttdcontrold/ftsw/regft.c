...
main(int argc, char **argv)
{
  ...
  if (argc < 3 || readonly) {
    ftsw = ftsw_open(ftid, FTSW_RDONLY);
  } else {
    ftsw = ftsw_open(ftid, FTSW_RDWR);
  }
  if (! ftsw) {
    perror("open");
  ...
  while (argc > 1) {
    ...
    if (isdigit(reg[0])) {
      ...
      if (ftsw_write(ftsw, FTSW_SVER, 0x58434c4b) < 0) {
	perror("write.xclk");
	return 1;
      }
      argc--, argv++;
      ...
      if (ftsw_write(ftsw, FTSW_SVER, 0x53544f50) < 0) {
	perror("write.stop");
	return 1;
      }
      argc--, argv++;
      ...
      if (ftsw_write(ftsw, FTSW_SVER, 0x434c4b00 | value) < 0) {
	perror("write.sclk");
	return 1;
      }
      argc--, argv++;
      ...
      int s3id  = ftsw_read(ftsw, FTSW_SID);
      int s3ver = ftsw_read(ftsw, FTSW_SVER);
      int v5id  = ftsw_read(ftsw, FTSW_VID);
      int v5ver = ftsw_read(ftsw, FTSW_VVER);
