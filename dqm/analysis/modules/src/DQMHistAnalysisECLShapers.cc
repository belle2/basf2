...
void DQMHistAnalysisECLShapersModule::initialize()
{
  ...
  if (getUseEpics()) {
    ...
    for (int i = 0; i < c_collector_count; i++) {
      std::string pv_name = (boost::format("ECL:logic_check:crate%02d") %
                             (i + 1)).str();
      SEVCHK(ca_create_channel(pv_name.c_str(), NULL, NULL, 10, &chid_logic[i]), "ca_create_channel failure");
    }
    SEVCHK(ca_create_channel("ECL:pedwidth:max_fw", NULL, NULL, 10,
                             &chid_pedwidth[0]), "ca_create_channel failure");
    SEVCHK(ca_create_channel("ECL:pedwidth:max_br", NULL, NULL, 10,
                             &chid_pedwidth[1]), "ca_create_channel failure");
    SEVCHK(ca_create_channel("ECL:pedwidth:max_bw", NULL, NULL, 10,
                             &chid_pedwidth[2]), "ca_create_channel failure");
    SEVCHK(ca_create_channel("ECL:pedwidth:max_al", NULL, NULL, 10,
