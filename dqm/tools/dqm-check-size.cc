...
namespace {
  void streamHistograms(TDirectory* directory, Belle2::MsgHandler& msgHandler, const std::string& directoryName = "")
  {
    TList* keylist = directory->GetList();
    ...
    while ((key = dynamic_cast<TKey*>(nextkey()))) {
      ...
      if (objectClass->InheritsFrom(TH1::Class())) {
        ...
        streamHistograms(subDirectory, msgHandler, object->GetName());
      }
    }
  }
}
...
int main(int argc, char* argv[])
{
  if (argc == 1 or std::string(argv[1]) == "--help" or std::string(argv[1]) == "-h") {
    std::cout << "Usage: " << argv[0] << " INPUT_FILE\n\n"
              "   This tool checks if the size of the DQM histogrames can be handled by the HLT\n"
              "   by compressing and decompressing the histograms as done within our ZeroMQ framework.\n";
    return 0;
  }
  ...
  std::unique_ptr<TFile> inputFile = std::unique_ptr<TFile>(TFile::Open(inputFileName.c_str(), "READ"));
  if (!inputFile or !inputFile->IsOpen() or inputFile->IsZombie()) {
    B2ERROR("The input file is not working!");
    return 1;
  }
  Belle2::MsgHandler msgHandler;
  streamHistograms(gDirectory, msgHandler);
  std::unique_ptr<Belle2::EvtMessage> evtMessage = std::unique_ptr<Belle2::EvtMessage>(msgHandler.encode_msg(
                                                     Belle2::ERecordType::MSG_EVENT));
  unsigned int maximalCompressedSize{100000000};
  std::vector<char> compressedBuffer;
  compressedBuffer.resize(maximalCompressedSize, 0);
  int compressedSize = LZ4_compress_default(evtMessage->buffer(), &compressedBuffer[0],
                                            evtMessage->size(), maximalCompressedSize);
  if (compressedSize <= 0) {
    B2ERROR("LZ4_compress_default failed"
            << LogVar("file", inputFileName)
            << LogVar("original size", evtMessage->size())
            << LogVar("compressed size", compressedSize));
    return 1;
  }
  zmq::message_t message(&compressedBuffer[0], compressedSize);
  unsigned int maximalUncompressedSize{128000000};
  std::vector<char> uncompressedBuffer;
  uncompressedBuffer.reserve(maximalUncompressedSize);
  int uncompressedSize = LZ4_decompress_safe(message.data<char>(), &uncompressedBuffer[0],
