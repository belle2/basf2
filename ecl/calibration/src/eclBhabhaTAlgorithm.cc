...
CalibrationAlgorithm::EResult eclBhabhaTAlgorithm::calibrate()
{
  ...
  /*
     B2INFO( "Default perfix: getPrefix() = " << getPrefix() ) ;
     B2INFO( "Setting user passed prefix" ) ;
     B2INFO( "collectorName = " << collectorName ) ;
     setPrefix( collectorName.c_str() ) ;
     B2INFO( "New prefix: getPrefix() = " << getPrefix() ) ;
  */
  ...
  //auto TimevsCrateNoCalibrations = getObjectPtr<TH2F>("TimevsCrateNoCalibrations") ;
  ...
  //auto TimevsRunPrevCrateCalibPrevCrystCalibCID1338 = getObjectPtr<TH2F>("TimevsRunPrevCrateCalibPrevCrystCalibCID1338") ;
  //auto TimevsRunPrevCrateCalibPrevCrystCalibCrate8 = getObjectPtr<TH2F>("TimevsRunPrevCrateCalibPrevCrystCalibCrate8") ;
  ...
  //histfile = new TFile("eclBhabhaTAlgorithm.root", "update") ;
  ...
  for (auto expRun : getRunList()) {
    ...
    //if ( minRunNum > runNumber )
    if (((minRunNum > runNumber)   && (minExpNum >= expNumber))  ||
        (minExpNum > expNumber)) {
      minExpNum = expNumber ;
      minRunNum = runNumber ;
    }
    //if ( maxRunNum < runNumber )
  ...
  //TimevsCrateNoCalibrations              ->Write() ;
  TimevsCrysPrevCrateCalibNoCrystCalib   ->Write() ;
  TimevsCrateNoCrateCalibPrevCrystCalib  ->Write() ;

  /*
     //  If we are doing a crystal calibration then try to save the plots for individual crystals but if not then the plot may not exist since the crystal may not have been hit so skip the plot
     if ( cellIDLo < cellIDHi )
     {
        auto TimevsRunPrevCrateCalibPrevCrystCalibCrate8 = getObjectPtr<TH2F>("TimevsRunPrevCrateCalibPrevCrystCalibCrate8") ;
        auto TimevsRunPrevCrateCalibPrevCrystCalibCID1338 = getObjectPtr<TH2F>("TimevsRunPrevCrateCalibPrevCrystCalibCID1338") ;
        TimevsRunPrevCrateCalibPrevCrystCalibCrate8  ->Write() ;
        TimevsRunPrevCrateCalibPrevCrystCalibCID1338 ->Write() ;
     }
  */
  ...
  if (debugOutput) {
    ...
    //tree_crystal->Branch("tcrate", &mean_crate)->SetTitle("Time offset mean, tcrate, ns") ;
    //tree_crystal->Branch("tcrateUnc", &mean_crate_unc)->SetTitle("Time offset mean, tcrate, ns") ;
    //tree_crystal->Branch("tcrateSigma", &sigma_crate)->SetTitle("Time offset mean, tcrate, ns") ;
    //tree_crystal->Branch("chi2", &chi2)->SetTitle("Chi2") ;   // comment out until we re-introduce fits
  ...
  /*
     //  Fill payload info with zeros just to get right length
     for (int i = 1 ; i <= 8736; i++)
     {
        t_offsets.push_back(0) ;
        t_offsets_unc.push_back(0) ;
     }
  ...
  */
  ...
  for (int crys_id = cellIDLo ; crys_id <= cellIDHi ; crys_id++) {
    ...
    */
    ...
    //TH1D* h_time = TimevsCrysNoCalibrations->ProjectionY("h_time_psi", crys_id, crys_id) ;
    ...
    //TH1D* h_timeMask = TimevsCrysNoCalibrations->ProjectionY("h_time_psi", crys_id, crys_id) ;
    //TH1D* h_timeMasked = TimevsCrysNoCalibrations->ProjectionY("h_time_psi", crys_id, crys_id) ;
    //TH1D* h_timeRebin = TimevsCrysNoCalibrations->ProjectionY("h_time_psi", crys_id, crys_id) ;

    //TH1D* h_time = (TH1D*)h_timeRebin->Clone() ;
        ...
        for (int rebinCounter = 1; rebinCounter <= meanCleanRebinFactor; rebinCounter++) {
          ...
          if (nonRebinnedBinNumber < h_time->GetNbinsX()) {
            if (h_timeRebin->GetBinContent(bin) >= histRebin_max * meanCleanCutMinFactor)
              //B2INFO( "rebin value, rebin max value = " << h_timeRebin->GetBinContent(bin) << ", " << histRebin_max ) ;
              //if ( h_timeRebin->GetBinContent(bin) == histRebin_max )
            {
    ...
    //double sig_min(0.5), sig_max(15) ;   // weird usage ??
    //double sig_min = 0 ;
    //double sig_max = ( fabs(hist_tmin) + fabs(hist_tmax) ) *2 /3. ;
    ...
    //mean = default_meanMasked ;
    //mean_unc = default_meanMasked_unc ;
    //mean = default_mean_masked ;
    //mean_unc = default_mean_unc ;
    sigma = default_sigma ;
    chi2 = -1 ;
    ...
    if ((numEntries >= minNumEntries)  &&  good_fit) {
      ...
      //return c_NotEnoughData ;   // can't do this becaue of crystals outside CDC acceptance.
    }
    /*
          if ( numEntries == 0 )
          {
    ...
    */
    ...
    /*
       if (numEntries < minNumEntries) {
       if (crys_id < 290) {
       crystalCalibSaved = -1 ;
       mean = -34 ;
       database_mean = mean ;
       mean_unc = 20 ;
       database_mean_unc = mean_unc ;
       } else if (crys_id > 8540) {
       crystalCalibSaved = -1 ;
       mean = -47 ;
       database_mean = mean ;
       mean_unc = 27 ;
    ...
    // HACK FOR OCT 1 2018 CALIBRATION !!!!!
    if (crys_id < 161) {
    crystalCalibSaved = -1 ;
    mean = -34 ;
    database_mean = mean ;
    ...
    */
    ...
    //histfile->WriteTObject(h_timeMask, (std::string("h_time_psi_mask") + std::to_string(crys_id)).c_str()) ;
    //histfile->WriteTObject(h_timeRebin, (std::string("h_time_psi_rebinned") + std::to_string(crys_id)).c_str()) ;
  ...
  */
  ...
  for (int crate_id = crateIDLo ; crate_id <= crateIDHi ; crate_id++) {
    ...
    //double Ts_previous = TsDatabase->GetBinContent( crate_id ) ; // wrong .  bad input crate not crystal
    //double Tcrate_previous = TcrateDatabase->GetBinContent( crate_id ) ;
    //tcrate_mean_prev[crate_id-1] = TcrateDatabase->GetBinContent( crate_id ) / numTimesFilled ;   //bad since wrong number of elements
        ...
        for (int rebinCounter = 1; rebinCounter <= meanCleanRebinFactor; rebinCounter++) {
          ...
          if (nonRebinnedBinNumber < h_time_crate->GetNbinsX()) {
            if (h_time_crate_rebin->GetBinContent(bin) >= histRebin_max * meanCleanCutMinFactor)
              //B2INFO( "rebin value, rebin max value = " << h_time_crate_rebin->GetBinContent(bin) << ", " << histRebin_max ) ;
              //if ( h_time_crate_rebin->GetBinContent(bin) == histRebin_max )
            {
    ...
    //double default_mean_crate = h_time_crate->GetMean() ;
    double default_mean_crate = h_time_crate_masked->GetMean() ;
    //double default_mean_crate_unc = h_time_crate->GetMeanError() ;
    double default_mean_crate_unc = h_time_crate_masked->GetMeanError() ;
    //double default_sigma_crate = h_time_crate->GetStdDev() ;
    ...
    //h_time_crate->Fit(gaus, "LIQ") ;
    //h_time_crate_masked->Fit(gaus, "LIQ") ;
    ...
    /*
          B2INFO("Number entries = " << h_time_crate->GetEntries() ) ;
          if ( h_time_crate->GetEntries()  < 100 )
          {
             B2INFO("Number of entries = " << h_time_crate->GetEntries() << " < 100 minimum") ;
    ...
    */
    ...
    /*
          mean_crate = default_mean_crate ;
          mean_crate_unc = default_mean_crate_unc ;
          sigma_crate = default_sigma_crate ;

          crateCalibSaved = 1 ;
          database_mean_crate = mean_crate ;
          database_mean_crate_unc = mean_crate_unc ;

          mean_crate_unc = default_sigma_crate ;
          database_mean_crate_unc = sigma_crate ;
    ...
    */
    ...
    //histfile->WriteTObject(h_time_crate_mask, (std::string("h_time_psi_crate_mask") + std::to_string(crate_id)).c_str()) ;
  ...
  //std::string calibrationNameRunIndex = std::string("ECLCrystalTimeOffset") + runNumsString ;   // not debugged yet.... chris says won't work properly
  //saveCalibration(BhabhaTCalib, calibrationNameRunIndex.c_str()) ;  // parallel
  ...
  // if too few events, return c_NotEnoughData ;
