...
void ECLKaonPdf::init(const char* parametersFileName)
{
  ...
  for (unsigned int ip(0); ip < m_n_p_bins; ++ip) {
    ...
    for (unsigned int ith(0); ith < m_n_theta_bins; ++ith) {
      ...
      Parameters& k_prm = m_params[i];

      // Create RooRealVar representing the fitted observable.
      k_prm.fitrange_dn = map.param(name((chargePrefix + "kaons_fit_range_dn_").c_str(), ip, ith));
      k_prm.fitrange_up = map.param(name((chargePrefix + "kaons_fit_range_up_").c_str(), ip, ith));
      RooRealVar var("eop", "E/P (c)", k_prm.fitrange_dn, k_prm.fitrange_up);

      // Build Gaussian PDF
      k_prm.mu3      = map.param(name((chargePrefix + "kaons_mu3_").c_str(), ip, ith));
      k_prm.sigma3   = map.param(name((chargePrefix + "kaons_sigma3_").c_str(), ip, ith));

      RooRealVar gaus_mu("#mu_{g}", "mean of gaussian", k_prm.mu3);
      RooRealVar gaus_sigma("#sigma_{g}", "width of gaussian", k_prm.sigma3);
      ...
      mu_prm.mu1      = map.param(name((chargePrefix + "kaons_mu1_").c_str(), ip, ith));
      mu_prm.sigma1l  = map.param(name((chargePrefix + "kaons_sigma1l_").c_str(), ip, ith));
      mu_prm.sigma1r  = map.param(name((chargePrefix + "kaons_sigma1r_").c_str(), ip, ith));
      mu_prm.fraction = map.param(name((chargePrefix + "kaons_fraction1_").c_str(), ip, ith));
      mu_prm.mu2      = map.param(name((chargePrefix + "kaons_mu2_").c_str(), ip, ith));
      mu_prm.sigma2   = map.param(name((chargePrefix + "kaons_sigma2_").c_str(), ip, ith));
      ...
      k_prm.fraction = map.param(name((chargePrefix + "kaons_fraction2_").c_str(), ip, ith));
      RooRealVar pdf1_frac("frac_{pdf1}", "pdf1 fraction", k_prm.fraction);
