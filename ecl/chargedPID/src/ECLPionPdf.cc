...
void ECLPionPdf::init(const char* parametersFileName)
{
  ...
  for (unsigned int ip(0); ip < m_n_p_bins; ++ip) {
    ...
    for (unsigned int ith(0); ith < m_n_theta_bins; ++ith) {
      ...
      RooRealVar gaus_mu("#mu_{g}", "mean of gaussian", pi_prm.mu3);
      RooRealVar gaus_sigma("#sigma_{g}", "width of gaussian", pi_prm.sigma3);
      ...
      mu_prm.mu1      = map.param(name((chargePrefix + "pions_mu1_").c_str(), ip, ith));
      mu_prm.sigma1l  = map.param(name((chargePrefix + "pions_sigma1l_").c_str(), ip, ith));
      mu_prm.sigma1r  = map.param(name((chargePrefix + "pions_sigma1r_").c_str(), ip, ith));
      mu_prm.fraction = map.param(name((chargePrefix + "pions_fraction1_").c_str(), ip, ith));
      mu_prm.mu2      = map.param(name((chargePrefix + "pions_mu2_").c_str(), ip, ith));
      mu_prm.sigma2   = map.param(name((chargePrefix + "pions_sigma2_").c_str(), ip, ith));
