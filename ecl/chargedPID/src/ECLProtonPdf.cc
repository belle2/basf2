...
void ECLProtonPdf::init(const char* parametersFileName)
{
  ...
  for (unsigned int ip(0); ip < n_p_bins; ++ip) {
    ...
    for (unsigned int ith(0); ith < n_theta_bins; ++ith) {
      ...
      prm.mu1 = map.param(name((chargePrefix + "protons_mu1_").c_str(), ip, ith));
      prm.mu2 = map.param(name((chargePrefix + "protons_mu2_").c_str(), ip, ith));
      prm.sigma1l = map.param(name((chargePrefix + "protons_sigma1l_").c_str(), ip, ith));
      prm.sigma1r = map.param(name((chargePrefix + "protons_sigma1r_").c_str(), ip, ith));
      prm.sigma2 = map.param(name((chargePrefix + "protons_sigma2_").c_str(), ip, ith));
      prm.fraction = map.param(name((chargePrefix + "protons_fraction1_").c_str(), ip, ith));
      ...
      pi_prm.mu3 = map.param(name((chargePrefix + "protons_mu3_").c_str(), ip, ith));
      pi_prm.sigma3 = map.param(name((chargePrefix + "protons_sigma3_").c_str(), ip, ith));
      pi_prm.fraction = map.param(name((chargePrefix + "protons_fraction2_").c_str(), ip, ith));
...
double ECLProtonPdf::pdffunc(const double& eop, unsigned int i) const
{
