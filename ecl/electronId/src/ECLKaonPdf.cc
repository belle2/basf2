...
void ECLKaonPdf::init(const char* parametersFileName)
{
  ...
  for (unsigned int ip(0); ip < n_p_bins; ++ip) {
    ...
    for (unsigned int ith(0); ith < n_theta_bins; ++ith) {
      ...
      prm.mu1 = map.param(name((charge_prefix + "kaons_mu1_").c_str(), ip, ith));
      prm.mu2 = map.param(name((charge_prefix + "kaons_mu2_").c_str(), ip, ith));
      prm.sigma1l = map.param(name((charge_prefix + "kaons_sigma1l_").c_str(), ip, ith));
      prm.sigma1r = map.param(name((charge_prefix + "kaons_sigma1r_").c_str(), ip, ith));
      prm.sigma2 = map.param(name((charge_prefix + "kaons_sigma2_").c_str(), ip, ith));
      prm.fraction = map.param(name((charge_prefix + "kaons_fraction1_").c_str(), ip, ith));
      ...
      pi_prm.mu3 = map.param(name("kaons_mu3_", ip, ith));
      pi_prm.sigma3 = map.param(name("kaons_sigma3_", ip, ith));
      pi_prm.fraction = map.param(name("kaons_fraction2_", ip, ith));
