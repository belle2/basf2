...
def fit_pion_eop(**kwargs):
    ...
    frame1 = eopvar.frame(ROOT.RooFit.Title("pions: E/p fit"))
    eopdata.plotOn(frame1, ROOT.RooFit.DataError(ROOT.RooAbsData.SumW2))

    # Create PDF to fit.
    bifurgaus_mu = ROOT.RooRealVar("#mu_{bg}", "mean of bifurcated gaussian", 0.2, 0.03, 0.33)
    bifurgaus_sigmal = ROOT.RooRealVar("#sigma_{bg}_{L}", "widthL of bifurcated gaussian", 0.01, 0.007, 0.1)
    ...
    gaus0_mean = ROOT.RooRealVar("#mu_{g0}", "mean of gaussian", 0.2, 0.03, 0.33)
    ...
    gaus1_mean = ROOT.RooRealVar("#mu_{g1}", "mean of gaussian", 0.4, 0.14, 0.8)
