...
namespace Belle2 {
  namespace ECL {
                ...
                int& m_AmpFit, int& m_TimeFit, int& m_QualityFit)
    {
      ...
      if (ttrig < 0) ttrig = 0;
      if (k_16 + n16 != 16) printf("disagreement in number of the points %d %d \n", k_16, n16);
      for (i = k_16, z00 = 0; i < 16; i++) z00 += y[i];
      z0 = z00 >> kz_s;
      it0 = 48 + ((23 - ttrig) << 2);
      it_h = 191;
      it_l = 0;
      ...
      if (A2 >= A0) {
        for (iter = 0, it = it0; iter < 3;) {
          iter++;
          s1 = (*(fg31 + it * 16)); //&0xffff;
          s2 = (*(fg32 + it * 16)); //&0xffff;
          A1 = (s1 * z0);
          B1 = (s2 * z0);
          for (i = 1; i < 16; i++) {
            s1 = (*(fg31 + i + it * 16)); //&0xffff;
            s2 = (*(fg32 + i + it * 16)); //&0xffff;
            B5 = y[15 + i];
            B5 = s1 * B5;
            A1 += B5;
            B3 = y[15 + i];
          ...
          if (iter != 3) {
            ...
            T = T < -2048 ? -2048 : T;
            C1 = (*(fg33 + it * 16) * z0);
            for (i = 1; i < 16; i++) C1 += *(fg33 + i + it * 16) * y[15 + i];
            C1 += (1 << (k_c - 1));
            C1 >>= k_c;
          }
        } // for (iter...)
