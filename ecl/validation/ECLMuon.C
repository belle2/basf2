...
void ECLMuon()
{
  ...
  hMuonsFake->GetListOfFunctions()->Add(new TNamed("Description", 
    "Fake reconstructed gamma from 1000 muons")); 
  hMuonsFake->GetListOfFunctions()->Add(new TNamed("Check", 
    "Should be low"));
  hMuonsFake->GetListOfFunctions()->Add(new TNamed("Contact", 
    "ecl2ml@bpost.kek.jp")); 
