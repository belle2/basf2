...
namespace Belle2 {
  ...
  public:
    ...
    /** Set list of streaming objects from Python */
    void setStreamingObjects(const boost::python::object& pyObj)
    {
      std::vector<std::string> emptyvector;
      setStreamObjs(PyObjConvUtils::convertPythonObject(pyObj, emptyvector));
    }
    /** Set list of streaming objects */
    void setStreamObjs(std::vector<std::string> strobjs)
    {
      m_streamobjs = strobjs;
      /* for debug
      for ( auto it=m_streamobjs.begin(); it!=m_streamobjs.end(); ++ it ) {
    ...
    std::vector<std::string> getStreamObjs()
    {
      return m_streamobjs;
    }
  ...
  private:
    ...
    std::vector<std::string> m_streamobjs;  /**< objects to be streamed in Tx module */
