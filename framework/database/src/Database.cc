  ...
  void Database::nextMetadataProvider()
  {
    ...
    try {
      if (remote) {
        ...
        if (m_currentProvider.find("belle2db.sdcc.bnl.gov") != std::string::npos) {
  ...
  void Database::initialize(const EDatabaseState target)
  {
    ...
    if (m_configState == c_InitGlobaltagList) {
      ...
      if (not m_testingPayloads.empty()) {
        B2WARNING(R"(Conditions data: configured to look for temporary testing payloads from one or more local folders.
