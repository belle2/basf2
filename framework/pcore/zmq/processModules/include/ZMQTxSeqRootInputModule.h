#pragma once

#include <framework/pcore/zmq/messages/ZMQIdMessage.h>
#include <framework/pcore/zmq/processModules/ZMQTxInputModule.h>
#include <framework/pcore/zmq/processModules/DataStoreInitialization.h>
#include <framework/dataobjects/EventMetaData.h>
#include <deque>

namespace Belle2 {
  class ZMQTxSeqRootInputModule : public ZMQTxInputModule {
  public:
    ZMQTxSeqRootInputModule() :
      ZMQTxInputModule()
    {
      addParam("inputFileName", m_param_inputFileName, "File name of the file to read the events from.");
    }

    void initialize() override
    {
      ZMQTxInputModule::initialize();
      m_seqRootFile.reset(new SeqFile(m_param_inputFileName.c_str(), "r"));

      DataStoreInitialization::initializeDataStore(m_seqRootFile);
    }

  protected:
    std::unique_ptr<ZMQIdMessage> readEventToMessage(std::string& NextWorkerID) override;

  private:
    std::string m_param_inputFileName;
    std::unique_ptr<SeqFile> m_seqRootFile;

    StoreObjPtr<EventMetaData> m_eventMetaData;
  };
}