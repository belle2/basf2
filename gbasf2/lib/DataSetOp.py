...
# gBasf2datasetrep is the script that replicates a
# dataset (generated by gbasf2output) from its source SE to a destination SE.
#
...
def ds_rep(entries, dstse):
    ...
    for file in entries.keys():
        ...
        if not cr_result['OK']:
            ...
            if lfn in cr_result['Value']['Successful']:
                ...
                if Suc_SEs is not None:
                    ...
                    if len(dst_ses) == 0:
                        print '%s already has replicas on every available SE,\
                              skip it' % lfn
                    else:
                        if dstse == '':
                            myse = dst_ses[0]
                            ...
                            if not dstse in dst_ses:
                                myse = dst_ses[0]
                                print '<%s> already has a replica, select a\
                                      different SE <%s> instead' % (dstse,
                                                                    myse)
                            else:
                                myse = dstse
                    ...
                    if no < max_no:
                        ...
                        for i in range(0, need_no):
                            ...
                            cr_result = repman.replicateAndRegister(lfn,
                                                                    dst_ses[i])
                            if not cr_result['OK']:
                                print 'Failed to replicate %s on %s'\
                                      % (lfn, dst_ses[i])
                                DIRAC.exit(1)
                            else:
    ...
    if aclient.checkDirectory(ds_path):
        ...
        for outfile in glob.glob('*.root'):
            ...
            if aclient.checkEntry(lfn):
                ...
                cr_result = eval(o)
                if lfn.replace('belle2', 'belle') in cr_result['Successful']:
                    print lfn, ' already exists in LFC, skip uploading, but \
                          register to AMGA...'
...
def ds_cmp_file_csum(lfn, local_file):
    ...
    if s == 0:
        cr_result = eval(o)
        if lfn in cr_result['Successful']:
