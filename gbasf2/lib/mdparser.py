...
#
# $Id: mdparser.py,v 1.3 2006/03/22 16:38:16 koblitz Exp $
#
...
class MDParser:
    ...
    def __isLiteral(self, start, literal):
        if self.__qp + len(literal) <= len(self.q) \
            ...
            and (len(self.q) > self.__qp + len(literal) and self.q[self.__qp
                 + len(literal)].isspace() or len(self.q) == self.__qp
                 + len(literal)):
