...
##############################################################################
# This steering file which shows all usage options for the vertex gun module
# in the generators package. The generated particles from the vertex gun are
# then fed through a full Geant4 simulation and the output is stored in a root
# file.
# The vertex gun is similar to the particle gun but instead creating either
# one vertex where all particle in a event come from or creating tracks that
# all come from different points it can create several vertices per event.
# The user can specify the number of vertices and the exact properties of all
# particles coming from each vertex. Most parameters have the same or similar
# meaning than the particle gun's parameters but have to be state for all
...
# ============================================================================
# Setting the number of tracks independently for every vertex in one event.
# The length of this list determines the number of vertices per event
# this number can be any list of ints >= 1
# default is [2,2]
...
# list.
# In this example the 4 first elements will only be used by the first vertex.
# Every track generated by at this vertex will get one of the pdg codes
# randomly assigned
...
# properties
# for every vertex individually so all parmaters are list with the same
# length
...
# momentumParams list of the particle gun. The same scheme applies to all
# the other Param1, Param2
