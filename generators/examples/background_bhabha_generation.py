...
def add_cut(name, minParticles, maxParticles, minTheta, maxTheta=None):
    """Ad a generator level cut and kill the event if the cut is not passed.  In
    ...
    selection = main.add_module("GeneratorPreselection", applyInCMS=True, nChargedMin=minParticles, nChargedMax=maxParticles,
                                MinChargedTheta=minTheta, MaxChargedTheta=maxTheta)
    selection.if_value("!=11", kill)
    selection.set_name("generator cut: " + name)


if generator == "bbbrem":
    main.add_module("BBBremInput")
    ...
    main.add_module("BHWideInput", ScatteringAngleRangeElectron=[1, 179], ScatteringAngleRangePositron=[1, 179])
