       subroutine virtual_FSRHelSum(ME,MU,p1,p2,p3,p4,p5,
...
c************************************************************************************
     ...
     -          p5mup1, p5mup2, p5mup3
       Complex*16 mup2mup3,mup1(0:3),fsr_no_emfull,C02fun,amp2,amp8,
     - amp10,ampc
       real*8 pii,alfa,amp0
       Real*8 dotrr
       Complex*16 dotrc,dotcc
      EXTERNAL dotrr,dotrc,dotcc
       Complex*16 result,resultn(12),resultb,result_counter,diag1,diag2,
     - counter1,counter3,counter8,counter95,dB0,C0count
     ...
     -            ,SC2rc,SC3rcc,SC3rrc
      ...
      Complex*32 Fs,Fq
       common/fsfq/fs,fq
       common/test_virt_no/fsr_no_emfull
c************************************************************************************
c************************************************************************************
c************************************************************************************
c************************************************************************************
...
c       print*,'C0124',C0124  
     ...
     1 Log(musq/s15)
c       print*,'C0125',C0125 ,1.d0/2.d0/MU2SQ*Log(musq/MU2SQ)  
...
c       print*,'C0245',C0245 
c       print*,'c0 count',qlI3(MU2SQ,0.d0,MU2SQ,0.d0,MU2SQ,MU2SQ,musq,-1)
c************************************************************************************
c************************************************************************************
...
c perm=!=0: 4-vert evaluated set to zero 
       ...
       result = -4*INVs23*CONJG(F(20))*hel(1)*ssav(1)+2*INVs23*CONJG(F
     -   (20))*hel(2)*ssav(2)+INVs23*CONJG(F(20))*hel(3)*ssav(3)+INVs
     -   23*CONJG(F(20))*hel(4)*ssav(3)+2*CONJG(F(15))*hel(1)*ssav(4)
     -   +CONJG(F(19))*hel(1)*ssav(4)+CONJG(F(21))*hel(1)*ssav(4)+CON
     -   JG(F(22))*hel(1)*ssav(4)+2*CONJG(F(15))*hel(2)*ssav(5)+CONJG
     -   (F(19))*hel(2)*ssav(5)+CONJG(F(21))*hel(2)*ssav(5)+CONJG(F(2
     -   2))*hel(2)*ssav(5)+4*MU**2*CONJG(F(15))*hel(3)*ssav(6)+2*MU*
     -   *2*CONJG(F(19))*hel(3)*ssav(6)+2*MU**2*CONJG(F(21))*hel(3)*s
     -   sav(6)+2*MU**2*CONJG(F(22))*hel(3)*ssav(6)+CONJG(F(15))*hel(
     -   4)*ssav(3)*ssav(7)+(CONJG(F(19))*hel(4)*ssav(3)*ssav(7))/2.+
     -   (CONJG(F(21))*hel(4)*ssav(3)*ssav(7))/2.+(CONJG(F(22))*hel(4
     -   )*ssav(3)*ssav(7))/2.+hel(1)*ssav(8)+hel(2)*ssav(9)+hel(3)*s
     -   sav(10)+hel(4)*ssav(11)+hel(1)*ssav(12)+hel(2)*ssav(13)+hel(
     -   3)*ssav(14)+hel(4)*ssav(15)+hel(1)*ssav(16)+hel(2)*ssav(17)+
     -   hel(3)*ssav(18)+hel(4)*ssav(19)+hel(1)*ssav(20)+hel(2)*ssav(
     -   21)+hel(3)*ssav(22)+hel(4)*ssav(23)+hel(1)*ssav(24)+hel(2)*s
     -   sav(25)+hel(3)*ssav(26)+hel(4)*ssav(27)
       endif
...
c       result =-result
     ...
     -   *CONJG(hel(3))*ssa(9))
c counter term
       diag1=-(4*hel(1)*ssav(1)-2*hel(2)*ssav(2)+
     - hel(3)*ssav(4)+hel(4)*ssav(4))
       diag2=-(hel(1)*ssav(4)+hel(2)*ssav(5)+2*MU**2*hel(3)*ssav(6)
...
c       print*,'-----------------------------------------------------'
c       print*, 'C0=',C0count,
c     - qlI3(MU2SQ,0.d0,MU2SQ,0.d0,MU2SQ,MU2SQ,musq,0)
       counter95=-CONJG(INVs23*INvs45MU**2*(A02+4.d0*MU2SQ*dB0*
     - (MU2SQ-s45)-2.d0*B012*(MU2SQ+s45)))*diag2
       counter3=-2.d0*CONJG(INVs23*INvs45MU*(B045
     - -2.d0*MU2SQ*C0count))*diag2
       counter8=-2.d0*CONJG(INVs23*INvs45MU*(B045
     - -2.d0*MU2SQ*C0count))*diag2
       counter1=CONJG(INVs23*(-2.d0*B045+B012+
     - 2.d0*MU2SQ*(dB0+2.d0*C0count)))*diag1
       result_counter=counter1+counter3+counter8+counter95
c       print*, '1+3+8+95=',result_counter
