...
if __name__ == "__main__":
    all_good = True
    for run_type, hlt_mode in itertools.product(
            constants.RunTypes, constants.SoftwareTriggerModes):
        basf2.B2INFO(f"Checking processing path for {run_type} in {hlt_mode} HLT mode")
        path = basf2.Path()
        add_hlt_processing(path, run_type=run_type, softwaretrigger_mode=hlt_mode)
        basf2.print_path(path)
        if not check_path(path):
            basf2.B2ERROR(f"Problems found for {run_type} in {hlt_mode} HLT mode")
            all_good = False
