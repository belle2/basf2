...
class EventInspectorLookback(basf2.Module):
    ...
    def __init__(self, exp, run, histName, pdfName, mode, window):
        ...
        print("Mode = {}  start = {}  end = {}  step = {}".format(mode, window[0], window[1], window[2]))
    ...
    def initialize(self):
        ...
        for window in range(self.windowMinValue, self.windowMaxValue+1, self.windowStepValue):
            label = "mappedRPCTimeCalByWindow{:04x}".format(window)
            title = "{}RPC time distribution for lookback-window {} = {:04x}".format(
        ...
        self.dict_nHit2ds = {}
        for window in range(self.windowMinValue, self.windowMaxValue+1, self.windowStepValue):
            label = "occupancyXYByWindow{:04x}".format(window)
            title = "{}Forward xy occupancy for lookback-window {} = {:04x};x(cm);y(cm)".format(expRun, self.windowMode, window)
    ...
    def terminate(self):
        ...
        hist_nEvents = ROOT.TH1F('nEvents',
                                 'Number of events;Lookback-window {} value'.format(self.windowMode),
        ...
        canvas.Print(self.pdfName, "Title:{}".format(hist_nEvents.GetName()))
        hist_nRawKLMs = ROOT.TH1F('nRawKLMs',
                                  'Mean number of RawKLM hits per event;Lookback-window {} value'.format(self.windowMode),
                                  nWindows, self.windowMinValue, self.windowMaxValue+self.windowStepValue)
        ...
        canvas.Print(self.pdfName, "Title:{}".format(hist_nRawKLMs.GetName()))
        hist_nHit2ds = ROOT.TH1F('nKLMHit2ds',
                                 'Mean number of KLMHit2ds per event;Lookback-window {} value'.format(self.windowMode),
                                 nWindows, self.windowMinValue, self.windowMaxValue+self.windowStepValue)
        ...
        canvas.Print(self.pdfName, "Title:{}".format(hist_nHit2ds.GetName()))
        ...
        for key in self.dict_mappedRPCTimeCalByWindow:
            ...
            theHist.Draw()
            canvas.Print(self.pdfName, "Title:{}".format(theHist.GetName()))

        self.hist_occupancyBackwardXY.Draw("colz")
        canvas.Print(self.pdfName, "Title:{}".format(self.hist_occupancyBackwardXY.GetName()))
        self.hist_occupancyForwardXY.Draw("colz")
        canvas.Print(self.pdfName, "Title:{}".format(self.hist_occupancyForwardXY.GetName()))
        for key in self.dict_occupancyXYByWindow:
            theHist = self.dict_occupancyXYByWindow[key]
            theHist.Draw("colz")
            lastTitle = "Title:{}".format(theHist.GetName())
