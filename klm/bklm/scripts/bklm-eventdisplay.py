...
if options.eNumber != '':
    ...
    if not exp.isdecimal():
        print('Input filename experiment number({}) is not valid'.format(exp))
        sys.exit()
if options.rNumber != '':
    ...
    if not run.isdecimal():
        print('Input filename run number({}) is not valid'.format(run))
        sys.exit()
if len(inputName) == 0:
...
eventPdfName = 'bklmEvents-e{}r{}.pdf'.format(exp, run)
