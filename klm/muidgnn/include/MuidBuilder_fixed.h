...
namespace Belle2 {
  ...
  public:
     ...
     */
    explicit MuidBuilder_fixed(int pdg);
    ...
    /**
     * Return the efficiency of layers that did not observed a hit while it is extrapolated.
     * Returns efficiency if no hit is observed.
     * Returns 2 if hit is observed
     * Returns -1 if not extrapolated.
     * @param[in] muid KLMMuidLikelihood dataobject.
     * @param[in] region of KLM. region = 0 for FWD, 1 for Barrel, 2 for BWD.
     * @param[in] layer number.
     */
    double getUnhitLayerEfficiency(const KLMMuidLikelihood* muid, int region, int layer) const;

    /**
     * Return the PDF of the specific layer under certain hypothesis.
     * Returns -1 if failed checkExtrapolationOutcome.
     * @param[in] hypothesis Hypothesis number.
     * @param[in] muid KLMMuidLikelihood dataobject.
     * @param[in] layer number.
     */
    double getLayerPDF(const KLMMuidLikelihood* muid, int layer) const;
  ...
  private:
     ...
     * Copy assignment.
     */
    MuidBuilder_fixed& operator=(const MuidBuilder_fixed&);
