...
/* Own header. */
// #include <klm/muid/MuidBuilder_fixed.h>
...
MuidBuilder_fixed::MuidBuilder_fixed() : m_ReducedChiSquaredDx(0.0)
{
...
MuidBuilder_fixed::MuidBuilder_fixed(int pdg) : m_ReducedChiSquaredDx(0.0)
{
...
MuidBuilder_fixed::~MuidBuilder_fixed()
{
}

void MuidBuilder_fixed::fillPDFs(MuidElementNumbers::Hypothesis hypothesis)
{
  for (int outcome = 1; outcome <= MuidElementNumbers::getMaximalOutcome(); ++outcome) {
    for (int lastLayer = 0; lastLayer <= MuidElementNumbers::getMaximalBarrelLayer(); ++lastLayer) {
...
double MuidBuilder_fixed::getPDF(const KLMMuidLikelihood* muid) const
{
  return getLongitudinalPDF(muid) * getTransversePDF(muid);
}

double MuidBuilder_fixed::getLongitudinalPDF(const KLMMuidLikelihood* muid) const
{
...
double MuidBuilder_fixed::getTransversePDF(const KLMMuidLikelihood* muid) const
{
...
double MuidBuilder_fixed::getUnhitLayerEfficiency(const KLMMuidLikelihood* muid, int region, int layer) const
{
  /* Setup the main ingredients for the calculation. */
  unsigned int outcome = muid->getOutcome();
  if ((outcome <= MuidElementNumbers::c_NotReached) || (outcome > MuidElementNumbers::getMaximalOutcome()))
  ...
  switch (region) {
    case 0: // forward-endcap
      if (!muid->getIsForward()) {return -1.;}
      maxLayer =  MuidElementNumbers::getMaximalEndcapForwardLayer();
      testBit <<= (MuidElementNumbers::getMaximalBarrelLayer() + 1);
      testBit <<= layer;
      if ((testBit & extLayerPattern) != 0) {
        if ((testBit & hitLayerPattern) != 0) { /* Checking the presence of a hit in the layer. */
          return 2.;
          ...
          if ((layer == 0) || (layer == maxLayer) || (layer < endcapExtLayer)) {
            //std::cout << "FWD layer "<< layer << " effi=" << muid->getExtEKLMEfficiencyValue(layer) << std::endl;
            return muid->getExtEKLMEfficiencyValue(layer);
          }
        }
      } else {return -1.;} // if not extrapolated
      break;
    case 1: // barrel
      testBit <<= layer;
      if ((testBit & extLayerPattern) != 0) { /*if extrapolated*/
        if ((testBit & hitLayerPattern) != 0) { /* Checking the presence of a hit in the layer. */
          return 2.;
        } else {
          if (((layer == 0) && (outcome < MuidElementNumbers::c_CrossBarrelStopInForwardMin))
              || (layer == MuidElementNumbers::getMaximalBarrelLayer()) || (layer < barrelExtLayer)) {
            //std::cout << "Barrel layer" << layer << " effi=" << muid->getExtBKLMEfficiencyValue(layer) << std::endl;
            return  muid->getExtBKLMEfficiencyValue(layer);
          }
        }
      } else {return -1.;} // if not extrapolated
      break;
    case 2: // backward-endcap
      if (muid->getIsForward()) {return -1.;}
      maxLayer =  MuidElementNumbers::getMaximalEndcapBackwardLayer();
      testBit <<= (MuidElementNumbers::getMaximalBarrelLayer() + 1);
      testBit <<= layer;
      if ((testBit & extLayerPattern) != 0) {
        if ((testBit & hitLayerPattern) != 0) { /* Checking the presence of a hit in the layer. */
          ...
          if ((layer == 0) || (layer == maxLayer) || (layer < endcapExtLayer)) {
            //std::cout << "BWD layer "<< layer << " effi=" << muid->getExtEKLMEfficiencyValue(layer) << std::endl;
            return muid->getExtEKLMEfficiencyValue(layer);
          }
        }
      } else {return -1.;} // if not extrapolated
