...
namespace Belle2 {
  ...
  public:
    ...
    // TODO FIXME: Set maximum layer number (for now set to 7) depending on XML or GT information or anything else to
    //             have it correct depending on the current setup
    /** Getter for number of clusters in a specific VTX layer.
     *
     *  All clusters, that have been part of a RecoTrack, that was converted into a Track, are removed.
     *  Only clusters, that in principle are considered valid, e.g. that are compatible with the event T0
     *  are counted. The maximum number of clusters per layer and direction, that are considered in the VTX
     *  is 255.
     *  @param layer  1 to 7 for respective VTX layer for which you want to have the remaining clusters.
     */
    uint16_t getNVTXClustersInLayer(uint16_t const layer)
    {
      if (layer >= 1 and layer <= 7) {
        return m_nVTXClusters[layer - 1];
      }
      B2FATAL("The VTX only has layer 1 up to 7 (or 5), but you asked for " << layer);
    }

    /** Setter for number of clusters in specific VTX layer */
    void setNVTXClustersInLayer(uint16_t const layer, uint16_t const nClusters)
    {
      if (layer >= 1 and layer <= 7) {
        m_nVTXClusters[layer - 1] = nClusters;
        return;
      }
      B2FATAL("The VTX only has layer 1 up to 7 (or 5), but you asked for " << layer);
    }
    ...
    /** Getter for flag indicating that the VTX CKF was aborted due to high combinatorics in the event. */
    bool hasVTXCKFAbortionFlag() const
    {
      return m_flagBlock[c_vtxckfAbortion];
    }

    /** Setter for flag indicating that the VTX CKF was aborted due to high combinatorics in the event. */
    void setVTXCKFAbortionFlag()
    {
      m_flagBlock.set(c_vtxckfAbortion);
    }
  ...
  private:
     ...
     *  and therefore sufficient.
     */
    uint16_t m_nVTXClusters[7] {0, 0, 0, 0, 0, 0, 0};
