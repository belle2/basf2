...
namespace Belle2 {
   ...
   *
   * Only hits from the outgoing arm are stored. VXD hits may originate from subdetectors PXD, SVD.
   * For each layer there are four bits. The layer counting is as defined in BELLE2-NOTE-TE-2015-010.
   *
   * Support for upgrade vertex detector with up to 15 pixel layers is added. The HitPatterVXD supports now two
   * modes for PXD+SVD and VTX to encode hit patterns. The default mode is PXD+SVD but it switches when adding
   * a VTX hit. The previously unused bit 31 is used to encode the mode.
   *
   * @sa HitPatternCDC
   *
  ...
  class HitPatternVXD {
    ...
    friend class HitPatternVXDTest_VTXSetterAndGetter_Test;
  ...
  public:
    ...
    /**
     * Set the number of hits in a specific layer of the VTX.
     * @param layerId VTX layer index.
     * @param nHits 0-3 hits possible
     */
    void setVTXLayer(const unsigned short layerId, unsigned short nHits);

    /**
     * Get the number of hits in a specific layer of the VTX.
     * @param layerId VTX layer index.
     * @return Number of hits in the VTX layer.
     */
    unsigned short getVTXLayer(const unsigned short layerId) const;

    /**
     * Reset the number of hits in a specific layer of the VTX.
     * @param layerId VTX layer index.
     */
    void resetVTXLayer(const unsigned short layerId);

    /**
     * Get total number of hits in the VTX.
     * @return Total number of hits in the VTX.
     */
    unsigned short getNVTXHits() const;

    /**
     * Returns the total number of activated layers of the VTX.
     * @return Total number of activated VTX layers.
     */
    unsigned short getNVTXLayers() const;

    /**
     * Returns the first activated layer of the VTX.
     * If there is no hit in the VTX pattern, -1 is returned.
     * @return Index of first layer.
     */
    short getFirstVTXLayer() const;

    /** Returns the last activated layer of the VTX.
     * If there is no hit in the VTX pattern, -1 is returned.
     * @return Index of last layer.
     */
    short getLastVTXLayer() const;
  ...
  private:
     ...
     * VTX has in each layer 2 bits.
     * If bit 31 is set, layers 1-15 are VTX layers otherwise layers 1-2 are PXD and layers 3-6 are SVD. Bit 31 is unset
     * by default but will be set by calling setVTXLayer() and can be tested by testVTXMode()
     */
    std::bitset<32> m_pattern;

    static const std::bitset<32> s_LayerMasks[6]; /**<  Masks to zero out all bits from other layers for PXD+SVD. */
    ...
    static const std::bitset<32> s_VTXLayerMasks[15]; /**<  Masks to zero out all bits from other layers for VTX. */
    ...
    static const std::list<unsigned short> s_VTXLayerNumbers;  /**< For iterating through layers. */
    ...
    /** Getter for the proper layer masks for VTX. Makes index handling more easy. */
    std::bitset<32> getVTXLayerMask(const unsigned short layerId) const { return HitPatternVXD::s_VTXLayerMasks[layerId - 1]; }

    /** Setter for VTX mode. */
    void setVTXMode() {m_pattern.set(31); }

    /** Getter for VTX mode. */
    bool testVTXMode() const { return m_pattern.test(31); }
