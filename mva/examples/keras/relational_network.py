...
class Relations(Layer):
    ...
    def __init__(self, number_groups, number_features, hidden_feature_shape=[30, 30], activation=tanh, **kwargs):
    ...
    def build(self, input_shape):
        """
        Build all weights for Relational Layer
        ...
        self.feature_len = input_shape[1] // self.number_groups

        dense_shape = [2 * self.feature_len] + self.hidden_feature_shape + [1]
    ...
    def call(self, inputs):
        ...
        """
        input_groups = [inputs[:, i * self.feature_len:(i + 1) * self.feature_len] for i in range(self.number_groups)]
        outputs = []
    ...
    def compute_output_shape(self, input_shape):
        ...
        combinations = np.int32(np.math.factorial(self.number_groups) / (2 * np.math.factorial(self.number_groups - 2)))

        return (input_shape[0], combinations * self.number_features)
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as path:
        ...
        specific_options.m_config = json.dumps({'use_relations': True, 'use_feed_forward': False,
        ...
        specific_options.m_config = json.dumps({'use_relations': False, 'use_feed_forward': True,
                                                'number_groups': number_total_lines, 'number_features': 3})
        basf2_mva.teacher(general_options, specific_options)
