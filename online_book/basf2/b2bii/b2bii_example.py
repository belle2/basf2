...
# respectively.
ma.fillParticleList('pi+:all', 'atcPIDBelle(3,2)<0.4', path=main)
ma.fillParticleList('K+:all', 'atcPIDBelle(3,2)>0.6', path=main)
...
ma.matchMCTruth('pi0:mdst', path=main)
...
ma.reconstructDecay('D0:Kpipi0 -> K-:all pi+:all pi0:mdst', cut='1.7 < M < 2.0', path=main)
ma.reconstructDecay('B+:D0pi -> anti-D0:Kpipi0 pi+:all', cut='4.8 < Mbc >5.2 and abs(deltaE) < 0.15', path=main)
...
belle1pid = ['eIDBelle', 'muIDBelleQuality', 'muIDBelle', 'Lkpi'] + vc.track + vc.track_hits + standard_vars
...
ma.variablesToNtuple('B+:D0pi', b_vars, filename='B2BII_B2D0pi_D2Kpipi0.root', path=main)
