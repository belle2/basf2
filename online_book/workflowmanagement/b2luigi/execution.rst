...
Calling :code:`python3 main.py --batch` on KEKcc will the trigger the full workflow execution. b2luigi will build the dependency tree for the :code:`Plot` task and execute only the required tasks for which no output files are existing in the given output directory.  Do not forget to setup basf2 beforehand, for the recommended release use :code:`b2setup $(b2help-releases)`. Remember that the reconstruction task is the only task not marked as :code:`local` and will therefore be submitted to the KEKcc batch system. 
