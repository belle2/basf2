...
The :code:`SkimTask` task again employs `b2luigi.basf2_helper.tasks.Basf2PathTask` to run a minimal steering script on the specified datasets and has a number of parameters specific to gbasf2, such as :code:`batch_system = "gbasf2"`. :doc:`b2luigi:index` will automatically setup a proxy, reschedule failed gbasf2 jobs a maximum of :code:`gbasf2_max_retries` times, download the skims and check for their completeness. Make sure that you are using the latest release of :doc:`b2luigi:index`, which may not be on the python package index.
