...
#include <pxd/calibration/PXDLandauGainCalibrationAlgorithm.h>
...
// Anonymous namespace for data objects used by PXDLandauGainCalibrationAlgorithm class
namespace {
...
PXDLandauGainCalibrationAlgorithm::PXDLandauGainCalibrationAlgorithm():
...
CalibrationAlgorithm::EResult PXDLandauGainCalibrationAlgorithm::calibrate()
{
  ...
  for (auto histoBin = 1; histoBin <= cluster_counter->GetXaxis()->GetNbins(); histoBin++) {
    ...
    if (numberOfHits >= minClusters) {
      ...
      // Store the gain
      //cout << "gainMapPar->setContent" <<  sensorID.getID() <<  uBin << vBin << gain;
...
double PXDLandauGainCalibrationAlgorithm::EstimateGain(VxdID sensorID, unsigned short uBin, unsigned short vBin)
{
  ...
  const std::string treename = str(format("tree_%1%_%2%_%3%_%4%_%5%") % layerNumber % ladderNumber % sensorNumber % uBin % vBin);
  const std::string histname_s = str(format("hist_%1%_%2%_%3%_%4%_%5%") % layerNumber % ladderNumber % sensorNumber % uBin % vBin);
  ...
  auto dataMPV = GetChargeMPVFromDB(sensorID, uBin, vBin);
  auto mcMPV = FitLandau(mc_signals, histname);

  double gain =  dataMPV / mcMPV;
  if (gain <= 0) {
...
double PXDLandauGainCalibrationAlgorithm::GetChargeMPVFromDB(VxdID sensorID, unsigned short uBin, unsigned short vBin)
{
...
double PXDLandauGainCalibrationAlgorithm::FitLandau(vector<double>& signals, const char* histname)
{
...
// save output signals and fit in root file for inspection
    TFile* f1 = new TFile("mc_signals.root", "UPDATE");
    f1->cd();
    hist_signals->Write();
    f1->Close();
