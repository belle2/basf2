...
class HeadTailShapeClassifierTrainer(ShapeClassifierTrainer):
    """Class for training of HeadTailShapeClassifier instances on track data"""
    ...
    def createShapeClassifier(self, Data, thetaU, thetaV):
        ...
        shape_classifier = HeadTailShapeClassifier()
        ...
        if alltracks == 0:
            print('Added head-tail shape classifier with coverage {:.2f}% on training data sample.'.format(0.0))
        ...
        dlabelfunc = np.vectorize(get_short_digital_label)
        dlabels, counts = np.unique(dlabelfunc(shapes, thetaU, thetaV), return_counts=True)
        ...
        for i, dlabel in enumerate(dlabels):
            ...
            it = np.where(dlabelfunc(shapes, thetaU, thetaV) == dlabel)
            DData = Data[it]
        ...
        print('Added head-tail shape classifier with coverage {:.2f}% on training data sample.'.format(100 * coverage))
    ...
    def subdivide_data(self, DData, dlabel, thetaU, thetaV, telcov, alltracks):
        ...
        tmp_classifier = HeadTailShapeClassifier()
...
class HeadTailShapeClassifier(ShapeClassifier):
    """
    The HeadTailShapeClassifier classifies shapes taking into account
    the position and signals of the cell where particle entered and left
    the sensor.
    """
    ...
    def getLabel(self, shape, thetaU, thetaV):
        ...
        dlabel = get_short_digital_label(shape, thetaU, thetaV)
    ...
    def setPercentiles(self, dlabel, percentiles):
        """ Set array with percentile bins for digital label"""
        self.percentile_dict[dlabel] = percentiles

    def getPercentiles(self, dlabel):
        """ Return array with percentile bins for digital label"""
