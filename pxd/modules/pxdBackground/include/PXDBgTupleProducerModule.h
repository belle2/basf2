...
namespace Belle2 {
  ...
  namespace PXD {
    ...
    inline int PXDBgTupleProducerModule::getRegionID(int uBin, int vBin) const
    {
      return uBin * m_nBinsV + vBin;
    }

    inline const PXD::SensorInfo& PXDBgTupleProducerModule::getInfo(const VxdID& sensorID)
    {
      return dynamic_cast<const PXD::SensorInfo&>(VXD::GeoCache::getInstance().getSensorInfo(sensorID));
    }

    inline  double PXDBgTupleProducerModule::getSensorArea(VxdID sensorID)
    {
      const PXD::SensorInfo& info = getInfo(sensorID);
      return info.getWidth() * info.getLength();
    }

    inline  double PXDBgTupleProducerModule::getRegionArea(VxdID sensorID, int vBin) const
    {
      const PXD::SensorInfo& info = getInfo(sensorID);
