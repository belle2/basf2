  ...
  , m_pxd2TrackEvent(), m_signal(0), m_estimated(0.0), m_run(0), m_exp(0)
{
  ...
  addParam("minPBetaSinTheta3o2", m_minPBetaSinTheta3o2, "Minimum p*Beta*sinn(theta_0)^{3/2}", float(2));
...
void PXDCDSTCollectorModule::collectFromTrack(const PXD2TrackEvent::baseType& track)
{
  ...
  for (auto& trackCluster : track.trackClusters) {
    ...
    if (cluster.pxdID <= 0 || m_estimated <= 0)
    ...
    // Fill effciency
    if (m_selectedEff) {
      auto phi = atan2(intersection.y, intersection.x);
