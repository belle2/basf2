...
void PXDInjectionDQMModule::beginRun()
{
  ...
  for (auto& it : hOccModAfterInjLER) if (it.second) it.second->Reset();
  for (auto& it : hOccModAfterInjHER) if (it.second) it.second->Reset();
  for (auto& it : hEOccModAfterInjLER) if (it.second) it.second->Reset();
  for (auto& it : hEOccModAfterInjHER) if (it.second) it.second->Reset();
  for (auto& it : hMaxOccModAfterInjLER) if (it.second) it.second->Reset();
  for (auto& it : hMaxOccModAfterInjHER) if (it.second) it.second->Reset();
}
