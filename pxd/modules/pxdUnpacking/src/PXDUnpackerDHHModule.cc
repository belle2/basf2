...
void PXDUnpackerDHHModule::terminate()
{
  ...
  if (flag != 0) {
    B2RESULT("PXD DHH Unpacker --> Error Statistics (counted once per event!) in Events: " << m_unpackedEventsCount);
    B2RESULT(errstr + " )");
    ...
    B2RESULT("PXD DHH Unpacker --> No Error found in Events: " << m_unpackedEventsCount);
  }
...
void PXDUnpackerDHHModule::event()
{
  ...
  if (verbose) {
    B2DEBUG(20, "PXD DHH Unpacker --> RawDHH Objects in event: " << nRaws);
  };
  ...
  for (auto& it : m_storeRawDHH) {
    if (verbose) {
      B2DEBUG(20, "PXD DHH Unpacker --> Unpack Objects: ");
    };
    unpack_rawdhh(it, inx++);
  }
...
void PXDUnpackerDHHModule::unpack_rawdhh(RawDHH& px, int inx)
{
  ...
  while (datafullsize - ll >= 16) {
    ...
    if (lo & 0x3) {
      ...
      B2DEBUG(20, "unpack DHE(C) frame: " << j << " with size $" << std::hex << lo << " at byte offset " << std::hex << ll);
      endian_swapper((char*)dataptr, lo);
      unpack_dhc_frame((char*)dataptr, lo, j, 256, daqpktstat);
    }
    ll += lo;
    dataptr += lo / 4;
    j++;// framecounter
...
int PXDUnpackerDHHModule::nr5bits(int i)
{
