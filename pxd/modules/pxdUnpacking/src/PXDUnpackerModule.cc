...
void PXDUnpackerModule::beginRun()
{
  ...
  for (int j = 0; j < Frames_in_event; j++) {
    ...
    if (lo & 0x3) {
      ...
      if (m_firmware > 0 && m_firmware < 10) {
        ...
        unpack_dhc_frame_v01(ll + reinterpret_cast<char*>(dataptr), lo, j, Frames_in_event, daqpktstat);
      } else if (m_firmware >= 10) {
        /// 10-- is used for new firmware
        unpack_dhc_frame_v10(ll + reinterpret_cast<char*>(dataptr), lo, j, Frames_in_event, daqpktstat);
      } else {
...
void PXDUnpackerModule::unpack_dhp_v01(void* data, unsigned int frame_len, unsigned int dhe_first_readout_frame_id_lo,
...
void PXDUnpackerModule::unpack_dhp_v10(void* data, unsigned int frame_len, unsigned int dhe_first_readout_frame_id_lo,
                                       unsigned int dhe_ID, unsigned dhe_DHPport, unsigned dhe_reformat, VxdID vxd_id,
                                       PXDDAQPacketStatus& daqpktstat)
{
  unsigned int nr_words = frame_len / 2; // frame_len in bytes (excl. CRC)!!!
...
void PXDUnpackerModule::unpack_dhc_frame_v01(void* data, const int len, const int Frame_Number, const int Frames_in_event,
                                             PXDDAQPacketStatus& daqpktstat)
{
...
void PXDUnpackerModule::unpack_dhc_frame_v10(void* data, const int len, const int Frame_Number, const int Frames_in_event,
                                             PXDDAQPacketStatus& daqpktstat)
{
  /// The following STATIC variables are used to save some state or count some things
  /// while depacking the frames. they are in most cases (re)set on the first frame or ONSEN trg frame
