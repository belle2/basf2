...
void PXDUnpackerNewModule::unpack_fce(unsigned short* data, unsigned int length, VxdID vxd_id)
{
...
void PXDUnpackerNewModule::unpack_dhp(void* data, unsigned int frame_len, unsigned int dhe_first_readout_frame_id_lo,
...
void PXDUnpackerNewModule::unpack_dhc_frame(void* data, const int len, const int Frame_Number, const int Frames_in_event,
                                            PXDDAQPacketStatus& daqpktstat)
{
      ...
      if (!m_doNotStore) {
        ...
        if (len >= dhc.data_onsen_roi_frame->getMinSize()) {
          if ((len - dhc.data_onsen_roi_frame->getMinSize()) % 8 != 0) {
            PXDUnpackerNewModule::dump_roi(data, len - 4); // minus CRC
          }
          unsigned int l;
