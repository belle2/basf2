...
namespace Belle2 {
  ...
  namespace PXD {
    ...
    public:
      ...
      explicit Pixel(unsigned int index = 0):
        m_charge(0), m_index(index), m_u(0), m_v(0) {}
      /** Construct from a given PXDDigit and its store index */
      Pixel(const PXDDigit* digit, unsigned int index):
        m_charge(digit->getCharge()), m_index(index),
      ...
      Pixel(const PXDRawHit* rawhit, unsigned int index):
        m_charge(rawhit->getCharge()), m_index(index),
      ...
      Pixel(unsigned int index, unsigned short u, unsigned short v, float charge):
        m_charge(charge), m_index(index), m_u(u), m_v(v) {}
