...
namespace Belle2 {
  ...
  namespace PXD {
    ...
    public:
      ...
      static void map_uv_to_rc_IF_OB(unsigned int& row_u, unsigned int& col_v,  const unsigned int& dhp_id, const  unsigned int dhe_ID);

      /** Maps U/V cell of inner backward (IB) and outer forward (OF) modules of the PXD to row/col/dhpid */
      static void map_uv_to_rc_IB_OF(unsigned int& row_u, unsigned int& col_v,  const unsigned int& dhp_id, const  unsigned int dhe_ID);
