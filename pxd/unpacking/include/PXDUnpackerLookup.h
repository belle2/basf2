...
namespace Belle2 {
  ...
  namespace PXD {
    ...
    public:
      ...
      /** Maps  U/V cell of inner forward (IF) and outer backward (OB) modules of the PXD to row/col */
      static void map_uv_to_rc_IF_OB(unsigned int& row_u, unsigned int& col_v, unsigned int dhp_id, unsigned int dhe_ID);

      /** Maps U/V cell of inner backward (IB) and outer forward (OF) modules of the PXD to row/col */
