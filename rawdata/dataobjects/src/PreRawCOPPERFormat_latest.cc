  ...
  for (int i = 0; i < MAX_PCIE40_CH; i++) {
    ...
    if (finesse_nwords > 0) {
      ...
      temp_ctime_trgtype_footer =
        m_buffer[ offset_finesse + finesse_nwords - (static_cast<int>(SIZE_B2LFEE_TRAILER) + static_cast<int>
                                                     (SIZE_B2LHSLB_TRAILER)) + POS_TT_CTIME_B2LFEE ];
      temp_eve_footer =
        m_buffer[ offset_finesse + finesse_nwords - (static_cast<int>(SIZE_B2LFEE_TRAILER) + static_cast<int>
                                                     (SIZE_B2LHSLB_TRAILER)) + POS_CHKSUM_B2LFEE ];
...
int PreRawCOPPERFormat_latest::CheckCRC16(int n, int finesse_num)
{
  ...
  int nwords = GetFINESSENwords(n, finesse_num) - (static_cast<int>(SIZE_B2LHSLB_HEADER) + static_cast<int>
