...
ClassImp(PreRawCOPPERFormat_v0);

PreRawCOPPERFormat_v0::PreRawCOPPERFormat_v0()
{
}
...
int PreRawCOPPERFormat_v0::GetBufferPos(int n)
{
...
unsigned int PreRawCOPPERFormat_v0::CalcDriverChkSum(int n)
{
...
int PreRawCOPPERFormat_v0::GetNumFINESSEBlock(int n)
{
...
int PreRawCOPPERFormat_v0::GetDetectorNwords(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int PreRawCOPPERFormat_v0::GetFINESSENwords(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int PreRawCOPPERFormat_v0::GetOffsetFINESSE(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int* PreRawCOPPERFormat_v0::GetFINESSEBuffer(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
int* PreRawCOPPERFormat_v0::GetDetectorBuffer(int n, int finesse_num)
{
  switch (finesse_num) {
    case 0 :
...
unsigned int PreRawCOPPERFormat_v0::GetB2LFEE32bitEventNumber(int n)
{
...
unsigned int  PreRawCOPPERFormat_v0::CalcXORChecksum(int* buf, int nwords)
{
...
void PreRawCOPPERFormat_v0::CheckData(int n,
                                      ...
                                      int prev_runsubrun_no, int* cur_runsubrun_no)
{
  ...
  *cur_evenum_rawcprhdr = GetEveNo(n);
  ...
  if (*cur_evenum_rawcprhdr != evenum_feehdr) {
    sprintf(err_buf, "CORRUPTED DATA: Event # in PreRawCOPPERFormat_v0 header and FEE header is different : cprhdr 0x%x feehdr 0x%x : Exiting...\n%s %s %d\n",
  ...
  *cur_copper_ctr = GetCOPPERCounter(n);
  ...
  if (tmp_trailer.GetChksum() != xor_chksum) {
    sprintf(err_buf, "CORRUPTED DATA: PreRawCOPPERFormat_v0 checksum error : block %d : length %d eve 0x%x : Trailer chksum 0x%.8x : calcd. now 0x%.8x\n %s %s %d\n",
...
bool PreRawCOPPERFormat_v0::CheckCOPPERMagic(int n)
{
...
void PreRawCOPPERFormat_v0::CheckUtimeCtimeTRGType(int n)
{
...
double PreRawCOPPERFormat_v0::GetEventUnixTime(int n)
{
#ifndef READ_OLD_B2LFEE_FORMAT_FILE
  double retval;
#ifdef USE_B2LFEE_FORMAT_BOTH_VER1_AND_2
...
unsigned int PreRawCOPPERFormat_v0::GetB2LHeaderWord(int n, int finesse_buffer_pos)
{
  unsigned int word[4];
...
unsigned int PreRawCOPPERFormat_v0::FillTopBlockRawHeader(unsigned int m_node_id, unsigned int m_data_type,
                                                          ...
                                                          int prev_runsubrun_no, int* cur_runsubrun_no)
{
  ...
  if (m_num_nodes * m_num_events != 1) {
    char err_buf[500];
    sprintf(err_buf,
            "This function should be used for PreRawCOPPERFormat_v0 containing only one datablock, while. this object has num_nodes of %d and num_events of %d\n %s %s %d\n",
...
#ifdef USE_B2LFEE_FORMAT_BOTH_VER1_AND_2
void PreRawCOPPERFormat_v0::CheckB2LFEEHeaderVersion(int n)
{
...
#ifdef TEMP
      char err_buf[500];
      sprintf(err_buf, "PreRawCOPPERFormat_v0 contains no FINESSE data. Exiting...\n %s %s %d\n",
              __FILE__, __PRETTY_FUNCTION__, __LINE__);
      string err_str = err_buf; throw (err_str);
//       sleep(12345678);
//      exit(-1);
...
int PreRawCOPPERFormat_v0::CalcReducedDataSize(RawDataBlock* raw_datablk)
{
  return raw_datablk->TotalBufNwords();
}

void PreRawCOPPERFormat_v0::CopyReducedData(RawDataBlock* raw_datablk, int* buf_to,
                                            int malloc_flag_from)
{
  ...
  memcpy(buf_from, buf_to, raw_datablk->TotalBufNwords() * sizeof(int));
...
int PreRawCOPPERFormat_v0::CalcReducedNwords(int n)
{
  int nwords_to = 0;
...
int PreRawCOPPERFormat_v0::CopyReducedBuffer(int n, int* buf_to)
{
  int* buf_from = NULL;
  ...
  RawCOPPERFormat_v0 m_reduced_rawcpr;
...
int PreRawCOPPERFormat_v0::CheckB2LHSLBMagicWords(int* finesse_buf, int finesse_nwords)
{
