...
//ClassImp(PreRawCOPPERFormat_v2);
...
unsigned int PreRawCOPPERFormat_v2::CalcDriverChkSum(int n)
{
...
int PreRawCOPPERFormat_v2::GetDetectorNwords(int n, int finesse_num)
{
...
int PreRawCOPPERFormat_v2::GetFINESSENwords(int n, int finesse_num)
{
  if (!CheckCOPPERMagic(n)) {
    ...
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : COPPER's magic word is invalid. Exiting... Maybe it is due to data corruption or different version of the data format. : slot%c eve 0x%x exp %d run %d sub %d\n %s %s %d\n",
  ...
  switch (finesse_num) {
    ...
    default :
      ...
      GetNodeName(n, hostname, sizeof(hostname));
      sprintf(err_buf,
              "[FATAL] %s ch=%d : ERROR_EVENT : Specified FINESSE number( = %d ) is invalid. Exiting... : slot%c eve 0x%x exp %d run %d sub %d\n %s %s %d\n",
              hostname, finesse_num,
              finesse_num,
              65 + finesse_num, GetEveNo(n), GetExpNo(n), GetRunNo(n), GetSubRunNo(n),
              __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
unsigned int PreRawCOPPERFormat_v2::GetB2LFEE32bitEventNumber(int n)
{
  ...
  if (err_flag == 1) {
    ...
    GetNodeName(n, hostname, sizeof(hostname));
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : CORRUPTED DATA: Different event number over HSLBs : slot A 0x%.8x : B 0x%.8x :C 0x%.8x : D 0x%.8x : eve 0x%x exp %d run %d sub %d\n%s %s %d\n",
            hostname, -1,
...
void PreRawCOPPERFormat_v2::CheckData(int n,
                                      ...
                                      unsigned int prev_exprunsubrun_no, unsigned int* cur_exprunsubrun_no)
{
  ...
  if (!CheckCOPPERMagic(n)) {
    ...
    GetNodeName(n, hostname, sizeof(hostname));
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : Invalid Magic word 0x7FFFF0008=%u 0xFFFFFAFA=%u 0xFFFFF5F5=%u 0x7FFF0009=%u : eve 0x%x exp %d run %d sub %d\n%s %s %d\n",
            hostname, -1,
  ...
  *cur_evenum_rawcprhdr = GetEveNo(n);
  ...
  if (*cur_evenum_rawcprhdr != evenum_feehdr) {
    ...
    GetNodeName(n, hostname, sizeof(hostname));
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : Event # in PreRawCOPPERFormat_v2 header and FEE header is different : cprhdr 0x%x feehdr 0x%x : Exiting... : eve 0x%x exp %d run %d sub %d\n%s %s %d\n",
            hostname, -1,
  ...
  *cur_copper_ctr = GetCOPPERCounter(n);
  if (prev_exprunsubrun_no == *cur_exprunsubrun_no) {
    ...
    if ((unsigned int)GetRunNo(n) != (prev_exprunsubrun_no & RawHeader_v2::RUNNO_MASK) >> RawHeader_v2::RUNNO_SHIFT) {
  ...
  if (tmp_trailer.GetChksum() != xor_chksum) {
    ...
    GetNodeName(n, hostname, sizeof(hostname));
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : PreRawCOPPERFormat_v2 checksum error : block %d : length %d eve 0x%x : Trailer chksum 0x%.8x : calcd. now 0x%.8x : eve 0x%x exp %d run %d sub %d\n %s %s %d\n",
            hostname, -1,
...
bool PreRawCOPPERFormat_v2::CheckCOPPERMagic(int n)
{
...
void PreRawCOPPERFormat_v2::CheckUtimeCtimeTRGType(int n)
{
...
unsigned int PreRawCOPPERFormat_v2::FillTopBlockRawHeader(unsigned int m_node_id, unsigned int prev_eve32,
                                                          unsigned int prev_exprunsubrun_no, unsigned int* cur_exprunsubrun_no)
{
  ...
  if (m_num_nodes * m_num_events != 1) {
    char err_buf[500];
    sprintf(err_buf,
            "[FATAL] This function should be used for PreRawCOPPERFormat_v2 containing only one datablock, while. this object has num_nodes of %d and num_events of %d\n %s %s %d\n",
  ...
  m_buffer[ tmp_header.POS_VERSION_HDRNWORDS ] = static_cast<int>(tmp_header.RAWHEADER_NWORDS) & tmp_header.HDR_NWORDS_MASK;
  ...
  // Fill offset values
  //
  int offset_1st_finesse = static_cast<int>(tmp_header.RAWHEADER_NWORDS) + SIZE_COPPER_HEADER;
  ...
  m_buffer[ tmp_header.POS_EXP_RUN_NO ] = finesse_buf[ static_cast<int>(SIZE_B2LHSLB_HEADER) + POS_EXP_RUN ];


  //
  // Fill event #
  //
  unsigned int cur_ftsw_eve32 =  finesse_buf[ static_cast<int>(SIZE_B2LHSLB_HEADER) + POS_TT_TAG ];
  m_buffer[ tmp_header.POS_EVE_NO ] = cur_ftsw_eve32;

  //
  // Copy FTSW words from B2LFEE header
  //
  m_buffer[ tmp_header.POS_TTCTIME_TRGTYPE ] = finesse_buf[ static_cast<int>(SIZE_B2LHSLB_HEADER) + POS_TT_CTIME_TYPE ];
  m_buffer[ tmp_header.POS_TTUTIME ] = finesse_buf[ static_cast<int>(SIZE_B2LHSLB_HEADER) + POS_TT_UTIME ];
    ...
    if (ff55_lower_bits != 0) {
      ...
      if ((ff55_lower_bits & (1 << linkdown_bit)) != 0) {
        ...
        if (((unsigned int)m_node_id & DETECTOR_MASK) == ARICH_ID) {
          ...
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= tmp_header.B2LINK_PACKET_CRC_ERROR;
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= (unsigned int)(0x1 << 28);
          sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link packet CRC error slot A eve %8u foooter %.8x : %s %s %d\n",
                  hostname, 0,
                  cur_ftsw_eve32,
                  m_buffer[ offset_1st_finesse + copper_buf[ POS_CH_A_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                  __FILE__, __PRETTY_FUNCTION__, __LINE__);
  ...
  if (copper_buf[ POS_CH_B_DATA_LENGTH ] != 0) {
    ...
    if (ff55_higher_bits != 0xff550000) {
      char err_buf[500];
      sprintf(err_buf,
              "[FATAL] %s ch=%d : ERROR_EVENT : HSLB slotB's trailer magic word(0xff55) is invalid. : eve %8u run %d foooter %.8x : %s %s %d\n",
              hostname, 1,
              cur_ftsw_eve32, (m_buffer[ tmp_header.POS_EXP_RUN_NO ] >> 8) & 0x3FFF,
    ...
    if (ff55_lower_bits != 0) {
      ...
      //      const int packet_crcerr_bit = 8;
      if ((ff55_lower_bits & (1 << linkdown_bit)) != 0) {
        sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link down on slot B eve %8u foooter %.8x : %s %s %d\n",
                hostname, 1,
                cur_ftsw_eve32,
                m_buffer[ offset_2nd_finesse + copper_buf[ POS_CH_B_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                __FILE__, __PRETTY_FUNCTION__, __LINE__);
        ...
        if (((unsigned int)m_node_id & DETECTOR_MASK) == ARICH_ID) {
          ...
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= tmp_header.B2LINK_PACKET_CRC_ERROR;
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= (unsigned int)(0x2 << 28);
          sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link packet CRC error slot B eve %8u foooter %.8x : %s %s %d\n",
                  hostname, 1,
                  cur_ftsw_eve32,
                  m_buffer[ offset_2nd_finesse + copper_buf[ POS_CH_B_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                  __FILE__, __PRETTY_FUNCTION__, __LINE__);
  ...
  if (copper_buf[ POS_CH_C_DATA_LENGTH ] != 0) {
    ...
    if (ff55_higher_bits != 0xff550000) {
      char err_buf[500];
      sprintf(err_buf,
              "[FATAL] %s ch=%d : ERROR_EVENT : HSLB slotC's trailer magic word(0xff55) is invalid. : eve %8u run %d foooter %.8x : %s %s %d\n",
              hostname, 2,
              cur_ftsw_eve32, (m_buffer[ tmp_header.POS_EXP_RUN_NO ] >> 8) & 0x3FFF,
    ...
    if (ff55_lower_bits != 0) {
      ...
      if ((ff55_lower_bits & (1 << linkdown_bit)) != 0) {
        sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link down on slot C eve %8u foooter %.8x : %s %s %d\n",
                hostname, 2,
                cur_ftsw_eve32,
                m_buffer[ offset_3rd_finesse + copper_buf[ POS_CH_C_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                __FILE__, __PRETTY_FUNCTION__, __LINE__);
        ...
        if (((unsigned int)m_node_id & DETECTOR_MASK) == ARICH_ID) {
          ...
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= tmp_header.B2LINK_PACKET_CRC_ERROR;
          m_buffer[ tmp_header.POS_TRUNC_MASK_DATATYPE ] |= (unsigned int)(0x4 << 28);
          sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link packet CRC error slot C eve %8u foooter %.8x : %s %s %d\n",
                  hostname, 2,
                  cur_ftsw_eve32,
                  m_buffer[ offset_3rd_finesse + copper_buf[ POS_CH_C_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                  __FILE__, __PRETTY_FUNCTION__, __LINE__);
  ...
  if (copper_buf[ POS_CH_D_DATA_LENGTH ] != 0) {
    ...
    if (ff55_higher_bits != 0xff550000) {
      char err_buf[500];
      sprintf(err_buf,
              "[FATAL] %s ch=%d : ERROR_EVENT : HSLB slotD's trailer magic word(0xff55) is invalid. : eve %8u run %d foooter %.8x : %s %s %d\n",
              hostname, 3,
              cur_ftsw_eve32, (m_buffer[ tmp_header.POS_EXP_RUN_NO ] >> 8) & 0x3FFF,
    ...
    if (ff55_lower_bits != 0) {
      ...
      if ((ff55_lower_bits & (1 << linkdown_bit)) != 0) {
        sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : B2link down on slot D eve %8u foooter %.8x : %s %s %d\n",
                hostname, 3,
                cur_ftsw_eve32,
                m_buffer[ offset_4th_finesse + copper_buf[ POS_CH_D_DATA_LENGTH ] - SIZE_B2LHSLB_HEADER ],
                __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
#ifdef USE_B2LFEE_FORMAT_BOTH_VER1_AND_2
void PreRawCOPPERFormat_v2::CheckB2LFEEHeaderVersion(int n)
{
  int* temp_buf;
  for (int i = 0; i < 4; i++) {
    if (GetFINESSENwords(n, i) > 0) {
    ...
    if (i == 3) {
      ...
      char hostname[128];
      GetNodeName(n, hostname, sizeof(hostname));
      sprintf(err_buf, "[FATAL] %s ch=%d : ERROR_EVENT : PreRawCOPPERFormat_v2 contains no FINESSE data. Exiting...\n %s %s %d\n",
              hostname, i,
              __FILE__, __PRETTY_FUNCTION__, __LINE__);
      printf("%s", err_buf); fflush(stdout);
      //      string err_str = err_buf; throw (err_str);
...
//int PreRawCOPPERFormat_v2::CalcReducedDataSize(RawDataBlock* raw_datablk)
int PreRawCOPPERFormat_v2::CalcReducedDataSize(int* bufin, int nwords, int num_events, int num_nodes)
{
  //
  ...
  for (int k = 0; k < radblk_fmt.GetNumEvents(); k++) {
    ...
    for (int l = 0; l < num_nodes_in_sendblock; l++) {
      ...
      if (radblk_fmt.CheckFTSWID(entry_id) || radblk_fmt.CheckTLUID(entry_id)) {
        ...
        PreRawCOPPERFormat_v2 temp_prerawcpr;
...
//void PreRawCOPPERFormat_v2::CopyReducedData(RawDataBlock* raw_datablk, int* buf_to, int delete_flag_from)
void PreRawCOPPERFormat_v2::CopyReducedData(int* bufin, int nwords, int num_events, int num_nodes, int* buf_to, int* nwords_to)
{
  //
...
int PreRawCOPPERFormat_v2::CalcReducedNwords(int n)
{
  //
  ...
  for (int j = 0; j < 4; j++) {
    ...
    if (finesse_nwords > 0) {
      ...
      // m_reduced_rawcpr should be PostRawCOPPERFormat_v2
      //
      nwords_to +=
        finesse_nwords
...
int PreRawCOPPERFormat_v2::CopyReducedBuffer(int n, int* buf_to)
{
  //
  ...
  for (int j = 0; j < 4; j++) {
    ...
    if (GetFINESSENwords(n, j) > 0) {
      ...
      if (finesse_nwords - SIZE_B2LHSLB_HEADER - SIZE_B2LFEE_HEADER
        ...
        GetNodeName(n, hostname, sizeof(hostname));
        sprintf(err_buf,
                "[FATAL] %s ch=%d : ERROR_EVENT : Finesse buffer size is too small( %d words < %d words). May be the data are corrupted. Exiting...\n %s %s %d\n",
                hostname, j,
  ...
  *(buf_to + m_reduced_rawcpr.tmp_header.POS_OFFSET_4TH_FINESSE) = pos_nwords_finesse[ 3 ];
  ...
  if ((old_rawcopper_chksum ^ removed_xor_chksum) != new_rawcopper_chksum) {
    ...
    GetNodeName(n, hostname, sizeof(hostname));
    sprintf(err_buf,
            "[FATAL] %s ch=%d : ERROR_EVENT : RawCOPPER XOR checksum is inconsistent between before/after data reduction.(%.8x != %.8x ^ %.8x = %.8x ) Exiting...\n %s %s %d\n",
            hostname, -1,
  ...
  *(buf_to + pos_nwords_to - tmp_trailer.GetTrlNwords() + tmp_trailer.POS_CHKSUM) = new_rawcopper_chksum;
  ...
  //    post_rawcopper_v2.CheckCRC16(0, 0);
  //       printf("fROM =======================================\n");
...
int PreRawCOPPERFormat_v2::CheckB2LHSLBMagicWords(int* finesse_buf, int finesse_nwords)
{
...
int PreRawCOPPERFormat_v2::CheckCRC16(int n, int finesse_num)
{
  //
  // Calculate CRC16
  //
...
int* PreRawCOPPERFormat_v2::PackDetectorBuf(int* packed_buf_nwords,
