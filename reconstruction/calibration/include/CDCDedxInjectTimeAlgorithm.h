...
namespace Belle2 {
  ...
  public:
    ...
    */
    void correctBinBias(std::map<int, std::vector<double>>& m_varcorr, std::map<int, std::vector<double>>& m_time, TH1D*& htimes,
                        std::string svar);

    /**
    * function to store payloads after full calibration
    */
    void createPayload(std::array<double, numdedx::nrings>& m_scale);
    ...
    */
    void plotRelConstants(std::map<int, std::vector<double>>& m_meancorr, std::map<int, std::vector<double>>& m_resocorr,
                          std::array<std::vector<TH1D*>, numdedx::nrings>& htime);

    /**
    * function to final constant from merging or abs fits
    */
    void plotFinalConstants(std::array<double, numdedx::nrings>& m_scale);
  ...
  private:
    ...
    std::vector<double> vtedges; /**< external time vector*/
    std::vector<double> vtlocaledges; /**< internal time vector*/
    ...
    bool isminStat; /**< flag to merge runs for statistics thershold*/
    ...
    std::map<int, std::vector<double>> m_vresos; /**< vector for reso values*/
    std::map<int, std::vector<double>> m_vtimes; /**< vector for time values*/
    std::map<int, std::vector<double>> m_vmeanserr; /**< vector for meanerr values*/
    std::map<int, std::vector<double>> m_vresoserr; /**< vector for resoerr values*/
    std::map<int, std::vector<double>> m_vtimeserr; /**< vector for timeerr values*/
    std::vector<std::vector<double>> vinjPayload; /**< vector to store payload values*/
