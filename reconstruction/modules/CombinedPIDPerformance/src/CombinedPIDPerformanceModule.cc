...
void CombinedPIDPerformanceModule::fillEfficiencyHistos(const TrackFitResult* trackFit, const PIDLikelihood* pid, int pdg)
{
  ...
  for (unsigned int i = 0; i < detset.size(); ++i) {
    ...
    if (pidavail(pid, det) and abs(pdg) == Const::pion.getPDGCode()) {
      ...
      m_piK_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::kaon, Const::pion, det) > 0) ? true : false;
      m_Kpi_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::proton, Const::pion, det) > 0) ? true : false;
      m_ppi_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::deuteron, Const::pion, det) > 0) ? true : false;
      m_dpi_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_pi, logl_k);
      h_ROC[0][detnum]->Fill(0.0, pidval, trackFit->getMomentum().R()); // pion eff

      pidval = pidvalue(logl_k, logl_pi);
      h_ROC[1][detnum]->Fill(0.0, pidval, trackFit->getMomentum().R()); // pion faking k

      pidval = pidvalue(logl_p, logl_pi);
      h_ROC[4][detnum]->Fill(0.0, pidval, trackFit->getMomentum().R()); // pion faking proton
    }

    // fill rocs, efficiencies, and fake rates for true kaons
    if (pidavail(pid, det) and abs(pdg) == Const::kaon.getPDGCode()) {
      pass = (pid->getDeltaLogL(Const::kaon, Const::pion, det) > 0) ? true : false;
      m_Kpi_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::pion, Const::kaon, det) > 0) ? true : false;
      m_piK_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::proton, Const::kaon, det) > 0) ? true : false;
      m_pK_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_k, logl_pi);
      h_ROC[1][detnum]->Fill(1.0, pidval, trackFit->getMomentum().R()); // kaon eff

      pidval = pidvalue(logl_pi, logl_k);
      h_ROC[0][detnum]->Fill(1.0, pidval, trackFit->getMomentum().R()); // kaon faking pion

      pidval = pidvalue(logl_p, logl_k);
      h_ROC[4][detnum]->Fill(1.0, pidval, trackFit->getMomentum().R()); // kaon faking proton
    }

    // fill rocs and efficiencies for true protons
    if (pidavail(pid, det) and abs(pdg) == Const::proton.getPDGCode()) {
      pass = (pid->getDeltaLogL(Const::proton, Const::pion, det) > 0) ? true : false;
      m_ppi_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pass = (pid->getDeltaLogL(Const::proton, Const::kaon, det) > 0) ? true : false;
      m_pK_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_p, logl_pi + logl_k);
      h_ROC[4][detnum]->Fill(4.0, pidval, trackFit->getMomentum().R()); // proton eff vs. pion and kaon
    }

    // fill efficiencies for true deuterons
    if (pidavail(pid, det) and abs(pdg) == Const::deuteron.getPDGCode()) {
      pass = (pid->getDeltaLogL(Const::deuteron, Const::pion, det) > 0) ? true : false;
      m_dpi_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());
    }
    ++it;
  ...
  for (unsigned int i = 0; i < edetset.size(); ++i) {
    ...
    if (pidavail(pid, det) and abs(pdg) == Const::electron.getPDGCode()) {
      ...
      m_epi_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_e, logl_pi_e);
      h_ROC[2][detnum]->Fill(2.0, pidval, trackFit->getMomentum().R()); // electron eff vs pion
    }

    // fill rocs and fake rates for true pions
    if (pidavail(pid, det) and abs(pdg) == Const::pion.getPDGCode()) {
      pass = (pid->getDeltaLogL(Const::electron, Const::pion, det) > 0) ? true : false;
      m_epi_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_pi_e, logl_e);
      h_ROC[0][detnum]->Fill(0.0, pidval, trackFit->getMomentum().R()); // pion faking electron
    }
    ++it;
  ...
  for (it = muonSet.begin(); it != muonSet.end(); ++it) {
    ...
    if (pidavail(pid, det) and abs(pdg) == Const::muon.getPDGCode()) {
      ...
      m_mpi_Efficiencies[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_mu, logl_pi_mu);
      h_ROC[3][klm]->Fill(3.0, pidval, trackFit->getMomentum().R()); // muon eff vs. pion
    }

    // fill rocs and fake ratesfor true pions
    if (pidavail(pid, det) and abs(pdg) == Const::pion.getPDGCode()) {
      pass = (pid->getDeltaLogL(Const::muon, Const::pion, det) > 0) ? true : false;
      m_mpi_FakeRates[i]->Fill(pass, trackFit->getMomentum().R());

      pidval = pidvalue(logl_pi_mu, logl_mu);
      h_ROC[3][klm]->Fill(0.0, pidval, trackFit->getMomentum().R()); // pion faking muon
    }
