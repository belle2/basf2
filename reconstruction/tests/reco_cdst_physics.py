...
'''
Test reconstruction of physics run.
'''
...
if __name__ == '__main__':
    ...
    rawdata_files = glob.glob(os.environ['BELLE2_VALIDATION_DATA_DIR'] + '/rawdata/physics.*HLT?.*.root')
    rawdata_file = random.choice(rawdata_files)
    basf2.B2INFO(f'Running the test using {rawdata_file} as input file.')
    assert(sp.call(['basf2', steering_file, rawdata_file, 'physics']) == 0)
