...
def add_simulation(
    ...
    @param useVTX: use the VTX instead of the VXD
    """
    ...
    # VTX digitization, BG Overlay, and sorting
    if (components is None or 'VTX' in components) and useVTX:
        add_vtx_simulation(path)
        if bkgfiles is not None and bkgOverlay:
            m = path.add_module('BGOverlayExecutor', components=['VTX'])
            m.set_name('BGOverlayExecutor_VTX')
        path.add_module("VTXDigitSorter")
