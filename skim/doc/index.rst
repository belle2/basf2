...
.. tip:: If you would like to know which skims are available, please browse the :ref:`skim_physics` section of the documentation. If you would like to know which cuts are made by a particular skim, then consult the source code by clicking the ``[source]`` button on that skim in `skim_physics`_, or by navigating to ``skim/scripts/skim/WGs/<your working group name>/`` in the basf2 repository.
