...
skimCode = sef.encodeSkimName('B0toDstarRho_D0pi')
...
from skim.btocharm import B0toDstarRhoList
...
ssc.loadStdDstarPlus_D0pi_Kpi(path=path)
ssc.loadStdDstarPlus_D0pi_Kpipipi(path=path)
ssc.loadStdDstarPlus_D0pi_Kpipi0(path=path)
B0toDstarRho_list = B0toDstarRhoList(path=path)
sef.skimOutputUdst(skimCode, B0toDstarRho_list, path=path)
ma.summaryOfLists(B0toDstarRho_list, path=path)
