...
skimCode = expert.encodeSkimName('DimuonPlusMissingEnergy')

from skim import dark
dimuon_list = dark.DimuonPlusMissingEnergyList(path=dimuon_path)

expert.skimOutputUdst(skimCode, dimuon_list, path=dimuon_path)
ma.summaryOfLists(dimuon_list, path=dimuon_path)

expert.setSkimLogging(path=dimuon_path)
