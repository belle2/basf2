...
def getSkimStandaloneScript(skimName, combined=False):
    ...
    try:
        ...
        cprint(f'WARNING! Skim {skimName} registered in skim registry, but no executable found!', 'red')
...
def testExecutable(skimScript):
    """Runs the standalone script for a given skim. If it fails, the error messages from basf2 are
    printed to the screen.

    Args:
        skimScript (str): The path to the standalone skim script to be tested.
    Returns:
        executableWorks (bool): True if basf2 exited normally, and False otherwise.
    """
...
def testScripts(standaloneSkims, combinedSkims):
    """Tests executable scripts for given lists of skims, and prints a report about which failed to
    run. If any failed, exits with return code 1.
    ...
    failedScripts = [script for script in allScripts if not testExecutable(script)]

    if failedScripts:
        cprint('The following standalone executables failed:', 'red', attrs=['bold'])
...
if __name__ == '__main__':
    ...
    testScripts(['SLUntagged', 'PRsemileptonicUntagged'], [])
