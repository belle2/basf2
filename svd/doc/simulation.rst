...
Each chargelet (small group of ionized :math:`e^{-}` or :math:`h^{+}`) are drifted towards the sensor sides (:math:`e^{-}` are collected on the v/N side, :math:`h^{+}` are collected on the u/P side) in a simplified electromagnetic field (including diffusion) and then each charge is assigned to the nearest readout or floating implant. Charge sharing among implants is simulated using coupling constants measured on data and stored in :ref:`SVDChargeSimulationCalibrations<svdchargesimulation>`. Each charge :math:`Q` on a readout strip isassigned an approximate time :math:`\delta` equals to the time they reach half-way to the sensor side. The APV25 waveform is the simulated in :math:`e^{-}` for each chargelet getting to the sensor side with the following function:

.. math::

   w(t) = Q \cdot 149.012 \cdot (t-\delta)^2 \cdot (1.0 + t - \delta)^{-10}
...
The total analog APV waveform is the sum of contributions from each chargelet.

When all ionized charges are collected on the implants, the final APV waveform is digitized in ADC taking into account that a :math:`e^{-}` in Geant4 is not exactly equivalent to a real :math:`e^{-}` and therefore is weighted with :ref:`SVDChargeSimulationCalibrations<svdchargesimulation>`, and using the gain in :ref:`SVDPulseShapeCalibrations<svdpulsecal>`. Noise from :ref:`SVDNoiseCalibrations<svdnoisecal>` is superimposed to the samples. We then check that at least one of the 6 samples is above the zero-suppression threshold and we mask strips according to :ref:`SVDFADCMaskedStrips<svdmaskedcal>` and disable APVs according to the channel mapping. Finally :ref:`SVDShaperDigits<svdshapers>` are created.
