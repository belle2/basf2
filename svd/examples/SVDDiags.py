...
class Clusters2Truehits(Module):
    ...
    def event(self):
        ...
        cluster_print = lambda index, cluster: \
            'Cluster {index}: u-side {side}, position {pos}, time {time}, signal {signal}'.format(index=index,
        ...
        digit_print = lambda index, digit: \
            'Digit {index}: u-side {side}, strip {pos}, time {time}, signal {signal}'.format(index=index,
