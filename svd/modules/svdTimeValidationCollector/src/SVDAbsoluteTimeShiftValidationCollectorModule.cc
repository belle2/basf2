 ...
 **************************************************************************/
#include <svd/modules/svdTimeValidationCollector/SVDAbsoluteTimeShiftValidationCollectorModule.h>
#include <TFile.h>
using namespace std;
...
//                 Register the Module
//-----------------------------------------------------------------
REG_MODULE(SVDAbsoluteTimeShiftValidationCollector);

//-----------------------------------------------------------------
//                 Implementation
//-----------------------------------------------------------------

SVDAbsoluteTimeShiftValidationCollectorModule::SVDAbsoluteTimeShiftValidationCollectorModule() : CalibrationCollectorModule()
{
...
void SVDAbsoluteTimeShiftValidationCollectorModule::prepare()
{
  ...
  for (const auto& alg : m_timeAlgorithms) {
    TString fileName =
      TString::Format("../../SVDClusterAbsoluteTimeShifter/0/algorithm_output/algorithm_svdClusterAbsoluteTimeShifter_%s_output_rev_1.root",
                      alg.c_str());
    ...
    if (!inputFile || inputFile->IsZombie()) {
      B2ERROR("Cannot open file: " << fileName);
      continue;
    } else {
      B2INFO("Successfully opened file: " << fileName);
    }
    inputFiles[alg] = inputFile;
    // inputFiles[alg] = std::move(inputFile);

  }

}

void SVDAbsoluteTimeShiftValidationCollectorModule::startRun()
{
...
void SVDAbsoluteTimeShiftValidationCollectorModule::collect()
{
