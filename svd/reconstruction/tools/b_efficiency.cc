...
const double sideband_factor = inner / (inner + outer);
const double dilution_factor = outer / (inner + outer);

/** String for TTree::Draw to select positive cases */
TString test_string(double level)
{
  return TString(Form("(p_signal>%f)", level));
}
...
int main(int argc, char** argv)
{
  ...
  for (unsigned int iamp = 0; iamp < sn_values.size(); ++iamp) {
    ...
    for (int i = 0; i < nPoints; ++i) {
      ...
      int total_true = dilution_factor * tree->GetSelectedRows();
      ...
      // Reshuffle:
      // total_selected = true_selected + outside_selected
      // total_selected = true_selected + sideband_factor * (total_selected - true_selected)
      total_selected = true_selected + sideband_factor * (total_selected - true_selected);
      true_selected *= dilution_factor;
      ...
      cout << "Amplitude: " << amplitude << " level: " << levels[i] << ">" << level << " Total selected: " << total_selected <<
           " True selected: " << true_selected << " Total true: " << total_true << " Sens: " << 1.0 * true_selected / total_true << " Sel: " <<
           1.0 * true_selected / total_selected << endl;
      // Set labels on the plot
      if (amplitude == 20) {
        TString level_label(Form("#alpha=%4.2f", levels[i]));
  ...
  // Add line to show overall purity of the sample
  ...
  mg->Add(g_low, "L");
  // Add line to show signal-region purity of the sample
  TGraph* g_high = new TGraph(2);
  for (int i = 0; i < 2; ++i) g_high->SetPoint(i, i, dilution_factor);
  g_high->SetLineColor(kRed);
  g_high->SetLineStyle(3);
  TLatex* high_label = new TLatex(0.05, dilution_factor + 0.01, "dilution in signal area");
  high_label->SetTextSize(0.03);
  g_high->GetListOfFunctions()->Add(high_label);
  mg->Add(g_high, "L");
  mg->Draw("A");
  ...
  cCalib->Modified();
  // Save the canvas
  cCalib->SaveAs("ci_SensitivitySelectivity.png");
  char c;
