...
int main(int argc, char** argv)
{
  ...
  for (auto amplitude : sn_values) {
    ...
    tree->Draw("lr_signal/(1+lr_signal):fit_t0", critProf.Data(), "prof");
    TProfile* htemp = (TProfile*)cCalib->GetPrimitive("htemp");
    htemp->GetXaxis()->SetTitle("fitted time [ns]");
    htemp->GetYaxis()->SetTitle("L(in)/(L(in)+L(out))");
    TString histoname(Form("In/(In+Out) likelihood ratios, S/N = %2.0f", amplitude));
    htemp->SetTitle(histoname.Data());
    // Now calculate probabilities from simulation
    TString critHisto(Form("true_amp==%2.0f", amplitude));
    TString formHisto(Form("(abs(true_t0)<=%f)*1.0 + (abs(true_t0)>%f)*0.0 : fit_t0 >> hp", inner, inner));
    tree->Draw(formHisto.Data(), critHisto.Data(), "prof same");
    TH1F* h = (TH1F*)gDirectory->Get("hp");
    h->SetMarkerColor(kRed);
    h->SetLineColor(kRed);
    // Save the canvas
    TString canvasName(Form("lrCalib_%2.0f.png", amplitude));
