...
def pre_collector():
    ...
    for moda in pre_path.modules():
        ...
        if moda.name() == 'SVDSimpleClusterizer':
            ...
            moda.param("RecoDigits", 'SVDRecoDigitsFromTracks')
            moda.param("ShaperDigits", 'SVDShaperDigitsFromTracks')
            moda.param("timeAlgorithm", 2)  # changed
...
def SVDCoGTimeCalibration(files, tags, uniqueID):
    ...
    # algorithm setup
    algorithm = SVD3SampleELSTimeCalibrationAlgorithm(uniqueID)  # changed
    algorithm.setMinEntries(100)
    algorithm.setAllowedTimeShift(2.)

    # calibration setup
    calibration = Calibration('SVD3SampleELSTime',  # changed
                              collector=collector,
...
if __name__ == "__main__":
    ...
    uniqueID = "SVD3SampleELSTimeCalibrations_" + str(now.isoformat()) + "_INFO:_3rdOrderPol_TBindep_Exp" + \
