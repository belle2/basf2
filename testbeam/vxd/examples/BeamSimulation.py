...
gun_x_position = -749.9  # cm ... 100cm ... outside magnet + plastic shielding + Al scatterer (air equiv.)
...
dataFile = open('beamscan.dat')
line = dataFile.readline()  # first comment line
line = dataFile.readline()  # parameters
fields = line.split()
...
binsizex = abs(xmax - xmin) / nbinx
binsizey = abs(ymax - ymin) / nbiny

line = dataFile.readline()  # second (third from file start) comment line
nxy = nbinx * nbiny
...
for i in range(nxy):
    ...
    mom = float(fields[3])
    errmom = float(fields[4])
    phi = float(fields[5])
    errphi = float(fields[6])
    theta = float(fields[7])
    errtheta = float(fields[8])

    if density == 0:
        continue

    gunxmin = (binx - 1) * binsizex + xmin
    gunxmax = binx * binsizex + xmin
    gunymin = (biny - 1) * binsizey + ymin
    gunymax = biny * binsizey + ymin

    gun = register_module('ParticleGun')
    gun.param('nTracks', density * 10)
    gun.param('pdgCodes', [11])
    gun.param('momentumGeneration', 'normal')
    gun.param('momentumParams', [mom, errmom])
    gun.param('thetaGeneration', 'normal')
    gun.param('thetaParams', [theta, errtheta])
    gun.param('phiGeneration', 'normal')
    gun.param('phiParams', [phi, errphi])
    gun.param('vertexGeneration', 'uniform')
    gun.param('xVertexParams', [gun_x_position, gun_x_position])
    gun.param('yVertexParams', [gunymin, gunymax])
    gun.param('zVertexParams', [gunxmin, gunxmax])
    gun.param('independentVertices', True)
    gun.param('varyNTracks', True)
    main.add_module(gun)

main.add_module(simulation)

histoman = register_module('HistoManager')
histoman.param('histoFileName', 'beamcheck.root')
main.add_module(histoman)

beamscan = register_module('BeamspotScan')
beamscan.param('dataFileName', 'beamcheck.dat')
