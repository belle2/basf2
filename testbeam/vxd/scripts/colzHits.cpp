...

{
    ...
    TFile *file = new TFile("BasicSetup.root");
    ...
    if (i<2){
    Hp = new TH2F("Hp", "Primary", 200, -pxd_height, pxd_height, 200, -pxd_height, pxd_height);
    Hs = new TH2F("Hs", "Secondary",  200, -pxd_height, pxd_height, 200, -pxd_height, pxd_height);
    }else{
    Hp = new TH2F("Hp", "Primary",  200, -svd_height, svd_height, 200, -svd_height, svd_height);
    Hs = new TH2F("Hs", "Secondary", 200, -svd_height, svd_height, 200, -svd_height, svd_height);        
    }

    std::string cond1 = std::string("VXDHits.m_fromPrimary&&VXDHits.m_sensorID==" + std::string(sensors[i]));
    tree->Draw("VXDHits.m_v:VXDHits.m_u>>Hp",cond1.c_str());    
    ...
    tree->Draw("VXDHits.m_v:VXDHits.m_u>>Hs",cond2.c_str());    
    ...
    Cpsa->cd(i+1+6);
    if (i<2){
        Ha = new TH2F("Ha", "All", 200, -pxd_height, pxd_height, 200, -pxd_height, pxd_height);
    }else{
        Ha = new TH2F("Ha", "All", 200, -svd_height, svd_height, 200, -svd_height, svd_height);
    }
    std::string cond3 = std::string("VXDHits.m_sensorID==" + std::string(sensors[i]));
    tree->Draw("VXDHits.m_v:VXDHits.m_u>>Ha",cond3.c_str());    
