...

{
    ...
    double pxd_height = 2.0;
    double svd_height = 2.0;
    ...
    if (i<2){
    //Hp = new TH1F("Hp", "Primary", 200, 0, pxd_height);
    Hs = new TH1F("Hs", "Secondary", 200, 0, pxd_height);
    }else{
    //Hp = new TH1F("Hp", "Primary", 200, 0, svd_height);
    Hs = new TH1F("Hs", "Secondary", 200, 0, svd_height);        
    }

    std::string cond1 = std::string("VXDHits.m_fromPrimary&&VXDHits.m_sensorID==" + std::string(sensors[i]));
    //tree->Draw("VXDHits.m_particleMomentum>>Hp",cond1.c_str());    
    ...
    tree->Draw("VXDHits.m_particleMomentum>>Hs",cond2.c_str());    
    ...
    Cpsa->cd(i+1+6);
    if (i<2){
        Ha = new TH1F("Ha", "All", 200, 0, pxd_height);
    }else{
        Ha = new TH1F("Ha", "All", 200, 0, svd_height);
    }
    std::string cond3 = std::string("VXDHits.m_sensorID==" + std::string(sensors[i]));
    tree->Draw("VXDHits.m_particleMomentum>>Ha",cond3.c_str());    
