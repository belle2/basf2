...
WaveTimingCalibModule::~WaveTimingCalibModule() {}

void WaveTimingCalibModule::initialize()
{
...
void WaveTimingCalibModule::beginRun()
{
...
void WaveTimingCalibModule::event()
{
        ...
        for (size_t c = 0; c < hits.size(); c++) {
          ...
          if (m_isSkim) { // I want to create a skim file
            ...
            if (hits[c].adc_height < -200 || (hits[c].q > 400 && hits[c].width > 3 && hits[c].width < 8)) {
              TOPCAFDigitCalib* this_topcafdigit = m_topcafdigits_ptr.appendNew(evtwaves_ptr[w]);
...
void WaveTimingCalibModule::terminate()
{
