...
bool analyzeHits(const char* filename)
{
  ...
  TH2F* ch_time = new TH2F("ch_time", "time vs pixel (quality cuts)", 512, 0., 512., 30000, -1500., 1500.);
  ch_time->GetXaxis()->SetTitle("pixel");
  ch_time->GetYaxis()->SetTitle("hit time [time bin]");

  TH2F* ch_tdcbin = new TH2F("ch_tdcbin", "tdc bin vs channel (quality cuts)", 512, 0., 512., 1500, 0., 1500.);
  ...
  TCut qual_cut("TOPCAFDigits.m_tdc_bin<1200&&TOPCAFDigits.m_flag>0");
  TCut laser_primary_cut("TOPCAFDigits.m_time>-500&&TOPCAFDigits.m_time<-480");
