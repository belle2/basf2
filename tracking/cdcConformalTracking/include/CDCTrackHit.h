...
namespace Belle2 {
  ...
  public:
     ...
     * The index should be the index of the CDCHit in the StoreArray (and as the CDCTrackHit objects should be created in the same order it is also the index of the CDCTrackHit in its StoreArray).
     * For the moment the definition of layerID oder superlayerID differs between CDCHit and CDCTrackHit, this will be made consistent later on.
     */
    ...
    /** Returns the layerId (1 - 56). */
    int getLayerId() const {return m_layerId;};

    /** Returns the wireId (1 - 384, depends on layer). */
    int getWireId() const {return m_wireId;};

    /** Returns the superlayerId (1-9). */
    int getSuperlayerId() const {return m_superlayerId;};

    /** Returns the DriftTime (which maybe saved as DriftLenght for the moment, see CDCHit).*/
    ...
    /** Returns a pointer to the original CDCHit. */
    CDCHit * getCDCHit() const { return cdcHit; };

    /** Returns a vector with indices of Tracks, to which this Hit may belong. */
    std::vector<int> getTrackIndices() { return m_TrackIndices; }
  ...
  private:
    ...
    CDCHit *cdcHit;                    /**< Pointer to the original CDCHit, from which this CDCTrackHit was created*/

    int m_layerId;                     /**< ID of the layer of the Hit */
