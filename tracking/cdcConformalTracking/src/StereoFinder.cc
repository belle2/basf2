...
void StereoFinder::AppendStereoSegments(vector<CDCSegment> & cdcStereoSegments, string CDCTrackCandidates)
{
  ...
  double strictCut = 0.025 ; //cut on the 'short' distance for the final decision (after shifting the stereo segments)
  ...
  //so, for each track it is checked if there es only one stereo segment in each superlayer, if not, the best segment is chosen and the others are removed
  for (int i = 0; i < nTracks; i++) {
