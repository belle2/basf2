...
namespace Belle2 {
  ...
  class CDCLegendreTrackHit : public TObject {
  public:
     ...
     */
    CDCLegendreTrackHit(CDCHit* hit, int index);
    ...
    /** Returns true if hit was already assigned to a track*/
    bool usedForTracking() const {return (m_TrackIndices.size() == 0);};

    /** Assign hit to track*/
    void assignToTrack(int ID);
    ...
    void shiftAlongZ(const CDCLegendreTrackCandidate& track);
    ...
    std::vector<int> getAssignedTracks() const {
  ...
  private:
    ...
    ClassDef(CDCLegendreTrackHit, 1);
