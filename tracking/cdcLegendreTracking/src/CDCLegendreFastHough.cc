...
void CDCLegendreFastHough::MaxFastHoughHighPtHeap(const std::vector<CDCLegendreTrackHit*>& hits, const int theta_min, const int theta_max,
...
//      level_diff = 0;
    ...
    if (voted_hits[t_index][r_index].size() >= m_limit) {
      ...
      if (nbins_r == 1) {
        ...
        //Recursive calling of the function with higher level and smaller box
        MaxFastHoughHighPtHeap(voted_hits[t_index][r_index], thetaBin[t_index], thetaBin[t_index + 1],
...
void CDCLegendreFastHough::MaxFastHoughHighPtStack(const std::vector<CDCLegendreTrackHit*>& hits, const int theta_min, const int theta_max,
                                                   double r_min, double r_max, int level)
{
...
//        B2INFO("LEVEL: " << level << "; r=" << (r[r_index] + r[r_index + 1]) / 2.);
        ...
        //Recursive calling of the function with higher level and smaller box
        MaxFastHoughHighPtStack(voted_hits[t_index][r_index], thetaBin[t_index], thetaBin[t_index + 1],
                                r[r_index], r[r_index + 1], level);
