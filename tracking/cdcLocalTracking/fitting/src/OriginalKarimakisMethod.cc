...
#include "../include/OriginalKarimakisMethod.h"
...
ClassImpInCDCLocalTracking(OriginalKarimakisMethod)


OriginalKarimakisMethod::OriginalKarimakisMethod() :
  _curved(true),
  _npar(_curved ? 3 : 2),
  _parameters(_npar),
  _covariance(_npar)
...
void OriginalKarimakisMethod::update(CDCTrajectory2D& trajectory2D,
...
void OriginalKarimakisMethod::simpleFitXY(bool flag, double xr, double yr) const
{
  _xRef = xr;
  _yRef = yr;
  _numPoints = 0;
...
void OriginalKarimakisMethod::addPoint(double x, double y, double w) const
{
  _numPoints++;
  double xl = x - _xRef;
  double yl = y - _yRef;
...
int OriginalKarimakisMethod::fit(double& Chi2, int& nPoints) const
{
  // averages
  double ax = _sx / _sw;
  double ay = _sy / _sw;
...
TVectorD OriginalKarimakisMethod::getPar() const
{
  return _parameters;
}


TMatrixDSym OriginalKarimakisMethod::getCov() const
{
  return _covariance;
}
