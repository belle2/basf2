...
void CDCCKFResultStorer::exposeParameters(ModuleParamList* moduleParamList, const std::string& prefix)
{
  ...
  moduleParamList->addParameter(TrackingUtilities::prefixed(prefix, "trackFindingDirection"),
                                m_param_trackFindingDirectionAsString,
                                "Direction in which the track is reconstructed (SVD/ECL seed)",
                                m_param_trackFindingDirectionAsString);

  moduleParamList->addParameter(TrackingUtilities::prefixed(prefix, "exportAllTracks"),
                                m_param_exportAllTracks,
                                "Export all tracks, even if they did not reach the center of the CDC",
                                m_param_exportAllTracks);

  moduleParamList->addParameter(TrackingUtilities::prefixed(prefix, "seedComponent"),
                                m_seedComponentString,
                                "Where does the seed track come from (typically SVD, ECL)",
                                m_seedComponentString);

  moduleParamList->addParameter(TrackingUtilities::prefixed(prefix, "setTakenFlag"),
...
void CDCCKFResultStorer::apply(const std::vector<CDCCKFResult>& results)
{
  for (const CDCCKFResult& result : results) {
    ...
    if (m_param_trackFindingDirection == TrackingUtilities::EForwardBackward::c_Forward) {
      trackState = &result.at(1).getTrackState();
    } else if (m_param_trackFindingDirection == TrackingUtilities::EForwardBackward::c_Backward) {
    ...
    if (not m_param_exportAllTracks
        && m_param_trackFindingDirection == TrackingUtilities::EForwardBackward::c_Backward
    ...
    for (const CDCCKFState& state : result) {
      ...
      auto rl = state.getRLinfo()  == TrackingUtilities::ERightLeft::c_Right ?
