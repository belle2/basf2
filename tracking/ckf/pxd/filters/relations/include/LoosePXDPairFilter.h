...
namespace Belle2 {
  ...
  private:
    ...
    double m_param_ThetaOverlayRegionPrecut = 0.5;//1.0;
    /// Pre-filter relations in phi between seed states and hit states
    double m_param_PhiSeedHitPrecut = 1.0;//M_PI / 2.;
    /// Pre-filter relations in phi between hit states
    double m_param_PhiHitHitPrecut = 1.0;//M_PI / 2.;
    /// Pre-filter relations in theta between hit states
    double m_param_ThetaHitHitPrecut = 0.5;//M_PI / 2.;
  };
}
