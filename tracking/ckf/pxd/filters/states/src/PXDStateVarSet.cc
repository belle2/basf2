...
namespace {
  ...
  double meanOver(const std::vector<const CKFToPXDState*>& states, const APredicate& t)
  {
    double sum = 0;
    unsigned int numberOfHits = 0;

    for (const CKFToPXDState* walkState : states) {
      const SpacePoint* spacePoint = walkState->getHit();
      if (not spacePoint) {
        continue;
      }
      for (auto& cluster : spacePoint->getRelationsTo<PXDCluster>()) {
        numberOfHits++;
        sum += t(cluster);
      }
    }
  ...
  template<class APredicate>
  double minOver(const std::vector<const CKFToPXDState*>& states, const APredicate& t)
  {
  ...
  template<class APredicate>
  double stdOver(const std::vector<const CKFToPXDState*>& states, const APredicate& t)
  {
    double sum = 0;
    double sumSquared = 0;
    unsigned int numberOfHits = 0;
...
bool PXDStateVarSet::extract(const BasePXDStateFilter::Object* pair)
{
  ...
  if (spacePoint->getType() == VXD::SensorInfoBase::SensorType::PXD) {
    const auto& clusters = spacePoint->getRelationsTo<PXDCluster>();

    B2ASSERT("Must be related to exactly 2 clusters", clusters.size() == 2);
    const PXDCluster* firstCluster = clusters[0];
    const PXDCluster* secondCluster = clusters[1];
  ...
  for (const CKFToPXDState* walkState : allStates) {
