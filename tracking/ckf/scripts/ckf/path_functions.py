...
def add_ckf_based_vtx_track_merger(path, cdc_reco_tracks, vtx_reco_tracks, use_mc_truth=False, direction="backward"):
    """
    Convenience function to merge existing CDC RecoTracks from CDCTrackFinding with existing VTX RecoTracks created
    with the VXDTF2.
    :param path: The path to add the module to
    :param cdc_reco_tracks: The name of the already created CDC Reco Tracks
    :param vtx_reco_tracks: The name of the already created VTX Reco Tracks
    :param use_mc_truth: Use the MC information in the CKF
    :param direction: where to extrapolate to. Valid options are forward and backward
    """
    # The CDC tracks need to be fitted
    ...
    if use_mc_truth:
        ...
        path.add_module("MCRecoTracksMatcher",
                        UsePXDHits=False, UseSVDHits=False, useVTXHits=True, UseCDCHits=False,
                        mcRecoTracksStoreArrayName="MCRecoTracks",
                        prRecoTracksStoreArrayName=vtx_reco_tracks)
    """
        result_filter = "truth_vtx_cdc_relation"
        result_filter_parameters = {}
    else:
        result_filter = "mva_with_relations"
    ...
    path.add_module("CDCToVTXSeedCKF",
                    advanceHighFilterParameters={"direction": direction},

                    fromRelationStoreArrayName=cdc_reco_tracks,
                    toRelationStoreArrayName=vtx_reco_tracks,

                    writeOutDirection=direction,

                    inputRecoTrackStoreArrayName=cdc_reco_tracks,
                    relatedRecoTrackStoreArrayName=vtx_reco_tracks,
                    relationCheckForDirection=direction,
                    cdcTracksStoreArrayName=cdc_reco_tracks,
                    vtxTracksStoreArrayName=vtx_reco_tracks,
                    ...
                    # filterParameters={"treeName": "merger_resultrecords_"+direction+"_eval",
                    # "rootFileName": "vtx_merger_resultfilter_records_"+direction+"_eval.root",
                    # "returnWeight": 1.0}
                    ).set_name(f"CDCToVTXSeedCKF_{direction}")


def add_vtx_ckf(path, cdc_reco_tracks, cdc_vtx_reco_tracks, use_mc_truth=False,
                first_filter_cut=0.2, second_filter_cut=0.1, third_filter_cut=0.1,
                result_filter_cut=0.01, use_best_results=10, use_best_seeds=50, direction="backward"):
    """
    Convenience function to add the VTX ckf to the path.
    :param path: The path to add the module to
    :param cdc_reco_tracks: The name of the already created CDC reco tracks
    :param cdc_vtx_reco_tracks: The name to output the VTX reco tracks to
    :param use_mc_truth: Use the MC information in the CKF
    :param first_filter_cut: CKF parameter for MVA firstFilter
    :param second_filter_cut: CKF parameter for MVA secondFilter
    :param third_filter_cut: CKF parameter for MVA thirdFilter
    :param result_filter_cut: CKF parameter for MVA resultFilter
    :param use_best_results: CKF parameter for useNResults
    :param use_best_seeds: CKF parameter for useBestNInSeed
    :param direction: where to extrapolate to. Valid options are forward and backward
    """
    if "VTXSpacePointCreator" not in [m.name() for m in path.modules()]:
        add_vtx_SPcreation(path)
    ...
    if use_mc_truth:
        ...
        module_parameters = dict(
            firstHighFilterParameters={
              'DBPayloadName': 'ckf_ToVTXStateFilter_1_Parameters',
              "direction": direction
            },
            firstHighUseNStates=use_best_seeds,

            secondHighFilterParameters={
              'DBPayloadName': 'ckf_ToVTXStateFilter_2_Parameters',
            },
            secondHighUseNStates=use_best_seeds,

            thirdHighFilterParameters={
              'DBPayloadName': 'ckf_ToVTXStateFilter_3_Parameters',
            },
            thirdHighUseNStates=use_best_seeds,

            filterParameters={
              'DBPayloadName': 'ckf_ToVTXResultFilter_Parameters',
            },
            useBestNInSeed=use_best_results,
        )

    path.add_module("CDCToVTXSpacePointCKF",
                    inputRecoTrackStoreArrayName=cdc_reco_tracks,
                    outputRecoTrackStoreArrayName=cdc_vtx_reco_tracks,
                    outputRelationRecoTrackStoreArrayName=cdc_reco_tracks,
                    relatedRecoTrackStoreArrayName=cdc_vtx_reco_tracks,
                    ...
                    **module_parameters).set_name(f"CDCToVTXSpacePointCKF_{direction}")


def add_cosmics_vtx_ckf(path, cdc_reco_tracks, cdc_vtx_reco_tracks, use_mc_truth=False,
                        use_best_results=5, use_best_seeds=10, direction="backward"):
    """
    Convenience function to add the VTX ckf to the path with cosmics settings.
    :param path: The path to add the module to
    :param cdc_reco_tracks: The name of the already created CDC reco tracks
    :param cdc_vtx_reco_tracks: The name to output the VTX reco tracks to
    :param use_mc_truth: Use the MC information in the CKF
