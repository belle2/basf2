...
class VtxCdcMergerHarvesterMCSide(HarvestingModule):
    """Gather the MC VTX-CDC-merger results into ROOT file"""
    ...
    def __init__(self, output_file_name):
        ...
        #: matcher used for the MCTracks from the CDC
        self.mc_track_matcher_cdc = Belle2.TrackMatchLookUp("MCRecoTracks", "CDCRecoTracks")
        #: matcher used for the MCTracks from the VTX
        self.mc_track_matcher_vtx = Belle2.TrackMatchLookUp("MCRecoTracks", "VTXRecoTracks")
    ...
    def peel(self, mc_particle):
        ...
        this_best_track_cdc = self.mc_track_matcher_cdc.getMatchedPRRecoTrack(mc_track)
        this_best_track_vtx = self.mc_track_matcher_vtx.getMatchedPRRecoTrack(mc_track)
        ...
        result.update(peelers.peel_reco_track_hit_content(this_best_track_vtx, key="vtx_{part_name}"))
        result.update(peelers.peel_reco_track_seed(this_best_track_vtx, key="vtx_{part_name}"))
        if this_best_track_vtx:
            result["vtx_store_array_number"] = this_best_track_vtx.getArrayIndex()
            result["vtx_was_fitted"] = this_best_track_vtx.wasFitSuccessful()
        else:
            result["vtx_store_array_number"] = -1
            result["vtx_was_fitted"] = -1

        if this_best_track_cdc:
            merged_vtx_track = this_best_track_cdc.getRelated("VTXRecoTracks")
        else:
            merged_vtx_track = None
        if this_best_track_vtx:
            merged_cdc_track = this_best_track_vtx.getRelated("CDCRecoTracks")
        else:
            merged_cdc_track = None
        ...
        flag_correct_not_merged_cdcmissing = False
        flag_incorrect_merged_cdcwrong_although_not_there = False
        flag_correct_not_merged_vtxmissing = False
        flag_incorrect_merged_vtxwrong_although_not_there = False
        ...
        # No tracks exists at all --> Problem of Trackfinder
        if not this_best_track_cdc and not this_best_track_vtx:
            # Case 7
            flag_not_found = True
        ...
        # VTX track not existing
        elif not this_best_track_vtx:
            # Cross check> merged track == best track!!
            if not merged_vtx_track:
                # Case 5
                flag_correct_not_merged_vtxmissing = True
            # If merged track i is not emtpy, merger made a mistake
            else:
                # Case 3 !!
                flag_incorrect_merged_vtxwrong_although_not_there = True

        # Both tracks exists but have not been merged, although they should have
        elif not merged_vtx_track and not merged_cdc_track:
            # Case 6
            flag_incorrect_not_merged = True

        # Tracks are merged correctly
        elif this_best_track_cdc == merged_cdc_track and this_best_track_vtx == merged_vtx_track:
            # Case 1
            flag_correct_merged = True
        ...
        # We miss the unmatched cdc/vtx reco tracks which are fakes!!!

        result.update({
            "flag_correct_not_merged_cdcmissing": flag_correct_not_merged_cdcmissing,
            "flag_correct_not_merged_vtxmissing": flag_correct_not_merged_vtxmissing,
            "flag_correct_merged": flag_correct_merged,
            "flag_incorrect_not_merged": flag_incorrect_not_merged,
            "flag_incorrect_merged": flag_incorrect_merged,
            "flag_incorrect_merged_cdcwrong_although_not_there": flag_incorrect_merged_cdcwrong_although_not_there,
            "flag_incorrect_merged_vtxwrong_although_not_there": flag_incorrect_merged_vtxwrong_although_not_there,
            "flag_not_found": flag_not_found,
        })
