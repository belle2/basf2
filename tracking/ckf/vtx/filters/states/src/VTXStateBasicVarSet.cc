...
bool VTXStateBasicVarSet::extract(const BaseVTXStateFilter::Object* pair)
{
  ...
  const Vector3D& position = Vector3D(firstMeasurement.getPos());
  const Vector3D& momentum = Vector3D(firstMeasurement.getMom());
  ...
  const CKFToVTXState* parent = previousStates.back();
...
// This part is commented as it might be beneficial in the future with the new VTXID, so I leave it in for now.
//   else {
//     const CKFToVTXState::stateCache& parentCache = parent->getStateCache();
//     var<named("last_layer")>() = parentCache.geoLayer;
//   }
