...
namespace Belle2 {
  ...
  /**
   * Combinatorial Kalman Filter to extrapolate CDC Reco Tracks into the VTX and collect space points.
   *
   ...
   * * Construct all possible candidates starting from a RecoTrack and going through the layers of the VTX collecting
  ...
  private:
    ...
    CKFRelationCreator<CKFToVTXState, ChooseableVTXRelationFilter> m_relationCreator;
    ...
    OverlapResolver<ChooseableVTXResultFilter> m_overlapResolver;
    /// Findlet for tagging the used space points
    SpacePointTagger<CKFToVTXResult, VTXCluster> m_spacePointTagger;
    /// Findlet for storing the results
    ResultStorer<CKFToVTXResult> m_resultStorer;
    ...
    std::vector<CKFToVTXResult> m_filteredResults;
  };
}
