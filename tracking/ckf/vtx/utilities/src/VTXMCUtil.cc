...
bool MCUtil::vtxHitIsCorrect(const RecoTrack* mcRecoTrack, const SpacePoint* spacePoint) const
{
  ...
  for (const VTXCluster& relatedCluster : spacePoint->getRelationsTo<VTXCluster>()) {
  ...
  const RecoHitInformation* recoHitInformationOfHit = mcRecoTrack->getRecoHitInformation(spacePoint->getRelatedTo<VTXCluster>());
...
unsigned int MCUtil::getNumberOfCorrectVTXHits(const RecoTrack* mcRecoTrack, const std::vector<const SpacePoint*>& hits) const
{

  const auto hitIsCorrectSpecialisation = [this, mcRecoTrack](const SpacePoint * spacePoint) {
    return vtxHitIsCorrect(mcRecoTrack, spacePoint);
  };
...
bool MCUtil::allStatesCorrect(const std::vector<TrackingUtilities::WithWeight<const CKFToVTXState*>>& states) const
{
  ...
  for (const CKFToVTXState* state : states) {
  ...
  const unsigned int numberOfCorrectHits = getNumberOfCorrectVTXHits(mcTrack, spacePoints);
