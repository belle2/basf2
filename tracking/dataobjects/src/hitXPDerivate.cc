                             ...
                             const VXD::SensorInfoBase& sensor)
{
  ...
  m_positionEntry = sensor.pointToGlobal(TVector3(hit.getEntryU(), hit.getEntryV(), hit.getEntryW()), false); //position at entrypoint
...
hitXPDerivate::hitXPDerivate(const PXDTrueHit& hit, const MCParticle& particle, const VXD::SensorInfoBase& sensor)
{
  ...
  m_info_omega = tan(MCParticleInfo(particle, TVector3(0, 0, 1.5)).getLambda()); //omega from particle info DO NOT USE, use the get
  m_reconstructed = false; //flag of reconstructed
