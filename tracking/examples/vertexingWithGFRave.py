...
# This steering shows the use of the vertexer module.
# The Vertexer modules uses Rave and Genfit for Vertex finding and fitting
import os
from basf2 import *
...
param_pGun = {
    ...
    # 'xVertexParams': [0.0, vertexStd],
    # 'yVertexParams': [0.0, vertexStd],
    # 'zVertexParams': [0.0, vertexStd],
...
# here you can choose the propagation method used during vertexing.
# Using the Genfit propagation is default
# If this parameter is set the false the Rave interal propagation is used.
# The Rave internal propagation ignores material.
vertexer.param('useGenfitPropagation', True)

# here you see how to give the beam spot info to the vertexer module.
# This does not seem to work correctly at the moment
# vertexer.param('useBeamSpot', True)
# vertexer.param('beamSpotPosition', [0.0,0.0,0.0])
# beamSpotCov = [vertexVar,0.0,0.0,
               # 0.0,vertexVar,0.0,
               # 0.0,0.0,vertexVar]
...
# adaptive vertex fitter -- a Kalman filter based robust vertex fitter
# (similar to the DAF for track fitting)
# vertexer.param('vertexingMethod','avf-Tini:256-ratio:0.25-sigmacut:3-smoothing:1')

# This methed has still problems!
# vertexer.param('vertexingMethod','tkf-ptcut:0-trkcutpv:0.05-trkcutsv:0.01-vtxcut:0.01-smoothing:1')

# the VertexFitChecker will calculated the vertex resolution, pulls and p values
