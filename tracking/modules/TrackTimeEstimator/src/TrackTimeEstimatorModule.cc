    ...
    if (svdHypos.size() > 0) { // if SVD eventT0 exists then loop over tracks, don't otherwise and leave their averageTime set at NaN
      ...
      for (auto& track : m_tracks) {
        ...
        if (recoTrack->hasOutgoingArmTime()) {
          track.setTrackTime(recoTrack->getOutgoingArmTime(nSVDHitsOutgoingArm) - svdBestT0.eventT0);
        } else {
          if (recoTrack->hasIngoingArmTime()) {
            track.setTrackTime(recoTrack->getIngoingArmTime(nSVDHitsIngoingArm) - svdBestT0.eventT0);
          }
        }
      }
    }
