...
namespace Belle2 {
  ...
  class VXDHit {
  public:
    VXDHit(short int hitType, std::string fullSecID, TVector3 hit, TVector3 mom, int pdg):
      m_type(hitType),
      m_secID(fullSecID),
      m_timeStamp(0),
      m_pxdHit(NULL),
...
//      std::cout << m_filterName <<  " includes " << m_valuePack.size() << " entries" << std::endl;
  ...
  protected:
    MapOfSectors m_sectorMap; /**< sectormap contains full info about sectors */
...
//    bool m_calcTrajectory; /**< outdated */
//    bool m_fixPt;
    ...
    bool m_PARAMlogDeltaZ; /**< set 'true' if you want to log delta Z-values between segments */
    ...
    bool m_PARAMlogDeltaTRadiustoIPDistance; /**< set 'true' to compare different TRadius2IPDistances */
