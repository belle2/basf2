...
namespace Belle2 {
  ...
  public:
    ...
    bool checkAcceptanceOfSecMap(FilterCalcNames::InternalRawSectorMap* secMap,
...
//     int m_PARAMdetectorType; /**< sets detector type -1 for VXD, Const::PXD for PXD and Const::SVD for SVD. */ WARNING OLD!
    ...
    int m_PARAMtestBeam; /**< some things which are important for the real detector are a problem for testbeams, if you want to use the SecMapTrainerBase and the testBeam = false does not work, then try setting the parameter to true. */
