...
RawSecMapMergerModule::~RawSecMapMergerModule()
{
...
void RawSecMapMergerModule::initialize()
{
...
void RawSecMapMergerModule::terminate()
{
...
//   TVector3 origin(m_PARAMsetOrigin[0], m_PARAMsetOrigin[1], m_PARAMsetOrigin[2]);
  ...
  B2INFO("RawSecMapMergerModule::terminate: importing the map took " << (m_fillStuff.count()) << "/" << int(
           m_fillStuff.count() * 0.000001) << " nano-/milliseconds")
}



double RawSecMapMergerModule::getXMLValue(GearDir& cuts, string& valueType, string& filterType)
{
  double aValue;
  try {
...
std::pair<int, int> RawSecMapMergerModule::importXMLMap()
{
...
std::pair<int, int> RawSecMapMergerModule::importROOTMap()
{
  int countSectors = 0,
      countExternalMaps = 0,
      countTotalValues =
