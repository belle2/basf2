...
namespace Belle2 {
   ...
   */
  class VXDtoCDCFindlet : public TrackFindingCDC::Findlet<> {
    /// Parent class
  ...
  private:
    ...
    /// Findlet for resolving overlaps Overlap ignorieren???????????????????????????????????????????????????????????????
    ...
    std::vector<const SpacePoint*> m_vxdVector;
