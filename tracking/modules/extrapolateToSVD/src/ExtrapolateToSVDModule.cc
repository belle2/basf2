...
void ExtrapolateToSVDModule::event()
{
  ...
  for (int i = 0; i < nTracks; i++) {
    ...
    //the fit of the CDCHits should already provide a very good momentum estimation, so it makes sense to use the result of this fit as start values for the fir with svd hits
  ...
  for (int iTrack = 0; iTrack < nTrackCands; iTrack ++) {
    ...
    for (int iLayer = 6; iLayer > 2; iLayer --) {
      ...
      //B2INFO("Best Hit in layer "<< iLayer<< " found: ID "<<bestHitID<<" ( distance: "<<minDistance<<" )");
      ...
      if (minDistance < distanceCut) {
        ...
        newGFTrackCands[iTrack]->addHit(1, bestHitID, layerId, ladderId);
