...
//  if (m_fitMCTracks == true) {
  ...
  for (int i = 0; i < nTracks; i++) {
    ...
    while (iter not_eq iterMax) {
      hitCounter++;
      track.addHit(svdRecoHits[*iter], 1, hitCounter);
...
// }//endif m_fitMCTracks
  ...
  /*
    if (m_fitRecoTracks == true) {
      ...
      B2INFO("GenFitter2: Number of TrackCandidates: " << trackCandidates.GetEntries());
      if (trackCandidates.GetEntries() == 0) B2WARNING("GenFitter2: CDCTrackCandidatesCollection is empty!");

      StoreArray<Relation> cdcTrackCandToCDCRecoHits(m_cdcTrackCandToRecoHitsCollectionName);
      //B2INFO("GenFitter2: Number of relations between Tracks and CDCRecoHits: "<<cdcTrackCandToCDCRecoHits.GetEntries());
      if (cdcTrackCandToCDCRecoHits.GetEntries() == 0) B2WARNING("GenFitter2: CDCTrackCandToCDCRecoHitsCollection is empty!");

      StoreArray<Relation> cdcTrackCandToMCPart(m_cdcTrackCandToMCParticleCollectionName);
      //B2INFO("GenFitter2: Number of relations between Tracks and MCParticles: "<<cdcTrackCandToMCPart.GetEntries());
      if (cdcTrackCandToMCPart.GetEntries() == 0) B2WARNING("GenFitter2: CDCTrackCandToMCParticleCollection is empty!");
      ...
      for (int i = 0; i < trackCandidates->GetEntries(); i++) {
        ...
        for (int j = 0; j < cdcRecoHits->GetEntries(); j++) {
          ...
          BOOST_FOREACH(int hit, indexList) {
            if (j == hit) {
              hitCounter++;
              track.addHit(cdcRecoHits[j], 0, hitCounter);
            }
          }
