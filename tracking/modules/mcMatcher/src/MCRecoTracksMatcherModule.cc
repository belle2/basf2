...
namespace {
                             ...
                             const StoreArray<RecoTrack>& storedRecoTracks)
  {
    ...
    for (const RecoTrack& recoTrack : storedRecoTracks) {
      ...
      for (const RecoHitInformation::UsedVTXHit* vtxHit : recoTrack.getVTXHitList()) {
        OriginTrackFinder originFinder = recoTrack.getFoundByTrackFinder(vtxHit);
        double weight = originFinder == c_MCTrackFinderAuxiliaryHit ? 0 : 1;
        totalWeight += weight;
        hitIDsInTrack.push_back({{Const::VTX, vtxHit->getArrayIndex()}, {recoTrackId, weight}});
      }
  ...
  : Module()
{
  setDescription("This module compares reconstructed tracks generated by some pattern recognition "
                 "algorithm for PXD, SVD, VTX and/or CDC to ideal Monte Carlo tracks and performs a "
  ...
  addParam("UseVTXHits",
           m_param_useVTXHits,
           "Set true if VTXHits or VTXClusters should be used in the matching in case they are present",
           true);
...
void MCRecoTracksMatcherModule::initialize()
{
  if (m_MCParticles.isOptional()) {
    ...
    // VTX
    if (m_param_useVTXHits) {
      m_VTXClusters.isOptional();
    }
...
void MCRecoTracksMatcherModule::event()
{
  ...
  // VTX
  if (m_param_useVTXHits) {
    if (m_VTXClusters.isOptional()) {
      int nHits = m_VTXClusters.getEntries();
      nHits_by_detId[Const::VTX] = nHits;
    }
  }
