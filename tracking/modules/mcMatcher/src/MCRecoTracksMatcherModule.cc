...
void MCRecoTracksMatcherModule::event()
{
  ...
  for (RecoTrackId prId = 0; prId < nPRRecoTracks; ++prId) {
    ...
    if (m_useFittedTracks) {
      ...
      // Only use nPositiveCharges and nNegativeCharges to assign a new value to foundTrackCharge if fitted Tracks exists and at least one of the two values is > 0
