#!/usr/bin/env python
# -*- coding: utf-8 -*-

from basf2 import *

# register modules
rootInputModule = register_module('RootInput')
filename_data = '<PATH TO SIMULATION DATA>/PXDClusterSimulationData.root'
rootInputModule.param({'inputFileName': filename_data})

gearboxModule = register_module('Gearbox')
geometryModule = register_module('Geometry')

PXDClusterGetNNData = register_module('PXDClusterGetNNData')
PXDClusterGetNNData.param({'dataType': 'background'})
PXDClusterGetNNData.param({'filenameData': '../data/PXDClusterGetNNData_background.data'})
PXDClusterGetNNData.param({'filenameLog': '../data/PXDClusterGetNNData_background.log'})
PXDClusterGetNNData.param({'maxPt': 0.065})

# create path and add modules
main = create_path()

main.add_module(rootInputModule)
main.add_module(gearboxModule)
main.add_module(geometryModule)

main.add_module(PXDClusterGetNNData)

# process
process(main)
