...
void RT2SPTCConverterModule::event()
{
  ...
  for (auto& recoTrack : m_recoTracks) {
    ...
    B2DEBUG(20, "number of VTX hits in RecoTrack : " << recoTrack.getNumberOfVTXHits());
...
RT2SPTCConverterModule::getSpacePointsFromRecoHitInformationViaTrueHits(std::vector<RecoHitInformation*> hitInfos)
{
  ...
  for (const RecoHitInformation* hitInfo : hitInfos) {
    ...
    // VTX case
    if (hitInfo->getTrackingDetector() == RecoHitInformation::c_VTX) {
      cluster = hitInfo->getRelatedTo<VTXCluster>();
      if (cluster) {
        relatedTrueHit = cluster->getRelatedTo<VTXTrueHit>();
        B2DEBUG(20, "RT2SPTCConverter::getSpacePointsFromClustersViaTrueHits: Number of related VTXTrueHits: "
                << cluster->getRelationsTo<VTXTrueHit>().size());
      }
    }
...
RT2SPTCConverterModule::getSpacePointsFromRecoHitInformations(std::vector<RecoHitInformation*> hitInfos)
{
  ...
  for (size_t iHit = 0; iHit < hitInfos.size(); ++iHit) {
    ...
    if (hitInfos[iHit]->getTrackingDetector() != RecoHitInformation::c_SVD &&
        hitInfos[iHit]->getTrackingDetector() != RecoHitInformation::c_PXD &&
        hitInfos[iHit]->getTrackingDetector() != RecoHitInformation::c_VTX) continue;
    ...
    if (m_vtxSpacePointsStoreArrayName) {
      // simple case VTX : there is a one to one relation between cluster and SpacePoint
      if (hitInfos[iHit]->getTrackingDetector() == RecoHitInformation::c_VTX) {
        VTXCluster* vtxCluster = hitInfos.at(iHit)->getRelated<VTXCluster>();
        SpacePoint* relatedVTXSP = NULL;
        if (vtxCluster) relatedVTXSP = vtxCluster->getRelated<SpacePoint>(*m_vtxSpacePointsStoreArrayName);
        // if found a spacepoint one is already done!
        if (relatedVTXSP) {
          finalSpacePoints.push_back(relatedVTXSP);
          continue;
        }
      } // end VTX case
    }
