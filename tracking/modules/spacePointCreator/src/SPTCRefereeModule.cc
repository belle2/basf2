  ...
  if (useMCInfo) {
    try {
      for (const SpacePoint* spacePoint : spacePoints) {  // loop over all SpacePoints
        if (spacePoint->getType() == VXD::SensorInfoBase::PXD) {
        ...
        } else if (spacePoint->getType() == VXD::SensorInfoBase::VTX) {
          dirsOfFlight.push_back(getDirOfFlightTrueHit<VTXTrueHit>(spacePoint, m_origin));
          ...
          SpacePointTrackCand::UnsupportedDetType(); // NOTE: should never happen, because SpacePointTrackCand can only handle PXD, SVD and VTX at the moment!
      }
