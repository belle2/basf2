  ...
  Module()
{
  ...
  addParam("SpacePointNames", m_PARAMspacePointNames, "Container names of SpacePoints.", {"SVDSpacePoints", "PXDSpacePoints", "VTXSpacePoints"});
  addParam("DetectorTypes", m_PARAMdetectorTypes,
           "detector types to determine which entries in 'TrueHitNames' and 'SpacePointNames' belong to which detector type. Entries have to be 'SVD' or 'PXD' or 'VTX'. NOTE: if more 'SpacePointNames' than 'DetectorTypes' get passed, the last entry in 'DetectorTypes' is assumed to be valid for all remaining 'SpacePointNames'!");
...
void SpacePoint2TrueHitConnectorModule::initialize()
{
  ...
  for (unsigned int i = 0; i < m_nContainers; ++i) {
    if (i < nDetTypes) { // handle the detector types
      ...
      if (detType.compare(std::string("SVD")) != 0 && detType.compare(std::string("PXD")) != 0
          && detType.compare(std::string("VTX")) != 0) {
        B2FATAL("Found entry " << detType << " in DetectorTypes, but only 'PXD', 'SVD' and 'VTX' are allowed!");
      }
      ...
      } else if (detType.compare(std::string("VTX")) == 0) {
        m_detectorTypes.push_back(c_VTX);
        m_VTXTrueHits = StoreArray<VTXTrueHit>(m_PARAMtrueHitNames.at(i));
        m_VTXTrueHits.isRequired();
        m_VTXClusters = StoreArray<VTXCluster>(m_PARAMclusterNames.at(i));
        m_VTXClusters.isRequired();
      } else {
  ...
  if (m_PARAMstoreSeparate) {
    ...
    for (unsigned int i = 0; i < m_nContainers; ++i) {
      ...
      } else if (m_inputSpacePoints.at(i).second == c_VTX) {
        m_outputSpacePoints.at(i).registerRelationTo(m_VTXTrueHits, DataStore::c_Event, DataStore::c_DontWriteOut);
        m_outputSpacePoints.at(i).registerRelationTo(m_VTXClusters, DataStore::c_Event, DataStore::c_DontWriteOut);
      } else {
    ...
    for (unsigned int i = 0; i < m_nContainers; ++i) {
      if (m_inputSpacePoints.at(i).second == c_SVD) {
        ...
        m_inputSpacePoints.at(i).first.registerRelationTo(m_VTXTrueHits, DataStore::c_Event, DataStore::c_DontWriteOut);
      } else {
  ...
  for (m_iCont = 0; m_iCont < m_nContainers; ++m_iCont) {
    ...
    for (int iSP = 0; iSP < nSpacePoints; ++iSP) {
      ...
      if (m_PARAMregisterAll) {
        ...
        else if (detType == c_VTX)  trueHitwWeight = getTHwithWeight<baseMapT, VTXTrueHit>(trueHitMap, m_VTXTrueHits, spacePoint, c_VTX);
...
MapType SpacePoint2TrueHitConnectorModule::processSpacePoint(Belle2::SpacePoint* spacePoint, e_detTypes detType)
{
  ...
  try {
    if (detType == c_PXD) {
      ...
      trueHitMap = getRelatedTrueHits<MapType, VTXCluster, VTXTrueHit>(spacePoint, m_VTXClusters.getName(), m_VTXTrueHits.getName());
    } else {
...
void SpacePoint2TrueHitConnectorModule::registerAllRelations(Belle2::SpacePoint* spacePoint, MapType trueHitMap, e_detTypes detType)
{
  ...
  if (m_PARAMstoreSeparate) { // if storing in separate Array, re-register the relations to the Clusters first
    ...
    else if (detType == c_VTX) reRegisterClusterRelations<VTXCluster>(spacePoint, newSP, m_VTXClusters.getName());
  ...
  for (const TrueHitInfo& info : trueHitInfos) {
    ...
    else if (detType == c_VTX) registerTrueHitRelation<VTXTrueHit>(newSP, info.m_Id, 1, m_VTXTrueHits); // only one Cluster for VTX!
