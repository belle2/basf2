...
//     double lWire = fabs(hit->getBackwardWirePosition().Z() - hit->getForwardWirePosition().Z());
    ...
    for (int iPdg = 0; iPdg != nPdg; ++iPdg) {  // loop over all pdg hypothesises
      ...
      */
      ...
      try {
        ...
        fitter->processTrack(gfTrack.get());
  ...
  [](std::pair<std::shared_ptr<genfit::Track>, double> const a, std::pair<std::shared_ptr<genfit::Track>, double> const b) {return a.second < b.second;});
