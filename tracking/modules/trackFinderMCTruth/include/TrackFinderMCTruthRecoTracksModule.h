...
namespace Belle2 {
   ...
   *
   *  The Relations MCParticles -> Hits for PXD, SVD, VTX and CDC are used.
   *  At the moment CDCHits, PXDTrueHits, SVDTrueHits, VTXTrueHits, PXDCluster hits, SVDCluster hits and VTXCluster hits can be used
  ...
  private:
    ...
    bool m_useVTXHits;                                          /**< Boolean to select if VTXHits should be used*/
    ...
    int m_minVTXHits;                                           /**< Minimum number of VTX hits per track to allow track candidate creation*/
    ...
    double m_splitAfterDeltaT; /**< Minimal time delay between two sim hits (in ns) after which MC reco track will be split into separate tracks. If < 0, don't do splitting.*/
