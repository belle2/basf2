...
TrackFinderMCTruthRecoTracksModule::TrackFinderMCTruthRecoTracksModule() : Module()
{
  ...
  addParam("UseVTXHits",
           m_useVTXHits,
           "Set true if VTXHits or VTXClusters should be used",
           true);
  addParam("UseCDCHits",
           m_useCDCHits,
  ...
  addParam("MinVTXHits",
           m_minVTXHits,
           "Minimum number of VTX hits needed to allow the created of a track candidate",
           0);
  ...
  addParam("WhichParticles",
           ...
           "\"PXD\", \"SVD\", \"VTX\", \"CDC\", \"TOP\", \"ARICH\", \"ECL\" or \"KLM\" particle must have hits in the subdetector with that name. "
...
void TrackFinderMCTruthRecoTracksModule::initialize()
{
  ...
  if (mcparticles.isOptional()) {
    ...
    for (int i = 0; i not_eq nProperties; ++i) {
      ...
      } else if (m_whichParticles[i] == "VTX") {
        m_particleProperties += 256;
...
void TrackFinderMCTruthRecoTracksModule::event()
{
  ...
  //VTX trueHits
  StoreArray<VTXTrueHit> vtxTrueHits;
  const int nVTXHits = vtxTrueHits.getEntries();
  B2DEBUG(20, "MCTrackFinder: Number of VTXTrueHits: " << nVTXHits);

  RelationArray mcPartToVTXTrueHits(mcParticles, vtxTrueHits);
  const int nMcPartToVTXHits = mcPartToVTXTrueHits.getEntries();
  B2DEBUG(20, "MCTrackFinder: Number of relations between MCParticles and VTXHits: " << nMcPartToVTXHits);

  //VTX clusters
  StoreArray<VTXCluster> vtxClusters;
  const int nVTXClusters = vtxClusters.getEntries();
  B2DEBUG(20, "MCTrackFinder: Number of VTXClusters: " << nVTXClusters);

  RelationArray vtxClusterToMCParticle(vtxClusters, mcParticles);
  const int nVtxClusterToMCPart = vtxClusterToMCParticle.getEntries();
  B2DEBUG(20, "MCTrackFinder: Number of relations between VTXCluster and MCParticles: " << nVtxClusterToMCPart);
  ...
  // prepare rejection of CDC/PXD/SVD/VTX hits from higher order loops
  ...
  for (int iPart = 0; iPart < nMcParticles; ++iPart) {
    ...
    if (aMcParticlePtr->hasSeenInDetector(Const::VTX)) {
      mcParticleProperties += 256;
    }
    ...
    // entry 1: indices in the respective hit arrays (PXD, SVD, VTX, CDC, ...)
    ...
    // create a list containing the indices to the VTXHits that belong to one track
    if (m_useVTXHits) {
      int hitCounter = 0;
      for (const MCParticle* trackMCParticle : trackMCParticles) {
        const RelationVector<VTXCluster>& relatedClusters = trackMCParticle->getRelationsFrom<VTXCluster>();
        ...
        for (size_t i = 0; i < relatedClusters.size(); ++i) {
          ...
          // currently only priority Hits for VTX
          ...
          if (std::isfinite(m_useNLoops) and not isWithinNLoops<VTXCluster, VTXTrueHit>(Bz, relatedClusters.object(i), m_useNLoops)) {
            mcFinder = RecoHitInformation::OriginTrackFinder::c_MCTrackFinderAuxiliaryHit;
          }
          ...
          const VTXCluster* vtxCluster = relatedClusters.object(i);
          const RelationVector<VTXTrueHit>& relatedTrueHits = vtxCluster->getRelationsTo<VTXTrueHit>();
          ...
          float time = NAN;
          for (const VTXTrueHit& vtxTrueHit : relatedTrueHits) {
            ...
            const RelationVector<MCParticle>& relatedMCParticles = vtxTrueHit.getRelationsFrom<MCParticle>();
            ...
            return &mcParticle == trackMCParticle;
              ...
              time = vtxTrueHit.getGlobalTime();
              break;
            }
          }
          if (not std::isnan(time)) {
            hitsWithTimeAndDetectorInformation.emplace_back(time, vtxCluster->getArrayIndex(), mcFinder, Const::VTX);
            ++hitCounter;
            ndf += 2;
          }
        }

        B2DEBUG(20, "     add " << hitCounter << " VTXClusters. " << relatedClusters.size() - hitCounter <<
                " VTXClusters were not added because they do not have a corresponding VTXTrueHit");
      } // next trackMCParticle

      if (hitCounter < m_minVTXHits) {
    ...
    for (const auto& hitInformationVector : hitsWithTimeAndDetectorInformationVectors) {
      ...
      for (const TimeHitIDDetector& hitInformation : hitInformationVector) {
        ...
        } else if (detectorInformation == Const::VTX) {
          const VTXCluster* vtxCluster = vtxClusters[hitID];
          newRecoTrack->addVTXHit(vtxCluster, hitCounter, hitOriginMCFinderType);
      ...
      B2DEBUG(20, "New RecoTrack: #PXDHits: " << newRecoTrack->getPXDHitList().size() <<
              ...
              " #VTXHits: " << newRecoTrack->getVTXHitList().size() <<
