...
{
  if (m_evtT0.isValid()) {
    const auto svdBestT0 = m_evtT0->getBestSVDTemporaryEventT0();
    if (svdBestT0) { // if SVD eventT0 exists then loop over tracks, don't otherwise and leave their averageTime set at NaN
