...
namespace Belle2 {
  ...
  **  - one with only tracks with exactly one hit per SVD layer, so no overlap
  **    are allowed (TTree_hitXPTiSel.root)
  **  - one that does not use hitXP class but uses standard classes, it is
  **    lighter and contains less information but is usable externally from
  ...
  */
  ...
  private:
    ...
    std::vector<double> m_EpositionEntryX; /**< external position at entry point, coordinate x */
    std::vector<double> m_EpositionEntryY; /**< external position at entry point, coordinate y */
    std::vector<double> m_EpositionEntryZ; /**< external position at entry point, coordinate z */
    std::vector<double> m_EmomentumEntryX; /**< external momentum at entry point, coordinate x */
    std::vector<double> m_EmomentumEntryY; /**< external momentum at entry point, coordinate x */
    std::vector<double> m_EmomentumEntryZ; /**< external momentum at entry point, coordinate x */
    std::vector<double> m_EpositionLocalEntryX; /**< external local position at entry point, coordinate x */
    std::vector<double> m_EpositionLocalEntryY; /**< external local position at entry point, coordinate y */
    std::vector<double> m_EpositionLocalEntryZ; /**< external local position at entry point, coordinate z */
    std::vector<int> m_EPDGID; /**< external PDGID */
    std::vector<double> m_Eposition0X; /**< external position at IP, coordinate x */
    std::vector<double> m_Eposition0Y; /**< external position at IP, coordinate y */
    std::vector<double> m_Eposition0Z; /**< external position at IP, coordinate z */
    std::vector<double> m_Emomentum0X; /**< external momentum at IP, coordinate x */
    std::vector<double> m_Emomentum0Y; /**< external momentum at IP, coordinate y */
    std::vector<double> m_Emomentum0Z; /**< external momentum at IP, coordinate z */
    ...
    int m_EnumberHitPerTrack = 0; /**< external number of the hit for each track */
