...
void TrackingPerformanceEvaluationModule::initialize()
{
  ...
  m_h1_nHitDetID = createHistogram1D("h1nHitDetID", "detector ID per hit", 5, -0.5, 4.5, "0=PXD, 1=SVD2D, 2=SVD, 3=CDC, 4=VTX",
                                     m_histoList);
...
void TrackingPerformanceEvaluationModule::event()
{
  ...
  // FIXME: pxdClusters && svdClusters are not needed here. Remove them?
    ...
    for (int i = 0; i < (int) tp_vector.size(); i++) {
      ...
      for (int mea = 0; mea < nMea; mea++) {
        ...
        if (vtxHit) {
          ...
          detId = 4;
          double uCoor = vtxHit->getU();
          double vCoor = vtxHit->getV();
          VxdID sensor = vtxHit->getSensorID();
          ...
          const VTXCluster* vtxcl = vtxHit->getCluster();
          RelationVector<VTXTrueHit> vtxth_fromcl = DataStore::getRelationsWithObj<VTXTrueHit>(vtxcl);

          if ((int)vtxth_fromcl.size() != 0) {
            const VTXTrueHit* trueHit = vtxth_fromcl[0];

            if (trueHit) {
              int trueHitIndex = trueHit->getArrayIndex();
              RelationVector<MCParticle> MCParticles_fromTrack = DataStore::getRelationsWithObj<MCParticle>(&theTrack);
              for (int mcp = 0; mcp < (int)MCParticles_fromTrack.size(); mcp++) {
                RelationVector<VTXTrueHit> trueHit_fromMCParticles = DataStore::getRelationsWithObj<VTXTrueHit>(MCParticles_fromTrack[mcp]);
                for (int th = 0; th < (int)trueHit_fromMCParticles.size(); th++) {
                  if (trueHit_fromMCParticles[th]->getArrayIndex() == trueHitIndex)
                    isTrueHit = true;
                }
