        ...
        if (fabs(det) > 1e-12) {
            ...
            if (c1 != 0) {
                ...
                if (root >= 0) {
                    ...
                    double ab1 = -(lp2.beta() * lp1.gamma() -
                                   lp1.beta() * lp2.gamma());
                    double ab2 = (lp2.alpha() * lp1.gamma() -
                                  lp1.alpha() * lp2.gamma());
                    double ac1 = -(lp2.beta() * lp1.kappa() -
                                   lp1.beta() * lp2.kappa());
                    double ac2 = (lp2.alpha() * lp1.kappa() -
                                  lp1.alpha() * lp2.kappa());
                    double dinv = 1.0 / det;
...
//cnv			double s1, s2;
//cnv			if (j == 0) {
//cnv			    s1 = lp1.s(v1(1), v1(2));
//cnv			    s2 = lp2.s(v1(1), v1(2));
//cnv			} else {
//cnv			    s1 = lp1.s(v2(1), v2(2));
//cnv			    s2 = lp2.s(v2(1), v2(2));
//cnv			}
//cnv     double phi1 = cp1.fi() + 2 * cp1.cu() * s1;
//cnv     double phi2 = cp2.fi() + 2 * cp2.cu() * s2;
                        double f = (1 + 2 * cp1.cu() * cp1.da()) *
                            (1 + 2 * cp2.cu() * cp2.da()) * cos(cp1.fi() -
                                                                cp2.fi());
                        f -= 2 *
                            (lp1.gamma() * lp2.kappa() +
                             lp2.gamma() * lp1.kappa());
