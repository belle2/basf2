...
TConformalFinder::selectGoodHits(void) {
    const bool ignoreSmallCell =
        (TCDC::getTCDC()->version() == "small cell") &&
        (! _useSmallCells);
...
//  w.skip(false);
    ...
    w.append(_allHits[2], Gdk::Color("grey"));
//  w.append(_hits[2], Gdk::Color("green"));
    for (unsigned i = 0; i < TCDC::getTCDC()->nSuperLayers(); i++) {
	unsigned superLayerId = i / 2;
	unsigned id = i % 2;
...
int
TConformalFinder::doit(const CAList<TWireHit> & axial,
...
TConformalFinder::conformal2Wire(const HepGeom::Point3D<double> & p) {
    ...
    float phi = p.phi();
    return TCDC::getTCDC()->wire(r, phi);
}
...
TConformalFinder::crossPointsInConformal(const AList<TSegment> & inList,
                                         HepGeom::Point3D<double> points[12])
    const {
...
#ifdef TRASAN_DEBUG_DETAIL
    const unsigned nSuperLayers = TCDC::getTCDC()->nSuperLayers();
    std::cout << "... refine segments : orignal sl = ";
    TDebugUtilities::bitDisplay(sl, nSuperLayers, 0);
    std::cout << ", output sl = ";
    TDebugUtilities::bitDisplay(SuperLayer(outList), nSuperLayers, 0);
    std::cout << std::endl;
#endif
...
//     //...MC tracks...
//     const unsigned nHep = TTrackMC::list().size();
