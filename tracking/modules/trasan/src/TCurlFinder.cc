...
//cnv
    ...
    const TLayer & l = * cdc.layer(layerId);
    if(ms_superb) {
...
//
int
TCurlFinder::doit(const CAList<TWireHit> & axialHits,
...
//
void
TCurlFinder::makeWireHitsListsSegments(
    const CAList<TWireHit> & axialList,
    const CAList<TWireHit> & stereoList) {
...
//                                            const unsigned num, bool stereo)
    ...
    const TCDC * const CDC = TCDC::getTCDC();
    for (unsigned i = 0; i < num; ++i) {
	if (list[i].length() == 0) continue;
...
#if DEBUG_CURL_MC
int
TCurlFinder::makeWithMC(const AList<TWireHit> & axialHits,
