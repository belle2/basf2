...
int
TFastFinder::doit(const AList<TWireHit> & axialHits,
...
void
TFastFinder::selectHits(const AList<TWireHit> & axialHits,
			const AList<TWireHit> & stereoHits) {
    unsigned n = axialHits.length();
    for (unsigned i = 0; i < n; i++) {
	const TWireHit& h = * axialHits[i];
	if ((h.state() & CellHitIsolated) && (h.state() & CellHitContinuous))
	    _axialHits.append((TWireHit &) h);
    }
    n = stereoHits.length();
    for (unsigned i = 0; i < n; i++) {
	const TWireHit& h = * stereoHits[i];
	if ((h.state() & CellHitIsolated) && (h.state() & CellHitContinuous))
	    _stereoHits.append((TWireHit &) h);
    }
}

void
TFastFinder::selectHits2(const AList<TWireHit> & axialHits,
			 const AList<TWireHit> & stereoHits) {
...
void
TFastFinder::selectSimpleSegments(const AList<TWireHit> & in,
				  AList<TWireHit> & out) const {
    AList<TWireHit> hits = in;
    while (hits.last()) {
	TWireHit & h = * hits.last();

	//...Start clustering...
	// AList<Belle2::TRGCDCWireHit> & cluster = * new AList<Belle2::TRGCDCWireHit>();
	AList<TWireHit> cluster;
	AList<TWireHit> toBeChecked;
	bool ok = true;
	toBeChecked.append(h);
	while (toBeChecked.length()) {
	    TWireHit & a = * toBeChecked.last();
		...
		if ((ptn >> i) % 2) {
		    const TWireHit & b = * a.wire().neighbor(i)->hit();
		    toBeChecked.append((TWireHit &) b);
		}
	    }
	}
