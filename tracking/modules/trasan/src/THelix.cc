//
// $Id: THelix.cc 10002 2007-02-26 06:56:17Z katayama $
//
// $Log$
...
//  const double
//  THelix::m_BFIELD = 15.0;            // KG
//  const double
//  THelix::m_ALPHA = 222.376063;       // = 10000. / 2.99792458 / BFIELD
  ...
  const double
  THelix::ConstantAlpha = 222.376063;

  const std::string THelix::invalidhelix("Invalid THelix");
  CLHEP::HepVector THelix::ms_amin(5, 0), THelix::ms_amax(5, 0);
  bool THelix::ms_check_range(false);
  bool THelix::ms_throw_exception(false);
  bool THelix::ms_print_debug(false);

  bool THelix::set_exception(bool t)
  {
    return ms_throw_exception = t;
  }

  bool THelix::set_print(bool t)
  {
    return ms_print_debug = t;
  }


  void THelix::set_limits(const CLHEP::HepVector& a_min, const CLHEP::HepVector& a_max)
  {
  ...
  THelix::THelix(const HepGeom::Point3D<double>  & pivot,
  ...
  THelix::THelix(const HepGeom::Point3D<double>  & position,
  ...
  double*
  THelix::x(double phi, double p[3]) const
  {
    DEBUG_HELIX;
    //
    // Calculate position (x,y,z) along helix.
  ...
  HepGeom::Point3D<double>
  THelix::x(double phi, CLHEP::HepSymMatrix& Ex) const
  {
    DEBUG_HELIX;
  ...
  CLHEP::Hep3Vector
  THelix::momentum(double phi) const
  {
    DEBUG_HELIX;
    //
    // Calculate momentum.
  ...
  CLHEP::Hep3Vector
  THelix::momentum(double phi, CLHEP::HepSymMatrix& Em) const
  {
    DEBUG_HELIX;
    //
    // Calculate momentum.
  ...
  THelix::momentum(double phi, double mass) const
  {
    DEBUG_HELIX;
    //
    // Calculate momentum.
  ...
  CLHEP::HepLorentzVector
  THelix::momentum(double phi, double mass, CLHEP::HepSymMatrix& Em) const
  {
    DEBUG_HELIX;
    //
    // Calculate momentum.
  ...
  THelix::pivot(const HepGeom::Point3D<double>  & newPivot)
  {
    DEBUG_HELIX;
  ...
  void
  THelix::set(const HepGeom::Point3D<double>  & pivot,
              const CLHEP::HepVector& a,
              const CLHEP::HepSymMatrix& Ea)
  {
    m_pivot = pivot;
  ...
  THelix&
  THelix::operator = (const THelix& i)
  {
  ...
  CLHEP::HepMatrix
  THelix::delApDelA(const CLHEP::HepVector& ap) const
  {
    DEBUG_HELIX;
    //
...
//cnv    double tnlp  = ap[4];
  ...
  CLHEP::HepMatrix
  THelix::delXDelA(double phi) const
  {
    DEBUG_HELIX;
    //
...
//cnv    const double & dz      = m_ac[3];
  ...
  CLHEP::HepMatrix
  THelix::delMDelA(double phi) const
  {
    DEBUG_HELIX;
    //
  ...
  CLHEP::HepMatrix
  THelix::del4MDelA(double phi, double mass) const
  {
    DEBUG_HELIX;
  ...
  CLHEP::HepMatrix
  THelix::del4MXDelA(double phi, double mass) const
  {
    DEBUG_HELIX;
...
//cnv    const double & dz      = m_ac[3];
  ...
  THelix::debugPrint(void) const
  {
    ...
    if (ms_print_debug) {
      std::cout << "THelix::dr = " << dr << " phi0 = " << phi0 << " cpa = " << cpa
...
//cnv  const double acpa   = abs(m_a[2]);
  ...
  void THelix::debugTHelix(void) const
  {
    if (!m_helixValid) {
