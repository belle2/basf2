...
void
THoughFinder::selectGoodHits(const CAList<TWireHit> & axial,
    ...
    for (unsigned i = 0; i < na; i++) {
        if (i < na - 1) {
            TLink& t = * _axial[i];
            TLink& u = * _axial[i + 1];
            const TWire & wt = * t.wire();
            const TWire & wu = * u.wire();
...
THoughFinder::goodTrackLowPt(const TTrack& t) const {
    ...
    //...Require at least two hits per axial super layer...
    static const unsigned nsl = TCDC::getTCDC()->nSuperLayers();
    static unsigned* nHits = new unsigned[nsl];
//  unsigned nHits[11];
...
//     static AList<TLink> * hits = new AList<TLink>[nsl];
    const unsigned nsl = TCDC::getTCDC()->nSuperLayers();
...
int
THoughFinder::doit(const CAList<TWireHit> & axialHits,
...
int
THoughFinder::doit1(const CAList<TWireHit> & axialHits,
		    const CAList<TWireHit> & stereoHits,
		    AList<TTrack> & tracks,
		    AList<TTrack> & tracks2D) {
...
//  AList<TLink> superLayerHits[11];
	...
	new AList<TLink>[TCDC::getTCDC()->nSuperLayers()];
...
int
THoughFinder::doit0(const CAList<TWireHit> & axialHits,
		    const CAList<TWireHit> & stereoHits,
		    AList<TTrack> & tracks,
		    AList<TTrack> & tracks2D) {
...
int
THoughFinder::doit2(const CAList<TWireHit> & axialHits,
                    const CAList<TWireHit> & stereoHits,
                    AList<TTrack> & tracks,
                    AList<TTrack> & tracks2D) {
...
int
THoughFinder::doit3(const CAList<TWireHit> & axialHits,
                    const CAList<TWireHit> & stereoHits,
                    AList<TTrack> & tracks,
                    AList<TTrack> & tracks2D) {
