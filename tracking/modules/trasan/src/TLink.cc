...
//  }
    ...
    //...Parent...
    if (mc) {
// 	std::vector<const TTrackMC *> list = TTrackMC::list();
// 	if (! list.size()) return;
// 	std::cout << pre;
// //cnv unsigned nMC = 0;
// 	for (unsigned i = 0; i < MCC_MAX; i++) {
// 	    if (MCC0[i] > 0) {
// 		const TTrackMC * h = list[i];
...
TLink::width(const AList<TLink> & list) {
    ...
    const TWire * const w0 = list[0]->wire();
...
//     dump(list);
	...
	const TWire * const w = list[i]->wire();
...
//      perror("$Id: TLink.cc 11153 2010-04-28 03:36:53Z yiwasaki $:N:malloc");
	...
	const TTrackMC & hep = * l.hit()->mc()->hep();
...
//  unsigned n[11];
    static unsigned * n = new unsigned[TCDC::getTCDC()->nSuperLayers()];
