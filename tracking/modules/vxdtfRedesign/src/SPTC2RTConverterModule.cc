...
SPTC2RTConverterModule::SPTC2RTConverterModule() : Module()
{
  ...
  addParam("svdHitsStoreArrayName", m_param_svdHitsStoreArrayName, "StoreArray name of the related svd hits.",
           std::string(""));
  addParam("vtxHitsStoreArrayName", m_param_vtxHitsStoreArrayName, "StoreArray name of the related vtx hits.",
           std::string(""));
  ...
  addParam("svdClustersName", m_param_svdClustersName, "StoreArray name of SVDClusters related to SpacePoints.",
           std::string(""));
  addParam("vtxClustersName", m_param_vtxClustersName, "StoreArray name of VTXClusters related to SpacePoints.",
           std::string(""));
...
void SPTC2RTConverterModule::initialize()
{
  ...
  // FIXME (benni) As far as i can see, the argument strings input to RecoTrack::registerRequiredRelations
  ...
  StoreArray<VTXCluster> VTXClusters; VTXClusters.isOptional(m_param_vtxClustersName);
}
...
void SPTC2RTConverterModule::createRecoTrack(const SpacePointTrackCand& spacePointTC)
{
  ...
  for (auto spacePoint : spacePointTC.getSortedHits()) {
    ...
    } else if (detID == VXD::SensorInfoBase::VTX) {
      RelationVector<VTXCluster> relatedClusters = spacePoint->getRelationsTo<VTXCluster>(m_param_vtxClustersName);
      B2DEBUG(20, "SPTC2RTConverter::event: Number of related VTX Clusters: " << relatedClusters.size());
      // relatedClusters should only contain 1 cluster for vtxHits. Loop over them to be robust against missing relations.
      for (const VTXCluster& cluster : relatedClusters) {
        newRecoTrack->addVTXHit(&cluster, sortingParameter, Belle2::RecoHitInformation::c_VTXTrackFinder);
        sortingParameter++;
      }
