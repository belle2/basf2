...
SectorMapBootstrapModule::bootstrapSectorMap(const SectorMapConfig& config)
{
  ...
  for (VxdID aSensorId : listOfSensors) {
    ...
    // for testbeams there might be other sensors in the geometry so filter for SVD, PXD and VTX only, as the CompactSecID dont like those!
    VXD::SensorInfoBase::SensorType type = geometry.getSensorInfo(aSensorId).getType();
    if (type != VXD::SensorInfoBase::SVD && type != VXD::SensorInfoBase::PXD && type != VXD::SensorInfoBase::VTX) {
      B2WARNING("Found sensor which is not PXD, SVD or VTX with VxdID: " << aSensorId << " ! Will skip that sensor ");
      continue;
    }
