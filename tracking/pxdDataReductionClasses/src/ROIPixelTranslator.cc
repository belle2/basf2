...
ROIPixelTranslator::ROIPixelTranslator(double user_sigmaSystU, double user_sigmaSystV, double user_numSigmaTotU, double user_numSigmaTotV, double user_maxWidthU, double user_maxWidthV)
                                  ...
                                  StoreArray<ROIid>* ROIidList)
{
  ...
  for (int i = 0; i < listOfIntercepts->getEntries(); i++) {
    ...
    B2DEBUG(2, "  widthU = " << maxU - minU
            << "  minU = "  << minU
            << "  maxU = "  << maxU
            << "  lengthU = " << aSensorInfo.getUSize((*listOfIntercepts)[i]->getCoorV())
           );

    B2DEBUG(2, "  widthV = " << maxV - minV
            << "  minV = " << minV
            << "  maxV = " << maxV
            << "  lengthV = " << aSensorInfo.getVSize());

    B2DEBUG(2, "  bottom left pixel (U,V) = (" << bottomLeft_uID << "," << bottomLeft_vID << ")");
    B2DEBUG(2, "  top right pixel (U,V) = (" << topRight_uID << "," << topRight_vID << ")");
    ...
    if (bottomLeft_uID > nPixelsU || topRight_uID < firstPixelID || bottomLeft_vID > nPixelsV || topRight_vID < firstPixelID) {
      B2DEBUG(1, "  OOOPS: this pixel does NOT belong to the sensor");
      inside = false;
    }
