...
"""
combined_to_pxd_ckf_mva_training
-----------------------------------------
...
the three MVA-based state filters and one result filter of the ToPXDCKF.
This CKF extraplates tracks found in CDC and SVD into the PXD and adds PXD hits
using a combinatorial tree search and a Kalman filter based track fit in each step.
...
You can test the b2luigi without running it via::
    ...
    python3 combined_to_pxd_ckf_mva_training.py --dry-run
    python3 combined_to_pxd_ckf_mva_training.py --show-output
...
run::
    ...
    python3 combined_to_pxd_ckf_mva_training.py
...
Then, execute your steering (e.g. in another terminal) with::
    ...
    python3 combined_to_pxd_ckf_mva_training.py --scheduler-port 8886
...
class ResultRecordingTask(Basf2PathTask, LSFTask):
    ...
    def create_result_recording_path(self, result_filter_records_name):
        ...
        write_tracking_mva_filter_payloads_to_db(
            ...
            iov,
            f"trk_ToPXDStateFilter_1{fast_bdt_string}",
            0.01)

        write_tracking_mva_filter_payloads_to_db(
            f"trk_ToPXDStateFilter_2_Parameter{fast_bdt_string}",
            iov,
            f"trk_ToPXDStateFilter_2{fast_bdt_string}",
            0.01)

        write_tracking_mva_filter_payloads_to_db(
            f"trk_ToPXDStateFilter_3_Parameter{fast_bdt_string}",
            iov,
        ...
        first_high_filter_parameters = {"DBPayloadName": f"trk_ToPXDStateFilter_1_Parameter{fast_bdt_string}",
                                        "direction": "backward"}
        second_high_filter_parameters = {"DBPayloadName": f"trk_ToPXDStateFilter_2_Parameter{fast_bdt_string}"}
        third_high_filter_parameters = {"DBPayloadName": f"trk_ToPXDStateFilter_3_Parameter{fast_bdt_string}"}
...
class ValidationAndOptimisationTask(Basf2PathTask, LSFTask):
    ...
    def output(self):
        ...
        yield self.add_to_output(
            f"to_pxd_ckf_validation{fbdt_state_filter_string}{fbdt_result_filter_string}.root")
    ...
    def create_optimisation_and_validation_path(self):
        ...
        write_tracking_mva_filter_payloads_to_db(
            ...
            iov,
            f"trk_ToPXDStateFilter_1{fbdt_state_filter_string}",
            self.state_filter_cut)

        write_tracking_mva_filter_payloads_to_db(
            f"trk_ToPXDStateFilter_2_Parameter{fbdt_state_filter_string}",
            iov,
            f"trk_ToPXDStateFilter_2{fbdt_state_filter_string}",
            self.state_filter_cut)

        write_tracking_mva_filter_payloads_to_db(
            f"trk_ToPXDStateFilter_3_Parameter{fbdt_state_filter_string}",
            iov,
            f"trk_ToPXDStateFilter_3{fbdt_state_filter_string}",
            self.state_filter_cut)

        write_tracking_mva_filter_payloads_to_db(
            f"trk_ToPXDResultFilter_Parameter{fbdt_result_filter_string}",
            iov,
        ...
        path.add_module('RelatedTracksCombiner',
                        VXDRecoTracksStoreArrayName="PXDRecoTracks",
                        CDCRecoTracksStoreArrayName="CDCSVDRecoTracks",
        ...
        path.add_module("MCRecoTracksMatcher", UsePXDHits=True, UseSVDHits=True, UseCDCHits=True,
                        mcRecoTracksStoreArrayName="MCRecoTracks",
