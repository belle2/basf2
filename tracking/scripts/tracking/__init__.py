...
def add_prefilter_tracking_reconstruction(path, components=None, skipGeometryAdding=False,
    ...
    if not is_any_detector_present(["SVD", "CDC", "VTX"], components):
        return
...
def add_time_extraction(path, append_full_grid_cdc_eventt0=False, components=None,
    ...
    if is_detector_present("CDC", components) and (append_full_grid_cdc_eventt0 or useVTX):
...
def add_track_finding(path, components=None, reco_tracks="RecoTracks",
    ...
    if not is_detector_present("PXD", components) and not useVTX:
    ...
    elif useVTX and is_detector_present("VTX", components):
        if use_ecl_to_cdc_ckf and is_detector_present("CDC", components):
            combined_ecl_reco_tracks = reco_tracks
        elif (not use_ecl_to_cdc_ckf):
            vtx_cdc_reco_tracks = reco_tracks
    elif useVTX and not is_detector_present("VTX", components) and is_detector_present("CDC", components):
        cdc_reco_tracks = reco_tracks
    ...
    if not inverted_tracking:
        ...
        elif is_detector_present("VTX", components) and useVTX:
            latest_reco_tracks, tmp_reco_track_list = \
    ...
    if is_detector_present("PXD", components) and not useVTX:
        """
...
def add_cr_track_finding(path, reco_tracks="RecoTracks", components=None,
    ...
    vtx_cdc_reco_tracks = "VTXCDCRecoTracks"
    if useVTX and not is_detector_present("VTX", components):
    ...
    if is_detector_present("VTX", components) and useVTX:
...
def add_mc_track_finding(path, components=None, reco_tracks="RecoTracks", use_second_cdc_hits=False):
    ...
    :param use_second_cdc_hits: If true, the second hit information will be used in the CDC track finding.
    """
    if is_any_detector_present(["VTX", "PXD", "SVD", "CDC"], components):
