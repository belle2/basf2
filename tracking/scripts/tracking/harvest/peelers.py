...
def peel_reco_track_hit_content(reco_track, key="{part_name}"):
    ...
    if reco_track:
        ...
        n_vtx_hits = reco_track.getNumberOfVTXHits()
        ndf = 2 * n_pxd_hits + n_svd_hits + n_cdc_hits + 2 * n_vtx_hits
        ...
        vtx_hits = [hit.getSensorID().getLayerNumber() for hit in getObjectList(reco_track.getVTXHitList())]
        if vtx_hits:
            first_vtx_layer = min(vtx_hits)
            last_vtx_layer = max(vtx_hits)
...
def peel_event_level_tracking_info(event_level_tracking_info, key="{part_name}"):
    ...
    return dict(has_vxdtf2_failure_flag=event_level_tracking_info.hasVXDTF2AbortionFlag(),
                ...
                has_vtxckf_failure_flag=event_level_tracking_info.hasVTXCKFAbortionFlag(),
...
def peel_quality_indicators(reco_track, key="{part_name}"):
    ...
    if reco_track:
        ...
        if not cdc_track_cand:
            svd_cdc_track_cand = reco_track.getRelated('SVDCDCRecoTracks')
            vtx_cdc_track_cand = reco_track.getRelated('VTXCDCRecoTracks')
            if svd_cdc_track_cand:
            ...
            elif vtx_cdc_track_cand:
                cdc_track_cand = vtx_cdc_track_cand.getRelated('CDCRecoTracks')
                if not cdc_track_cand:
                    cdc_track_cand = vtx_cdc_track_cand.getRelated('CKFCDCRecoTracks')
                if not cdc_track_cand:
                    temp_cdc_track_cand = vtx_cdc_track_cand.getRelated('VTXPlusCDCStandaloneRecoTracks')
                    if temp_cdc_track_cand:
                        cdc_track_cand = temp_cdc_track_cand.getRelated('CDCRecoTracks')
        ...
        space_point_track_cand_vtx = reco_track.getRelated('SPTrackCands')

        # adjust relations if VTX->CDC CKF enabled
        if not space_point_track_cand_vtx:
            vtx_track_cand = reco_track.getRelated('VTXRecoTracks')
            if not vtx_track_cand:
                vtx_cdc_track_cand = reco_track.getRelated('VTXCDCRecoTracks')
                if vtx_cdc_track_cand:
                    vtx_track_cand = vtx_cdc_track_cand.getRelated('VTXRecoTracks')
                    if not vtx_track_cand:
                        temp_vtx_track_cand = vtx_cdc_track_cand.getRelated('VTXPlusCDCStandaloneRecoTracks')
                        if temp_vtx_track_cand:
                            vtx_track_cand = temp_vtx_track_cand.getRelated('VTXRecoTracks')
            if vtx_track_cand:
                space_point_track_cand_vtx = vtx_track_cand.getRelated('SPTrackCands')

        if space_point_track_cand_vtx:
            vtx_qi = space_point_track_cand_vtx.getQualityIndicator()

    crops = dict(
        quality_indicator=qi,
        svd_quality_indicator=svd_qi,
        cdc_qualityindicator=cdc_qi,
        vtx_qualityindicator=vtx_qi,
    )
...
def peel_trackfinder(reco_track, key="{part_name}"):
    ...
    used_VTXTrackFinder = False
    used_VTXtoCDCCKF = False
    used_CDCtoVTXCKF = False
    ...
    if reco_track:
        ...
        if reco_track.getNumberOfCDCHits() > 0:
            ...
            used_VTXtoCDCCKF = cdc_tf == Belle2.RecoHitInformation.c_VTXtoCDCCKF

        if reco_track.getNumberOfVTXHits() > 0:
            info = get_reco_hit_information(reco_track, reco_track.getVTXHitList()[0])
            vtx_tf = info.getFoundByTrackFinder()
            used_VTXTrackFinder = vtx_tf == Belle2.RecoHitInformation.c_VTXTrackFinder
            used_CDCtoVTXCKF = vtx_tf == Belle2.RecoHitInformation.c_CDCtoVTXCKF
    ...
    crops = dict(
        ...
        foundby_VTXTrackFinder=used_VTXTrackFinder,
        foundby_VTXtoCDCCKF=used_VTXtoCDCCKF,
        foundby_CDCToVTXCKF=used_CDCtoVTXCKF,
    )
...
def peel_subdetector_hit_efficiency(mc_reco_track, reco_track, key="{part_name}"):
    ...
    return dict(**get_efficiency("CDC"), **get_efficiency("SVD"), **get_efficiency("PXD"), **get_efficiency("VTX"))
