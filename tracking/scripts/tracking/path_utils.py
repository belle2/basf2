...
from pybasf2 import B2WARNING, B2ERROR, B2FATAL
...
def add_hit_preparation_modules(path, components=None, pxd_filtering_offline=False,
    ...
    if is_detector_present("SVD", components) and not useVTX:
        add_svd_reconstruction(path)

    # Preparation of the PXD clusters
    if is_detector_present("PXD", components) and not pxd_filtering_offline and not create_intercepts_for_pxd_ckf and not useVTX:
...
def add_mc_matcher(path, components=None, mc_reco_tracks="MCRecoTracks",
    ...
    if (matching_method == "hit"):
        path.add_module('TrackFinderMCTruthRecoTracks',
                        ...
                        UseVTXHits=is_detector_present("VTX", components),
...
def add_prune_tracks(path, components=None, reco_tracks="RecoTracks"):
    ...
    if components and not is_any_detector_present(["SVD", "CDC", "VTX"], components):
        return
...
def add_default_cdc_vtx_tracking_chain(path,
                                       components,
                                       vtx_reco_tracks,
                                       cdc_reco_tracks,
                                       output_reco_tracks,
    ...
    """
    Add the default CDC based tracking chain to the path, i.e. CDC standalone followed by the ToVTXSpacePointCKF, the VTX standalone
    track finding, a CKF based merger for standalone tracks, and finally the VTXToCDCCKF (if setup as such).

    :param path: The path to add the tracking reconstruction modules to
    :param components: the list of geometry components in use or None for all components.
    :param vtx_reco_tracks: name of the VTX standalone RecoTracks StoreArray
    :param cdc_reco_tracks: name of the CDC standalone RecoTracks StoreArray
    :param output_reco_tracks: name of the combined CDC+VTX RecoTracks StoreArray that is the final result of this tracking path
    :param with_cdc_cellular_automaton: If true, in the CDC track finding the cellular automaton algorithm will be used too,
        after the global algorithm (Legendre)
    :param use_second_cdc_hits: whether to use the secondary CDC hit during CDC track finding or not
    :param add_cdcTrack_QI: If true, add the MVA track quality estimation
        to the path that sets the quality indicator property of the found CDC standalone tracks
    :param use_mc_truth: Use the truth information in the CKF modules
    :param VTX_ckf_mode: how to apply the CKF (with or without VTX standalone tracking). Defaults to "VXDTF2_after".
    :param add_both_directions: Curlers may be found in the wrong orientation by the CDC track finder, so try to
           extrapolate also in the other direction.
    ...
    if is_vtx_used(components):
        add_vtx_track_finding(path,
                              components=components,
                              input_reco_tracks=latest_reco_tracks,
                              output_reco_tracks=output_reco_tracks,
                              use_mc_truth=use_mc_truth,
