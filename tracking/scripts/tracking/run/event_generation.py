...
def setup_tracking_detector_vtx(path):
    components = ["BeamPipe", "VTX", "CDC", "MagneticField"]
    override = [
        ("/DetectorComponent[@name='MagneticField']//Component[@type='3d'][2]/ExcludeRadiusMax",
         "4.20", "m", )  # Remove the second compontent which is the magnetic field outside the tracking volumn.
    ]
...
def setup_tracking_detector_vtx_constant_b(path):
    components = ["BeamPipe", "VTX", "CDC", "MagneticFieldConstant4LimitedRCDC"]
    adjustments.adjust_module(path, "Geometry", components=components)
    return components
...
detector_setups_by_short_name = {
    ...
    'TrackingDetectorVTX': setup_tracking_detector_vtx,
    'TrackingDetectorVTXConstB': setup_tracking_detector_vtx_constant_b,
