  ...
  SpacePoint createSpacePoint(VxdID sensorId, bool pxd, short nClusters = 0, double u = 0.1, double v = 0.2)
  {
    if (pxd) {
      ...
      if (!nClusters) { // if 0 set both
        clusters.push_back(new SVDCluster(sensorId, true, u, 0.1, 0.001, 1, 1, 1, 1, 1.0));
        clusters.push_back(new SVDCluster(sensorId, false, v, 0.1, 0.001, 1, 1, 1, 1, 1.0));
      } else { // set only one coordinate
        bool setU = (nClusters > 0);
        clusters.push_back(new SVDCluster(sensorId, setU, u, 0.1, 0.001, 1, 1, 1, 1, 1.0)); // position doesnot actually matter!
      }
