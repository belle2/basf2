...
bool NewSegmentTruthVarSet::extract(const CDCRecoSegment2D* segment)
{
  ...
  var<named("track_is_alrady_found_truth")>() = trackIsAlreadyFound;
  var<named("segment_is_fake_truth")>() = segmentIsFake;
  var<named("truth")>() = not segmentIsFake and not trackIsAlreadyFound;
  return true;
}
