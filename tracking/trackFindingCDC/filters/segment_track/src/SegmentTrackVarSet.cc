...
bool SegmentTrackVarSet::extract(const std::pair<const CDCRecoSegment2D*, const CDCTrack*>* testPair)
{
  ...
  bool isAxialSegment = segment->getStereoType() != StereoType::c_Axial;

  for (const CDCRecoHit3D& recoHit : *track) {
    if (isAxialSegment and recoHit.getStereoType() == StereoType::c_Axial) {
    ...
    } else if (not isAxialSegment and recoHit.getStereoType() != StereoType::c_Axial) {
