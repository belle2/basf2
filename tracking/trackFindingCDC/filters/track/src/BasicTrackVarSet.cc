...
bool BasicTrackVarSet::extract(const CDCTrack* track)
{
  ...
  cont_layer_acc; /* continuous layer number [from 0 to 55] (not to be distinguished from SuperLayer [from 0 to 8] or Layer (within SuperLayer, so from [0 to 7]) */
  ...
  /*
  std::sort(cont_layer.begin(), cont_layer.end());
  std::sort(super_layer.begin(), super_layer.end());
  unsigned int unique_cont_layers = std::distance(cont_layer.begin(), std::unique(cont_layer.begin(), cont_layer.end())) + 1; // is this +1 needed? I am not so sure if distance gives the array size or just the size -1
  unsigned int unique_super_layers = std::distance(super_layer.begin(), std::unique(super_layer.begin(), super_layer.end())) + 1;
  */
