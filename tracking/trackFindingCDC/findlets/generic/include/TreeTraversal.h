...
namespace Belle2 {
  ...
  namespace TrackFindingCDC {
     ...
     * - AStateRejecter:
     *     Should be roughly of type Findlet<const AState* const, AState*>
     *     with an apply methode like:
     *
     *     apply(const std::vector<const AState*>& currentPath,
     *           std::vector<AState*>& nextStates);
     *
     *     Therefore it receives the current path and a list of potential next states,
     *     that it should sort out for viable continuations to traverse next.
     *
