...
namespace Belle2 {
  namespace TrackFindingCDC {
    ...
    public:
       ...
       */
      FastHough(bool reconstructCurler, int maxLevel, int nbinsTheta, double rMax);
      ...
      /** Provide set of hits */
      void setAxialHits(std::vector<TrackHit*>& hits) {m_hits = hits;};

      /** Provides pointer to list of track candidates */
      void initializeCandidatesVector(std::vector< std::pair<std::vector<TrackHit*>, std::pair<double, double> > >* candidates) {m_candidates = candidates;};
...
//    double m_rMin; /**< Minimum in r direction, initialized in initializer list of the module */
      ...
      std::vector<TrackHit*> m_hits; /**< Holds vector of hits */
      double Rcell; /**< Rough estimation of cell size */

      std::vector< std::pair<std::vector<TrackHit*>, std::pair<double, double> > >* m_candidates; /**< Holds list of track candidates */
