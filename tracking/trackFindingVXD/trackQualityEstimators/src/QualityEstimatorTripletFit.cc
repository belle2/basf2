...
double QualityEstimatorTripletFit::estimateQuality(std::vector<SpacePoint const*> const& measurements)
{
  ...
  for (int i = 0; i < nTriplets; i++) {
    ...
    // First get the orientation and average radlength of the sensor plate for material budged calculation
    double entranceAngle = theta;
    double averageRadLength = m_averageRadLength;
    double materialBudgetFactor = m_materialBudgetFactor;
    VxdID::baseType sensorID = measurements.at(i + 1)->getVxdID();
    int detID = measurements.at(i + 1)->getType();

    if (sensorID != 0) {
      const VXD::SensorInfoBase& sensor = VXD::GeoCache::getInstance().getSensorInfo(sensorID);
      ...
      if (detID == VXD::SensorInfoBase::VTX) {
        // FIXME: Hardcoded average radiation length for 7 layer
        // VTX. For 5 layer VTX, layer three should be 0.003 as well,
        // so this would be wrong right now. Better to read these numbers
        // from VTX::SensorInfo.
        if (VxdID(sensorID).getLayerNumber() <= 3) {
          averageRadLength = 0.001;
        } else {
          averageRadLength = 0.003;
        }
      }
    }

    const double XoverX0 = materialBudgetFactor * averageRadLength / cos(entranceAngle);
