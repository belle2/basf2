...
MeasurementAdder::MeasurementAdder(
  ...
  m_param_storeArrayNameOfVTXHits(storeArrayNameOfVTXHits),
...
void MeasurementAdder::createGenfitMeasurementFactory()
{
  ...
  StoreArray<RecoHitInformation::UsedVTXHit> vtxHits(m_param_storeArrayNameOfVTXHits);
  ...
  if (vtxHits.isOptional())
    m_genfitMeasurementFactory.addProducer(Const::VTX,
                                           new genfit::MeasurementProducer<RecoHitInformation::UsedVTXHit, VTXRecoHit>(vtxHits.getPtr()));
...
void MeasurementAdder::resetMeasurementCreatorsToDefaultSettings()
{
  ...
  m_vtxMeasurementCreators = { std::shared_ptr<VTXBaseMeasurementCreator>(new VTXCoordinateMeasurementCreator(m_genfitMeasurementFactory)) };
  ...
  const std::vector<std::shared_ptr<BaseMeasurementCreator>>& additionalMeasurementCreators)
{
  ...
  m_vtxMeasurementCreators = vtxMeasurementCreators;
  ...
  const std::map<std::string, std::map<std::string, std::string>>& additionalMeasurementCreators)
{
  ...
  VTXMeasurementCreatorFactory vtxMeasurementCreatorFactory(m_genfitMeasurementFactory);
  ...
  vtxMeasurementCreatorFactory.setParameters(vtxMeasurementCreators);
  ...
  vtxMeasurementCreatorFactory.initialize();
  ...
  m_vtxMeasurementCreators = vtxMeasurementCreatorFactory.getCreators();
...
bool MeasurementAdder::addMeasurements(RecoTrack& recoTrack) const
{
  ...
  B2ASSERT("Reco Track was built with another VTX store array: Reco Track "
           << recoTrack.getStoreArrayNameOfVTXHits()
           << ", this class " << m_param_storeArrayNameOfVTXHits,
           checkTwoStoreArrayNames<RecoHitInformation::UsedVTXHit>(recoTrack.getStoreArrayNameOfVTXHits(), m_param_storeArrayNameOfVTXHits));
  ...
  if (m_cdcMeasurementCreators.size() == 0 and m_svdMeasurementCreators.size() == 0 and m_pxdMeasurementCreators.size() == 0 and
      m_bklmMeasurementCreators.size() == 0 and m_eklmMeasurementCreators.size() == 0 and
      m_vtxMeasurementCreators.size() == 0 and m_additionalMeasurementCreators.size() == 0) {
  ...
  // Add the measurements created by the CDC, SVD, PXD and VTX measurement creators.
  ...
  recoTrack.mapOnHits<RecoHitInformation::UsedVTXHit>(m_param_storeArrayNameOfVTXHits, [&](RecoHitInformation & recoHitInformation,
  RecoHitInformation::UsedVTXHit * vtxHit) {
    addMeasurementsFromHitToRecoTrack<RecoHitInformation::UsedVTXHit, Const::VTX>(recoTrack, recoHitInformation, vtxHit,
        m_vtxMeasurementCreators, trackPointHitMapping);
  });
