                                           ...
                                           const bool useClosestHitToIP, const bool useBFieldAtHit)
{
  ...
  B2DEBUG(20, trackReps.size() << " track representations available.");
  ...
  for (const auto& trackRep : trackReps) {
    ...
    if (not Const::chargedStableSet.contains(particleType)) {
      B2DEBUG(20, "Track fitted with hypothesis that is not a ChargedStable (PDG code = " << particleType.getPDGCode() << ")");
      continue;
    }

    // Check if the fit worked.
    if (not recoTrack.wasFitSuccessful(trackRep)) {
      B2DEBUG(20, "The fit with the given track representation (" << std::abs(trackRep->getPDG()) <<
    ...
    try {
      if (useClosestHitToIP) {
        const genfit::MeasuredStateOnPlane* msopPtr = recoTrack.getMeasuredStateOnPlaneClosestTo(TVector3(0, 0, 0), trackRep);
        if (msopPtr) {
          msop = *msopPtr;
        } else {
          B2WARNING("No StateOnPlane found. Will skip current RecoTrack.");
          continue;
        }
      } else {
    ...
    B2DEBUG(24, "Point of closest approach: " << poca.x() << "  " << poca.y() << "  " << poca.z());
    B2DEBUG(24, "Track direction in POCA: " << dirInPoca.x() << "  " << dirInPoca.y() << "  " << dirInPoca.z());
  ...
  B2DEBUG(20, "Number of fitted hypothesis = " << newTrack.getNumberOfFittedHypotheses());
  if (newTrack.getNumberOfFittedHypotheses() > 0) {
    ...
    if (mcParticle) {
      B2DEBUG(29, "Relation to MCParticle set.");
      addedTrack->addRelationTo(mcParticle, mcParticleWithWeight.second);
    } else {
      B2DEBUG(29, "Relation to MCParticle not set. No related MCParticle to RecoTrack.");
    }
    return true;
  } else {
    B2DEBUG(29, "Relation to MCParticle not set. No related MCParticle to RecoTrack.");
...
uint32_t TrackBuilder::getHitPatternVXDInitializer(const RecoTrack& recoTrack)
{
  ...
  if (nNotFittedVXDhits > 0) {
    B2DEBUG(20, " No KalmanFitterInfo associated to some TrackPoints with VXD hits, not filling the HitPatternVXD");
    B2DEBUG(20, nNotFittedVXDhits << " had no FitterInfo");
  }
...
uint64_t TrackBuilder::getHitPatternCDCInitializer(const RecoTrack& recoTrack)
{
  ...
  if (nNotFittedCDChits > 0) {
    B2DEBUG(20, " No KalmanFitterInfo associated to some TrackPoints with CDC hits, not filling the HitPatternCDC");
    B2DEBUG(20, nNotFittedCDChits << " out of " << nCDChits << " had no FitterInfo");
  }
