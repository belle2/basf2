...
/*
<header>
<input>VXDCDCMergerEvtGenTruthFinder.root</input>
<output>VXDCDCMergerEvtGenTruthFinderHist.root</output>
<contact>tracking@belle2.kek.jp</contact>
</header>
*/

void VXDCDCMergerEvtGenTruthFinder(){

  TString dataobj = "$BELLE2_LOCAL_DIR/lib/$BELLE2_SUBDIR/libdataobjects.so";  
  gROOT->LoadMacro(gSystem->ExpandPathName(dataobj.Data()));

  TFile* inputFile = TFile::Open("../VXDCDCMergerEvtGenTruthFinder.root");
  TTree* tree = (TTree*) inputFile->Get("Stats");

  // write output file for the validation histograms
  TFile* output = TFile::Open("VXDCDCMergerEvtGenTruthFinderHist.root", "recreate");
 ...
 /////////////SCANS//////////////
  ...
  h22->Sumw2();
  TH1F *eff_ptEGtt = new TH1F("eff_ptEGtt","Eff vs Pt EvtGen Truth Track Finder",100,0.,3.);
  tree->Draw("TMath::Sqrt((Px*Px+Py*Py))>>eff_ptEGtt","TruthTag==1&&GoodTag==1");
  ...
  eff_ptEGtt->GetListOfFunctions()->Add(new TNamed("Description","Efficiency vs Pt")); 
  eff_ptEGtt->GetListOfFunctions()->Add(new TNamed("Check","Should be -> 1 above 0.3 GeV"));
  eff_ptEGtt->GetListOfFunctions()->Add(new TNamed("Contact","tracking@belle2.kek.jp")); 
  eff_ptEGtt->GetXaxis()->SetTitle("Pt (GeV)");
  eff_ptEGtt->GetYaxis()->SetTitle("Efficiency");
  eff_pt=eff_ptEGtt;
  eff_pt->Divide(eff_ptEGtt, h22, 1.0, 1.0, "B");
  ...
  h24->Sumw2();
  TH1F *eff_thetaEGtt = new TH1F("eff_thetaEGtt","Eff vs Theta EvtGen Truth Track Finder",50, -1, 1.);
  eff_thetaEGtt->SetMaximum(1.1);
  tree->Draw("(Pz/(P))>>eff_thetaEGtt","TruthTag==1&&GoodTag==1");
  eff_thetaEGtt->Sumw2();
  gPad->RedrawAxis();
  ...
  eff_thetaEGtt->GetXaxis()->SetTitle("Polar Angle (rad)");
  eff_thetaEGtt->GetYaxis()->SetTitle("Efficiency");
  eff_thetaEGtt->GetListOfFunctions()->Add(new TNamed("Description","Efficiency vs thetaEG")); 
  eff_thetaEGtt->GetListOfFunctions()->Add(new TNamed("Check","Should be (ideally) flat"));
  eff_thetaEGtt->GetListOfFunctions()->Add(new TNamed("Contact","tracking@belle2.kek.jp")); 
  eff_theta=eff_thetaEGtt;
  eff_theta->Divide(eff_thetaEGtt, h24, 1.0, 1.0, "B");
  //TPaveStats *st = (TPaveStats*)h->FindObject("stats");
  //st->SetX1NDC(0.7); //new x start position
  //st->SetX2NDC(0.1); //new x end position
  //gPad->SetOptStat(1)
