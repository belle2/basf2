...
void VXDCDCMergerSinglePart(){
 ...
 /////////////SCANS//////////////
  ...
  TH1F *h22 = new TH1F("h22","Eff vs Pt VXDTF+Trasan",100,0.,5.);
  h22->SetMaximum(1.1);
  tree->Draw("TMath::Sqrt((Px*Px+Py*Py))>>h22","TruthTag==1");
  h22->Sumw2();
  TH1F *eff_thetasp = new TH1F("eff_thetasp","Eff vs Pt",100,0.,5.);
  tree->Draw("TMath::Sqrt((Px*Px+Py*Py))>>eff_thetasp","TruthTag==1&&GoodTag==1");
  ...
  //eff_thetasp->Draw("E");
  TH1F *eff_pt = new TH1F("eff_pt", "Eff vs Pt VXDTF+Trasan",100,0.,5.);
  ...
  eff_pt=eff_thetasp;
  eff_pt->Divide(eff_thetasp, h22, 1.0, 1.0, "B");
  ...
  h24->Sumw2();
  TH1F *eff_ptsp = new TH1F("eff_ptsp","Eff vs Theta VXDTF+Trasan",50, -1, 1.);
  eff_ptsp->SetMaximum(1.1);
  tree->Draw("(Pz/(P))>>eff_ptsp","TruthTag==1&&GoodTag==1");
  eff_ptsp->Sumw2();
  gPad->RedrawAxis();
  //eff_ptsp->Divide(h24);
  //eff_ptsp->GetYaxis()->SetRange(0,2);
  //eff_ptsp->Draw("E");
  TH1F *eff_theta = new TH1F("eff_theta", "Eff vs Theta VXDTF+Trasan", 50, -1, 1.);
  ...
  eff_theta=eff_ptsp;
  eff_theta->Divide(eff_ptsp, h24, 1.0, 1.0, "B");
  //TPaveStats *st = (TPaveStats*)h->FindObject("stats");
  //st->SetX1NDC(0.7); //new x start position
  //st->SetX2NDC(0.1); //new x end position
  //gPad->SetOptStat(1)
