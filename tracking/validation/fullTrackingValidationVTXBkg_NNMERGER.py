...
<header>
  ...
  <output>VTXFullTrackingValidationBkg_NNMERGER.root</output>
...
import basf2
VALIDATION_OUTPUT_FILE = 'VTXFullTrackingValidationBkg_NNMERGER.root'
N_EVENTS = 1000
ACTIVE = True
...
def setupFinderModule(path):
    tracking.add_tracking_reconstruction(
        ...
        use_nn_vtx_cdc_merger=True,
        use_mc_vtx_cdc_merger=False)


class VTXFullTrackingValidationBkg_NNMERGER(TrackingValidationRun):
...
def main():
    validation_run = VTXFullTrackingValidationBkg_NNMERGER()
