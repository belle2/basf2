...
namespace Belle2 {
  ...
  public:
    ...
    VXDSector(): m_sectorID(0), m_distance2Origin(0), m_useDistance4sort(false) { m_friends.clear(); }



    /** useful constructor for cases where sectors are treated by fullSecID (parameter 1) */
    VXDSector(unsigned int secID): m_sectorID(secID), m_distance2Origin(0), m_useDistance4sort(false) { m_friends.clear(); } // default values
...
//      B2WARNING("'>'-sort of VXDSector using distance2origin");
    ...
    /** getter - returns vector of addresses of friends stored in friendMap */
    const std::vector<VXDSector*>& getFriendPointers() const { return m_friendPointers; }



    /** getter - returns friendMap.size() */
    int getFriendMapSize() const { return m_friendMap.size(); } // TODO long term: replace by vector of iterators for that map (faster)
