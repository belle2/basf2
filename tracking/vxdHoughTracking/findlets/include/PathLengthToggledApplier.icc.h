...
namespace Belle2 {
  namespace vxdHoughTracking {
    ...
    {
      m_twoHitFilterFindlet.exposeParameters(moduleParamList, TrackingUtilities::prefixed(prefix, "twoHit"));
      m_threeHitFilterFindlet.exposeParameters(moduleParamList, TrackingUtilities::prefixed(prefix, "threeHit"));
      m_fourHitFilterFindlet.exposeParameters(moduleParamList, TrackingUtilities::prefixed(prefix, "fourHit"));
      m_fiveHitFilterFindlet.exposeParameters(moduleParamList, TrackingUtilities::prefixed(prefix, "fiveHit"));
    }

    /// The weight is calculated using the subfilter based on the geometrical layer of the state.
    template <class AHit, class AFindlet>
    void PathLengthToggledApplier<AHit, AFindlet>::apply(const std::vector<TrackingUtilities::WithWeight<const AHit*>>& currentPath,
