...
namespace Belle2{
  ...
  int TRGCDCJLUT::setFloatFunction(function<double(double)> function,
    ...
    double tt_factor = log((max(t_outputRealMaxInt, abs(t_outputRealMinInt)) - t_outputRealMinInt)/((pow(2,lutOutputBitwidth)-0.5)*t_outputToReal))/log(2);
    ...
    Belle2::TRGCDCJSignal t_shiftOutputMin = Belle2::TRGCDCJSignal(t_int, t_toReal, t_int, t_int, t_actual, t_actual, t_actual, -1, t_commonData);
    ...
    Belle2::TRGCDCJSignal t_shiftOffsetOutputMax = Belle2::TRGCDCJSignal(t_int, t_toReal, t_int, t_int, t_actual, t_actual, t_actual, -1, t_commonData);
  ...
  TRGCDCJSignal const TRGCDCJLUT::operate(TRGCDCJSignal const& in, TRGCDCJSignal &out) {
    ...
    else m_shiftOffsetInput.assignTo(Belle2::TRGCDCJSignal::toSlv(t_offsetInput), t_offsetInput.calFinishClock(), t_shiftOffsetInputCode);
    ...
    Belle2::TRGCDCJSignal t_offsetOutput(intOutput, m_shiftOutputMin.getToReal(), 0, m_shiftOffsetOutputMax.getInt(), t_actual, 0, m_shiftOffsetOutputMax.getActual(), t_finishClock, t_commonData, 1);
