...
namespace Belle2{
  ...
  TRGCDCJSignal::TRGCDCJSignal(int const & bitwidth, double const & value, double const & minValue, double const & maxValue, int const & clock, TRGCDCJSignalData * commonData){
    m_bitsize = bitwidth;
  ...
  TRGCDCJSignal::TRGCDCJSignal(signed long long const & intValue, double const & toReal, signed long long const & minInt, signed long long const & maxInt, double const & actual, double const & minActual, double const & maxActual, int const & finishClock, TRGCDCJSignalData * commonData, bool b_slv){
    m_int = intValue;
    m_minInt = minInt;
    m_maxInt = maxInt;
    m_actual = actual;
  ...
  void TRGCDCJSignal::choose(TRGCDCJSignal & target, TRGCDCJSignal const & targetMin, TRGCDCJSignal const & targetMax, TRGCDCJSignal const & reference, std::vector<std::vector<TRGCDCJSignal> > data, int targetClock){
    ...
    for(unsigned iCondition=0; iCondition<data.size(); iCondition++){
      ...
      // Change blank signal constructors to min and max.
        ...
        if(from->m_bitsize==-1){
          (*from) = TRGCDCJSignal(t_reference.m_minInt, t_reference.m_toReal, t_reference.m_minInt, t_reference.m_minInt, t_reference.m_minActual, t_reference.m_minActual, t_reference.m_minActual, -1, t_reference.m_commonData);
        }
        if(to->m_bitsize==-1){
          (*to) = TRGCDCJSignal(t_reference.m_maxInt, t_reference.m_toReal, t_reference.m_maxInt, t_reference.m_maxInt, t_reference.m_maxActual, t_reference.m_maxActual, t_reference.m_maxActual, -1, t_reference.m_commonData);
        }
      }
  ...
  void TRGCDCJSignal::choose(TRGCDCJSignal & target, TRGCDCJSignal const & targetMin, TRGCDCJSignal const & targetMax, TRGCDCJSignal const & reference, std::vector<std::vector<TRGCDCJSignal> > data) {
    choose(target, targetMin, targetMax, reference, data, -3);
  }

  void TRGCDCJSignal::choose(TRGCDCJSignal & target, TRGCDCJSignal const & reference, std::vector<std::vector<TRGCDCJSignal> > data, int targetClock) {
    ...
    for(unsigned iCondition=0; iCondition<data.size(); iCondition++){
      ...
      if(iCondition==0) {
        ...
        if(t_toReal != assign->getToReal()) cout<<"[Error] TRGCDCJSignal::choose() => toReal are not same between assignments in data."<<endl;
      }
    }
  ...
  void TRGCDCJSignal::ifElse(std::vector<std::pair<TRGCDCJSignal, std::vector<std::pair<TRGCDCJSignal *, TRGCDCJSignal> > > > & data, int targetClock) {
    // Handle input errors.
    if(data.size()==0) {
    ...
    for(unsigned iCase=0; iCase<data.size(); iCase++){
      ...
      if(iCase!=(data.size()-1)) {
        ...
        if(comp.m_minInt!=0 || comp.m_maxInt!=1) cout<<"[Error] TRGCDCJSignal::ifElse() => comp signal m_minInt or m_maxInt is not correct."<<endl;
      }
  ...
  TRGCDCJSignal const TRGCDCJSignal::limit(signed long long minInt, signed long long maxInt, double minActual, double maxActual) const {
  ...
  TRGCDCJSignal const TRGCDCJSignal::slvToSigned(TRGCDCJSignal const & in, double const & toReal, signed long long const & minInt, signed long long const & maxInt, double const & actual, double const & minActual, double const & maxActual, int const & finishClock){
  ...
  TRGCDCJSignal const TRGCDCJSignal::slvToUnsigned(TRGCDCJSignal const &in, double const & toReal, signed long long const & minInt, signed long long const & maxInt, double const & actual, double const & minActual, double const & maxActual, int const & finishClock){
  ...
  void TRGCDCJSignal::calVhdlTypeBitwidth(TRGCDCJSignal const & first, std::string operation, TRGCDCJSignal const & second, int & type, int & bitwidth) {
  ...
  void TRGCDCJSignal::vhdlCode(TRGCDCJSignal const & first, std::string operation, TRGCDCJSignal const & second, TRGCDCJSignal const & result, string & resultVhdlCode) {
    ...
    // For arithmetic operations.
      ...
      if(logicType > 0 && vhdlType == -1) {
        if(logicBitwidth+1>vhdlBitwidth) t_first.m_vhdlCode = "resize("+t_first.m_vhdlCode+","+to_string(logicBitwidth+1)+")";
        resultCode = t_first.m_vhdlCode + operation + t_second.m_vhdlCode;
  ...
  void TRGCDCJSignal::vhdlCode(std::string operation, TRGCDCJSignal const & first, TRGCDCJSignal const & result, string & resultVhdlCode){
    ...
    if(logicType == 1 && vhdlType == -1) {
      if(logicBitwidth+1>vhdlBitwidth) t_first.m_vhdlCode = "resize("+t_first.m_vhdlCode+","+to_string(logicBitwidth+1)+")";
      if(operation=="shift_left" || operation=="shift_right") resultCode = operation + "(" + t_first.m_vhdlCode + ","+to_string(abs(first.m_bitsize-result.m_bitsize))+")";
  ...
  std::string TRGCDCJSignal::replaceWithSignalNames(std::string const & fromVhdlCode, std::vector<pair<std::string, vector<int> > > const & fromArgumentSignals, int const & finishClock, std::map<std::string, vector<int> > & buffers) {
    ...
    while(end != string::npos) {
      ...
      if(t_buffer == -1 || t_buffer == -2 || fromArgumentSignals[index].second[2]==-1 ) t_vhdlCode+=fromVhdlCode.substr(start, end-start) + fromArgumentSignals[index].first;
      else if(t_buffer > -1) {
  ...
  std::string TRGCDCJSignal::assignVhdlCode(TRGCDCJSignal const & target, TRGCDCJSignal const & from) {
    ...
    string t_vhdlCode=replaceWithSignalNames(t_from.m_vhdlCode, t_from.m_argumentSignals, t_from.m_finishClock, t_from.m_commonData->m_buffers);
  ...
  std::string TRGCDCJSignal::chooseVhdlCode(TRGCDCJSignal const & target, TRGCDCJSignal const & reference, std::vector<std::vector<TRGCDCJSignal> > const & data) {
    ...
    string t_referenceVhdlCode=replaceWithSignalNames(t_reference.m_vhdlCode, t_reference.m_argumentSignals, target.m_finishClock, t_reference.m_commonData->m_buffers);
    ...
    string t_fromVhdlCode=replaceWithSignalNames(from.m_vhdlCode, from.m_argumentSignals, target.m_finishClock, target.m_commonData->m_buffers);
    string t_toVhdlCode=replaceWithSignalNames(to.m_vhdlCode, to.m_argumentSignals, target.m_finishClock, target.m_commonData->m_buffers);
    ...
    t_vhdlCode<<"if ("<<t_referenceVhdlCode<<" >= "<<t_fromPrefixSigned<<t_fromVhdlCode<<t_fromPostfixSigned<<") and ("<<t_referenceVhdlCode<<" <= "<<t_toPrefixSigned<<t_toVhdlCode<<t_toPostfixSigned<<") then"<<endl;
    ...
    for(unsigned iCondition=1; iCondition<data.size()-1; iCondition++){
      ...
      t_fromVhdlCode=replaceWithSignalNames(from.m_vhdlCode, from.m_argumentSignals, target.m_finishClock, target.m_commonData->m_buffers);
      ...
      t_vhdlCode<<"elsif ("<<t_referenceVhdlCode<<" >= "<<t_fromPrefixSigned<<t_fromVhdlCode<<t_fromPostfixSigned<<") and ("<<t_referenceVhdlCode<<" <= "<<t_toPrefixSigned<<t_toVhdlCode<<t_toPostfixSigned<<") then"<<endl;
      t_vhdlCode<<"  "<<assignVhdlCode(target,assign)<<endl;
    }
  ...
  std::string TRGCDCJSignal::ifElseVhdlCode(std::vector<std::pair<TRGCDCJSignal, std::vector<std::pair<TRGCDCJSignal *, TRGCDCJSignal> > > > const & data){
    ...
    for(unsigned iCase=0; iCase<data.size(); iCase++){
      TRGCDCJSignal const & comp = data[iCase].first;
      string t_compVhdlCode = replaceWithSignalNames(comp.m_vhdlCode, comp.m_argumentSignals, comp.m_finishClock, comp.m_commonData->m_buffers);
  ...
  bool TRGCDCJSignal::isSameSignal(TRGCDCJSignal const & lhs, TRGCDCJSignal const & rhs) {
    return lhs.m_int==rhs.m_int && lhs.m_minInt == rhs.m_minInt && lhs.m_maxInt == lhs.m_maxInt 
        && lhs.m_toReal==rhs.m_toReal && lhs.m_actual==rhs.m_actual && lhs.m_minActual==rhs.m_minActual
        && lhs.m_maxActual==rhs.m_maxActual;
  }

  void TRGCDCJSignal::valuesToMapSignals(std::vector<std::tuple<string, double,int,double,double,int> > const & inValues, Belle2::TRGCDCJSignalData* inCommonData, std::map<std::string, Belle2::TRGCDCJSignal> & outMap){
  ...
  void TRGCDCJSignal::mapSignalsToValues(std::map<std::string, Belle2::TRGCDCJSignal> const & inMap, std::vector<std::pair<std::string, int> > const & inChoose, std::vector<std::tuple<string, double,int,double,double,int> > & outValues){
