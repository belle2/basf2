...
#include <vtx/calibration/VTXClusterPositionCalibrationAlgorithm.h>
...
VTXClusterPositionCalibrationAlgorithm::VTXClusterPositionCalibrationAlgorithm():
  CalibrationAlgorithm("VTXClusterPositionCollector"),
...
CalibrationAlgorithm::EResult VTXClusterPositionCalibrationAlgorithm::calibrate()
{
  ...
  typedef pair<VTXClusterShapeIndexPar, VTXClusterShapeClassifierPar> buffervalue_t;
  ...
  for (auto clusterKind : clusterKinds) {
    ...
    for (auto uBin = 1; uBin <= grid->GetXaxis()->GetNbins(); uBin++) {
      for (auto vBin = 1; vBin <= grid->GetYaxis()->GetNbins(); vBin++) {
        ...
        auto localShapeIndexer = VTXClusterShapeIndexPar();
        auto localShapeClassifier = VTXClusterShapeClassifierPar();
  ...
  VTXClusterShapeIndexPar* shapeIndexer = new VTXClusterShapeIndexPar();
  ...
  saveCalibration(shapeIndexer, "VTXClusterShapeIndexPar");

  // Create position estimator
  VTXClusterPositionEstimatorPar* positionEstimator = new VTXClusterPositionEstimatorPar();
  ...
  saveCalibration(positionEstimator, "VTXClusterPositionEstimatorPar");

  B2INFO("VTXClusterPosition Calibration Successful");
  return c_OK;
}


VTXClusterShapeClassifierPar VTXClusterPositionCalibrationAlgorithm::mirrorShapeClassifier(VTXClusterShapeClassifierPar*
    shapeClassifier, VTXClusterShapeIndexPar* shapeIndexer, int clusterKind)
{
  // Create a mirrored shape classifier
  auto mirroredShapeClassifier = VTXClusterShapeClassifierPar();
  ...
  for (auto indexAndValue : offsetMap) {
    ...
    for (auto offset : indexAndValue.second) {
      ...
      auto mirroredOffset = VTXClusterOffsetPar(offset.getU(), shift - offset.getV(), offset.getUSigma2(), offset.getVSigma2(),
...
VTXClusterShapeClassifierPar VTXClusterPositionCalibrationAlgorithm::localToGlobal(VTXClusterShapeClassifierPar* shapeClassifier,
    VTXClusterShapeIndexPar* shapeIndexer, VTXClusterShapeIndexPar* globalShapeIndexer)
{
  // Create a shape classifier using global shape indices
  auto globalShapeClassifier = VTXClusterShapeClassifierPar();
...
void VTXClusterPositionCalibrationAlgorithm::createShapeClassifier(string treename,
    VTXClusterShapeClassifierPar* shapeClassifier, VTXClusterShapeIndexPar* shapeIndexer)
{
  ...
  for (auto iter : offsetHistosVec) {
    ...
    for (auto& histo : histovec) {
      ...
      auto offset = VTXClusterOffsetPar(offsetU, offsetV, covU, covV, covUV);
