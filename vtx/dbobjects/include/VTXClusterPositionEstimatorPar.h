...
#include <vtx/dbobjects/VTXClusterShapeClassifierPar.h>
#include <vtx/dbobjects/VTXClusterOffsetPar.h>

namespace Belle2 {


  /** The class for VTX cluster position lookup table payload
   */

  class VTXClusterPositionEstimatorPar: public TObject {
  public:
    /** Default constructor */
    VTXClusterPositionEstimatorPar() {}
    ...
    void addGrid(int clusterkind, const TH2F& grid)
    {
      ...
      m_shapeClassifiers[clusterkind] = std::vector<VTXClusterShapeClassifierPar>();
      ...
      for (auto uBin = 1; uBin <= m_gridmap[clusterkind].GetXaxis()->GetNbins(); uBin++) {
        for (auto vBin = 1; vBin <= m_gridmap[clusterkind].GetYaxis()->GetNbins(); vBin++) {
          ...
          m_shapeClassifiers[clusterkind].push_back(VTXClusterShapeClassifierPar());
        }
      }
    }
    ...
    void setShapeClassifier(const VTXClusterShapeClassifierPar& classifier, int uBin, int vBin, int clusterkind)
    {
    ...
    const VTXClusterShapeClassifierPar& getShapeClassifier(int uBin, int vBin, int clusterkind) const
    {
      int key = m_gridmap.at(clusterkind).GetBinContent(uBin, vBin);
      const VTXClusterShapeClassifierPar& classifier = m_shapeClassifiers.at(clusterkind)[key];
      return classifier;
    }

    /** Returns shape classifier for incidence angles and clusterkind */
    const VTXClusterShapeClassifierPar& getShapeClassifier(double thetaU, double thetaV, int clusterkind) const
    {
    ...
    const VTXClusterOffsetPar* getOffset(int shape_index, float eta, double thetaU, double thetaV, int clusterkind) const
    {
      //Check if there is a classifier
      if (not hasClassifier(thetaU, thetaV, clusterkind)) {
        return nullptr;
      }
      const VTXClusterShapeClassifierPar& classifier = getShapeClassifier(thetaU, thetaV, clusterkind);
  ...
  private:
    ...
    std::map<int, std::vector<VTXClusterShapeClassifierPar> > m_shapeClassifiers;

    ClassDef(VTXClusterPositionEstimatorPar, 2);   /**< ClassDef, must be the last term before the closing {}*/
  };
