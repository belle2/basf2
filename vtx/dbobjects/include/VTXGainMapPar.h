...
namespace Belle2 {
  ...
  /** The payload class for VTX gain corrections
   *
   *  The payload class stores the gain correction values
   *  for VTX sensors on a grid nBinsU x nBinsV. The values
  ...
  class VTXGainMapPar: public TObject {
  public:

    /** Default constructor */
    VTXGainMapPar(int nBinsU = 4, int nBinsV = 6, float defaultValue = 1.0) : m_nBinsU(nBinsU), m_nBinsV(nBinsV),
    ...
    /** Return unordered_map with all VTX calibrations */
  ...
  private:
    ...
    /** Map for holding the calibrations for all VTX sensors by sensor id (unsigned short). */
    std::unordered_map<unsigned short, std::vector<float> > m_sensorCalibrationMap;

    ClassDef(VTXGainMapPar, 3);  /**< ClassDef, must be the last term before the closing {}*/
  };
