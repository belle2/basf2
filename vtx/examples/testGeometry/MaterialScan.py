...
#############################################################################
#
# This steering file measures the material budget of the Belle II upgrade detector
# geometry. It measures the material of BeamPipe and VTX.
#
# Usage: basf2 MaterialScan.py -- --vtx=VTX-CMOS-5layer-Discs
#
##############################################################################
...
# beampipe and the VTX.

# list of parts to be excluded from the scan.
excluded_parts = ['COIL', 'STR', 'ServiceGapsMaterial', 'Cryostat', 'FarBeamLine', 'HeavyMetalShield', 'VXDService',
                  'PXD', 'SVD', 'CDC', 'ECL', 'ARICH', 'TOP', 'KLM']

main.add_module('Geometry', excludedComponents=excluded_parts,
                additionalComponents=[args['vtx']],
                logLevel=LogLevel.INFO, assignRegions=True,
                useDB=False)
...
materialscan.param({
    ...
    'Filename': 'MaterialScan_{}.root'.format(args['vtx']),
    # Do Spherical scan?
    'spherical': True,
    ...
    'spherical.cosTheta': False,  # True,
    # Number of steps in theta
    'spherical.nPhi': 1,  # 100,
    # Minimal theta value
    'spherical.minPhi': 0,
    # Maximal theta value
