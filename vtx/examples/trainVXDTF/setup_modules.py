...
def setup_VXDTF2(path=None,
    ...
    :param usePXD: if true use pxd hits. Default False.
    :param useSVD: if true use svd hits. Default True.
    :param useVTX: if true use vtx hits. Default False.
    :param use_sec_map_fromDB: if true use sec_map from DB. Default True.
    ...
    if useVTX:
        spCreatorVTX = register_module('VTXSpacePointCreator')
        spCreatorVTX.logging.log_level = log_level
        spCreatorVTX.logging.debug_level = debug_level
        spCreatorVTX.param('NameOfInstance', 'VTXSpacePoints')
        spCreatorVTX.param('SpacePoints', 'SpacePoints')
        modules.append(spCreatorVTX)

    # SecMap Bootstrap
    secMapBootStrap = register_module('SectorMapBootstrap')
    if use_sec_map_fromDB:
        secMapBootStrap.param('ReadSecMapFromDB', True)
...
def setup_RTCtoSPTCConverters(
        ...
        VTXSPscollection='VTXSpacePoints',
        RTCinput='mcTracks',
    ...
    @param SPscollection the name of the storeArray containing SPs of both SVD, PXD and VTX.
    ...
    @param useSVD If False svdClusters are ignored.

    @param useVTX If False vtxClusters are ignored.
    ...
    if useSVD:
        if usePXD:
            ...
            # All SpacePoints are assumed to be in the same StoreArray
    ...
    if useVTX:
        # All SpacePoints are assumed to be in the same StoreArray
        spacePointNames.append(VTXSPscollection)
        detectorTypes.append('VTX')
        trueHitNames.append('')
        clusterNames.append('')
    ...
    recoTrackCandConverter.param('SVDSpacePointStoreArrayName', None)
    if useSVD:
    ...
    recoTrackCandConverter.param('VTXSpacePointStoreArrayName', None)
    if useVTX:
        recoTrackCandConverter.param('VTXSpacePointStoreArrayName', VTXSPscollection)
