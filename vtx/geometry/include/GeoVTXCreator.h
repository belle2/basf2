...
#ifndef GEOVTXCREATOR_H
#define GEOVTXCREATOR_H

#include <vector>
#include <vxd/geometry/GeoVXDCreator.h>
#include <vtx/dbobjects/VTXGeometryPar.h>
...
namespace Belle2 {
  ...
  namespace VTX {
    ...
    /** The creator for the VTX geometry of the Belle II detector.   */
    class GeoVTXCreator : public VXD::GeoVXDCreator {
    private:
      //! Create a parameter object from the Gearbox XML parameters.
      VTXGeometryPar createConfiguration(const GearDir& param);

      //! Create the geometry from a parameter object.
      void createGeometry(const VTXGeometryPar& parameters, G4LogicalVolume& topVolume, geometry::GeometryTypes type);
    public:
      /** Constructor of the GeoVTXCreator class. */
      GeoVTXCreator(): VXD::GeoVXDCreator("VTX") {};

      /** The destructor of the GeoVTXCreator class. */
      virtual ~GeoVTXCreator();
      ...
      {
        VTXGeometryPar config = createConfiguration(content);
      ...
      {
        DBImportObjPtr<VTXGeometryPar> importObj;
      ...
      /**
       * Create support structure for a VTX Layer
      ...
      /**
       * Create support structure for a VTX Ladder
       * @param layer Layer ID to create the support
       * @param support Reference to the database containing the parameters
       */
      ...
      /**
       * Create support structure for VTX Half Shell, that means everything
       * that does not depend on layer or sensor alignment
       * @param parameters Reference to the database containing the parameters
       */
      virtual VXD::GeoVXDAssembly createHalfShellSupport(const VTXGeometryPar& parameters);
       ...
       * @param sensor Reference to the database containing the parameters
       */
      VTXSensorInfoPar* readSensorInfo(const GearDir& sensor);
       ...
       * that does not depend on layer or sensor alignment
       * @param support Reference to the database containing the parameters
       * @param vtxGeometryPar VTX geometry
       */
      void readHalfShellSupport(const GearDir& support, VTXGeometryPar& vtxGeometryPar);
