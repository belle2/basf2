...
#include <vtx/geometry/GeoVTXCreator.h>
#include <vxd/geometry/GeoCache.h>
#include <vtx/geometry/SensorInfo.h>
#include <vtx/simulation/SensitiveDetector.h>
...
namespace Belle2 {
  ...
  namespace VTX {
    ...
    geometry::CreatorFactory<GeoVTXCreator> GeoVTXFactory("VTXCreator");

    GeoVTXCreator::~GeoVTXCreator()
    {
      for (SensorInfo* sensorInfo : m_SensorInfo) delete sensorInfo;
      m_SensorInfo.clear();
    }

    VXD::SensorInfoBase* GeoVTXCreator::createSensorInfo(const VXDGeoSensorPar& sensor)
    {

      const VTXSensorInfoPar& infoPar = dynamic_cast<const VTXSensorInfoPar&>(*sensor.getSensorInfo());
      ...
      info->setVTXSensorParams(
        ...
        infoPar.getApplyElectronicEffects(),
        infoPar.getElectronicNoise(),
        infoPar.getApplyBinaryReadout(),
        infoPar.getBinaryHitThreshold(),
        infoPar.getElectronToADU(),
        infoPar.getMaxADUCode(),
        infoPar.getApplyPoissonSmearing(),
        infoPar.getApplyIntegrationWindow(),
        infoPar.getSegmentLength(),
        infoPar.getElectronGroupSize(),
        infoPar.getElectronStepTime(),
        infoPar.getElectronMaxSteps(),
        infoPar.getHardwareDelay(),
        infoPar.getADCunit(),
        infoPar.getCloudSize(),
        infoPar.getGq(),
        infoPar.getX0average(),
        infoPar.getTanLorentzAngle(),
        infoPar.getResolutionCoefficientU(),
        infoPar.getResolutionCoefficientV()
      );
    ...
    VTXSensorInfoPar* GeoVTXCreator::readSensorInfo(const GearDir& sensor)
    {
      VTXSensorInfoPar* info = new VTXSensorInfoPar(
        VxdID(0, 0, 0),
      ...
      info->setVTXSensorParams(
        ...
        sensor.getLength("BorderU"),
        sensor.getLength("BorderV"),
        sensor.getDouble("ChargeThreshold"),
        sensor.getDouble("NoiseFraction"),
        sensor.getBool("ApplyElectronicEffects"),
        sensor.getDouble("ElectronicNoise"),
        sensor.getBool("ApplyBinaryReadout"),
        sensor.getDouble("BinaryHitThreshold"),
        sensor.getDouble("ElectronToADU"),
        sensor.getDouble("MaxADUCode"),
        sensor.getBool("ApplyPoissonSmearing"),
        sensor.getBool("ApplyIntegrationWindow"),
        sensor.getLength("SegmentLength"),
        sensor.getDouble("ElectronGroupSize"),
        sensor.getTime("ElectronStepTime"),
        sensor.getDouble("ElectronMaxSteps"),
        sensor.getTime("HardwareDelay"),
        sensor.getDouble("ADCunit"),
        sensor.getDouble("CloudSize"),
        sensor.getDouble("Gq"),
        sensor.getDouble("X0average"),
        sensor.getDouble("TanLorentzAngle"),
        sensor.getDouble("ResolutionCoefficientU"),
        sensor.getDouble("ResolutionCoefficientV")
      );
      info->setIntegrationWindow(
    ...
    VTXGeometryPar GeoVTXCreator::createConfiguration(const GearDir& content)
    {
      // Create an empty payload
      VTXGeometryPar vtxGeometryPar;

      //Read prefix ('SVD' or 'PXD' or 'VTX')
      vtxGeometryPar.setPrefix(m_prefix);
      ...
      vtxGeometryPar.setGlobalParams(globals);
      ...
      vtxGeometryPar.setEnvelope(envelope);

      // Read alignment for detector m_prefix ('VTX' or 'SVD')
      ...
      if (!paramsAlign) {
        ...
        return vtxGeometryPar;
      }
      vtxGeometryPar.getAlignmentMap()[m_prefix] = VXDAlignmentPar(paramsAlign.getLength("du"),
      ...
      for (const GearDir& paramsSensor : components.getNodes("Sensor")) {
        ...
        VTXSensorInfoPar* vtxInfo = readSensorInfo(GearDir(paramsSensor, "Active"));
        sensor.setSensorInfo(vtxInfo);
        sensor.setComponents(getSubComponents(paramsSensor));
        vtxGeometryPar.getSensorMap()[sensorTypeID] = sensor;
        vtxGeometryPar.getSensorInfos().push_back(vtxInfo);
      }

      //Build all ladders including Sensors
      GearDir support(content, "Support/");
      readHalfShellSupport(support, vtxGeometryPar);
      ...
      for (const GearDir& shell : content.getNodes("HalfShell")) {
        ...
        vtxGeometryPar.getAlignmentMap()[shellName] = VXDAlignmentPar(paramsShell.getLength("du"),
        ...
        for (const GearDir& layer : shell.getNodes("Layer")) {
          ...
          readLadder(layerID, components, vtxGeometryPar);
      ...
      if (vtxGeometryPar.getGlobalParams().getActiveChips() &&  radiationDir) {
        ...
        vtxGeometryPar.setRadiationSensors(radiationSensors);
      }

      return vtxGeometryPar;
    }

    void GeoVTXCreator::readHalfShellSupport(const GearDir& support, VTXGeometryPar& vtxGeometryPar)
    {
      if (!support) return;
      ...
      for (const GearDir& endflange : support.getNodes("Endflange")) {
        ...
        vtxGeometryPar.getEndflanges().push_back(endflangePar);
      }

      // Cout outs for endflanges
      vtxGeometryPar.setNCutOuts(support.getInt("Cutout/count"));
      vtxGeometryPar.setCutOutWidth(support.getLength("Cutout/width"));
      vtxGeometryPar.setCutOutHeight(support.getLength("Cutout/height"));
      vtxGeometryPar.setCutOutShift(support.getLength("Cutout/shift"));
      vtxGeometryPar.setCutOutRPhi(support.getLength("Cutout/rphi"));
    ...
    void GeoVTXCreator::createGeometry(const VTXGeometryPar& parameters, G4LogicalVolume& topVolume, geometry::GeometryTypes)
    {
    ...
    VXD::SensitiveDetectorBase* GeoVTXCreator::createSensitiveDetector(VxdID sensorID, const VXDGeoSensor& sensor,
        const VXDGeoSensorPlacement& placement __attribute__((unused)))
    {
    ...
    VXD::GeoVXDAssembly GeoVTXCreator::createLayerSupport() { return VXD::GeoVXDAssembly(); }

    VXD::GeoVXDAssembly GeoVTXCreator::createLadderSupport() { return VXD::GeoVXDAssembly(); }

    VXD::GeoVXDAssembly GeoVTXCreator::createHalfShellSupport(const VTXGeometryPar& parameters)
    {
      ...
      for (const VXDPolyConePar& endflange : Endflanges) {
        ...
        for (int i = 0; i < nCutouts; ++i) {
          ...
          supportCone = new G4SubtractionSolid("VTX Support endflange", supportCone, box, placement);
        }


        string materialName = endflange.getMaterial();
        G4Material* material = geometry::Materials::get(materialName);
        if (!material) B2FATAL("Material '" << materialName << "', required by VTX component " << name << ", could not be found");
