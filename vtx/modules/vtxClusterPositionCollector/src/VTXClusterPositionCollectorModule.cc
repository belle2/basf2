...
#include <vtx/modules/vtxClusterPositionCollector/VTXClusterPositionCollectorModule.h>
...
VTXClusterPositionCollectorModule::VTXClusterPositionCollectorModule() : CalibrationCollectorModule()
  , m_clusterEta(0), m_positionOffsetU(0), m_positionOffsetV(0), m_sizeV(0), m_pitchV(0)
{
  // Set module properties
  setDescription("Calibration collector module for VTX cluster position estimation");
  setPropertyFlags(c_ParallelProcessingCertified);

  addParam("clustersName", m_storeClustersName, "Name of the collection to use for VTXClusters", string(""));
...
void VTXClusterPositionCollectorModule::prepare() // Do your initialise() stuff here
{
  m_vtxCluster.isRequired();
...
void VTXClusterPositionCollectorModule::collect() // Do your event() stuff here
{
  // If no input, nothing to do
  if (!m_vtxCluster) return;

  string gridname = str(format("GridKind_%1%") % m_clusterKind);
  auto grid = getObjectPtr<TH2F>(gridname);

  for (auto& cluster :  m_vtxCluster) {

    // Ignore clustes with more than one truehit
    if (cluster.getRelationsTo<VTXTrueHit>().size() > 1) {
      continue;
    }
    ...
    if (m_clusterKind != VTX::VTXClusterPositionEstimator::getInstance().getClusterkind(cluster)) {
      continue;
    }

    for (auto& truehit : cluster.getRelationsTo<VTXTrueHit>()) {
      ...
      for (int i = 0; i < cluster.getSize(); i++) {
        const VTXDigit* const storeDigit = cluster.getRelationsTo<VTXDigit>("VTXDigits")[i];
        pixels.insert(VTX::Pixel(storeDigit, i));
      }

      // Fill tree variables
      m_shapeName = VTX::VTXClusterPositionEstimator::getInstance().getShortName(pixels, cluster.getUStart(), cluster.getVStart(),
                    cluster.getVSize(), thetaU, thetaV);
      m_mirroredShapeName = VTX::VTXClusterPositionEstimator::getInstance().getMirroredShortName(pixels, cluster.getUStart(),
                            cluster.getVStart(), cluster.getVSize(), thetaU, thetaV);
      m_clusterEta = VTX::VTXClusterPositionEstimator::getInstance().computeEta(pixels, cluster.getVStart(), cluster.getVSize(), thetaU,
                     thetaV);
