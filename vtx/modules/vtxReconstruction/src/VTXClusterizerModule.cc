...
#include <vtx/modules/vtxReconstruction/VTXClusterizerModule.h>
...
//                 Register the Module
//-----------------------------------------------------------------
REG_MODULE(VTXClusterizer);

//-----------------------------------------------------------------
//                 Implementation
//-----------------------------------------------------------------

VTXClusterizerModule::VTXClusterizerModule() : Module()
  , m_elNoise(0.7), m_cutSeed(1.0), m_cutAdjacent(1.0), m_cutCluster(1.0)
  , m_cutAdjacentSignal(0), m_sizeHeadTail(3), m_clusterCacheSize(0)
{
  //Set module properties
  setDescription("Cluster VTXHits");
  ...
  addParam("ClusterCacheSize", m_clusterCacheSize,
           "Maximum number of PIXEL columns the cluster cache can handle", 0);
...
void VTXClusterizerModule::initialize()
{
  //Register collections
  StoreArray<VTXCluster> storeClusters(m_storeClustersName);
  ...
  B2DEBUG(20,
          "VTXClusterizer Parameters (in default system units, *=cannot be set directly):");
  ...
  B2DEBUG(20,
          " -->  Clusters:           " << DataStore::arrayName<VTXCluster>(m_storeClustersName));
  B2DEBUG(20,
...
void VTXClusterizerModule::event()
{
  const StoreArray<MCParticle> storeMCParticles(m_storeMCParticlesName);
  const StoreArray<VTXTrueHit> storeTrueHits(m_storeTrueHitsName);
  const StoreArray<VTXDigit> storeDigits(m_storeDigitsName);
  StoreArray<VTXCluster> storeClusters(m_storeClustersName);
  ...
  for (int i = 0; i < nPixels; i++) {
    ...
    if (sensorID != storeDigit->getSensorID()) {
      ...
      B2FATAL("Pixels are not sorted correctly, please include the "
              "VTXDigitSorter module before running the Clusterizer or fix "
    ...
    try {
      ...
      B2WARNING("VTX clustering: Ignoring pixel " << px.getU() << "," << px.getV() << ": " << e.what());
    }
  }
  writeClusters(sensorID);
}

void VTXClusterizerModule::createRelationLookup(const RelationArray& relation, RelationLookup& lookup, size_t digits)
{
  lookup.clear();
...
void VTXClusterizerModule::fillRelationMap(const RelationLookup& lookup, std::map<unsigned int, float>& relation,
                                           unsigned int index)
{
...
void VTXClusterizerModule::writeClusters(VxdID sensorID)
{
  if (m_cache->empty())
    return;
  ...
  for (ClusterCandidate& cls : *m_cache) {
    ...
    for (const VTX::Pixel& px : cls.pixels()) {
      ...
      //Obtain relations from VTXTrueHits
    ...
    const double coeffU = info.getResolutionCoefficientU();
    const double coeffV = info.getResolutionCoefficientV();

    //Calculate position and error with u as primary axis, fixed pitch size
    calculatePositionError(cls, projU, projV, pitchU, pitchU, pitchU, coeffU);
    //Calculate position and error with v as primary axis, possibly different pitch sizes
    calculatePositionError(cls, projV, projU, info.getVPitch(projV.getMinPos()), pitchV, info.getVPitch(projV.getMaxPos()), coeffV);
    ...
    int clusterkind = VTXClusterPositionEstimator::getInstance().getClusterkind(cls.pixels(), sensorID);

    // Compute sorted set of pixel
    set<Pixel> pixelSet(cls.pixels().begin(), cls.pixels().end());

    // Compute classifier variables needed for later retrival of position correction in VTX CKF
    vector<float> sectorEtaValues = {0, 0, 0, 0};
    sectorEtaValues[0] = VTXClusterPositionEstimator::getInstance().computeEta(pixelSet, projV.getMinCell(), projV.getSize(), +1.0,
                         +1.0);
    sectorEtaValues[1] = VTXClusterPositionEstimator::getInstance().computeEta(pixelSet, projV.getMinCell(), projV.getSize(), -1.0,
                         +1.0);
    sectorEtaValues[2] = VTXClusterPositionEstimator::getInstance().computeEta(pixelSet, projV.getMinCell(), projV.getSize(), -1.0,
                         -1.0);
    sectorEtaValues[3] = VTXClusterPositionEstimator::getInstance().computeEta(pixelSet, projV.getMinCell(), projV.getSize(), +1.0,
                         -1.0);

    vector<int> sectorShapeIndices = { -1, -1, -1, -1};
    sectorShapeIndices[0] = VTXClusterPositionEstimator::getInstance().computeShapeIndex(pixelSet, projU.getMinCell(),
                            projV.getMinCell(), projV.getSize(), +1.0, +1.0);
    sectorShapeIndices[1] = VTXClusterPositionEstimator::getInstance().computeShapeIndex(pixelSet, projU.getMinCell(),
                            projV.getMinCell(), projV.getSize(), -1.0, +1.0);
    sectorShapeIndices[2] = VTXClusterPositionEstimator::getInstance().computeShapeIndex(pixelSet, projU.getMinCell(),
                            projV.getMinCell(), projV.getSize(), -1.0, -1.0);
    sectorShapeIndices[3] = VTXClusterPositionEstimator::getInstance().computeShapeIndex(pixelSet, projU.getMinCell(),
                            projV.getMinCell(), projV.getSize(), +1.0, -1.0);
...
void VTXClusterizerModule::calculatePositionError(const ClusterCandidate& cls, ClusterProjection& primary,
                                                  const ClusterProjection& secondary, double minPitch, double centerPitch, double maxPitch, double coeff)
{

  primary.setError(coeff * centerPitch / sqrt(12.0));

  /* FIXME: This code needs revision. Someone should check if it is better or worse than the code above
