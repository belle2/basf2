...
namespace Belle2 {
  ...
  namespace VTX {
     ...
     *
     * This class is a wrapper around VTXDigit or VTXRawHit to provide ordered
     * access. Ordering is row-wise: first all digits/rawhits belonging to the
     * first row, ordered by column in ascending order.
     *
     * The Pixel object contains a copy of the essential values of the VTXDigit
     * or VTXRawHit to speed up lookup during sorting or clustering. VTXDigit
     ...
     * b) Inheriting from TObject makes the VTXDigit/PXDRawHit more than three
    ...
    public:
      ...
      explicit Pixel(unsigned int index = 0): m_index(index), m_u(0), m_v(0),
        m_charge(0) {}
      /** Construct from a given VTXDigit and its store index */
      Pixel(const VTXDigit* digit, unsigned int index): m_index(index),
    ...
    private:
      /** Index of the corresponding VTXDigit in the StoreArray */
