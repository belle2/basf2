...
#include <vtx/dbobjects/VTXGainMapPar.h>
...
namespace Belle2 {
  ...
  namespace VTX {
    /**
    * Singleton class for managing gain corrections for the VTX.
    */
    class VTXGainCalibrator {

    public:

      /** Initialize the VTXGainCalibrator */
      void initialize();

      /** Set gains from DB. */
      void setGains();

      /** Main (and only) way to access the VTXGainCalibrator. */
      static VTXGainCalibrator& getInstance();
      ...
      const VTXGainMapPar& getGainMapParameters() const {return m_gains;}
    ...
    private:
      ...
      VTXGainCalibrator(const VTXGainCalibrator&) = delete;
      /** Singleton class, forbidden assignment operator */
      VTXGainCalibrator& operator=(const VTXGainCalibrator&) = delete;

      /** Masked pixels retrieved from DB. */
      std::unique_ptr<DBObjPtr<VTXGainMapPar>> m_gainsFromDB;

      /** Map of gain corrections. */
      VTXGainMapPar m_gains;
    };
  }
} //Belle2 namespace
