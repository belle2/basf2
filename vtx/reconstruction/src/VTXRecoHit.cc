...
VTXRecoHit::VTXRecoHit():
  genfit::PlanarMeasurement(HIT_DIMENSIONS), m_sensorID(0), m_trueHit(0), m_cluster(0),
  m_energyDep(0)//, m_energyDepError(0)
{}

VTXRecoHit::VTXRecoHit(const VTXTrueHit* hit, const genfit::TrackCandHit*, float sigmaU, float sigmaV):
...
VTXRecoHit::VTXRecoHit(const VTXCluster* hit, float sigmaU, float sigmaV, float covUV):
...
VTXRecoHit::VTXRecoHit(const VTXCluster* hit, const genfit::TrackCandHit*):
  genfit::PlanarMeasurement(HIT_DIMENSIONS), m_sensorID(0), m_trueHit(0), m_cluster(hit),
  m_energyDep(0)//, m_energyDepError(0)
{
  // Set the sensor UID
...
genfit::AbsMeasurement* VTXRecoHit::clone() const
{
  return new VTXRecoHit(*this);
}
...
float VTXRecoHit::getShapeLikelyhood(const genfit::StateOnPlane& state) const
{
  ...
  if (this->getCluster()) {
    ...
    return VTX::VTXClusterPositionEstimator::getInstance().getShapeLikelyhood(*this->getCluster(), state5[1], state5[2]);
  }
  // If we reach here, we can do no better than return zero
  return 0;
}

TVectorD VTXRecoHit::applyPlanarDeformation(TVectorD hitCoords, std::vector<double> planarParameters,
...
std::vector<genfit::MeasurementOnPlane*> VTXRecoHit::constructMeasurementsOnPlane(const genfit::StateOnPlane& state) const
{
  ...
  if (this->getCluster()) {
    ...
    const TVectorD& state5 = state.getState();
    auto offset = VTX::VTXClusterPositionEstimator::getInstance().getClusterOffset(*this->getCluster(), state5[1], state5[2]);
