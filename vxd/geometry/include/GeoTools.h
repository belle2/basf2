...
namespace Belle2 {
  namespace VXD {
    ...
    public:
      ...
      /** Get total number of chips in VTX
       * @return total number of chips in VTX
       */
      size_t getTotalVTXChips() const { return m_listOfVTXChips.size(); }
      ...
      * @param isU true for u side of the sensor (DCD)
       ...
       * @return Index of sensor in plots.
       */
      int getVTXSensorIndex(VxdID sensorID) const
      {
        return getSensorIndex(sensorID);
      }
      /** Return index of sensor in plots.
        * Use for only VTX sensor counting, without SVD.
        * @param layer layer position of sensor
        * @param ladder ladder position of sensor
        * @param sensor sensor position of sensor
        * @return Index of sensor in plots.
        */
      int getVTXSensorIndex(int layer, int ladder, int sensor) const
      {
        return getSensorIndex(VxdID(layer, ladder, sensor));
      }

      /** Return index of a VTX sensor in plots.
        * Use for only VTX sensor counting, without SVD.
        * @param vtxIndex Index of sensor in plots.
        */
      VxdID getSensorIDFromVTXIndex(int vtxIndex) const
      {
        return m_listOfSensors[vtxIndex];
      }

      /*********************************************************************
       * VTX chip indexing
       ********************************************************************/
      /** Return VTX chip index in the list of VTX chips.
        * 1-based chip numbering.
        * @param sensorID VxdID of the sensor
        * @param isU true for u side of the sensor (DCD)
        * @param chip Chip position on sensor - DCDs or Switchers
        * @return Index of sensor in plots.
        */
      int getVTXChipIndex(VxdID sensorID, bool isU, int chip) const
      {
        VxdID chipID(sensorID);
        chip = isU ? chip - 1 : chip - 1 + c_nVTXChipsU;
        chipID.setSegmentNumber(static_cast<unsigned short>(chip));
        return std::distance(
                 m_listOfVTXChips.begin(),
                 std::find(m_listOfVTXChips.begin(), m_listOfVTXChips.end(), chipID));
      }
      ...
      /** Return VTX chip index in the list of VTX chips.
        ...
        * @return Index of sensor in plots.
        */
      int getVTXChipIndex(int layer, int ladder, int sensor, bool isU, int chip) const
      {
        return getVTXChipIndex(VxdID(layer, ladder, sensor), isU, chip);
      }

      /** Return chipID (VxdID + side and chipNo) for index in the list. .
       * @param vtxChipIndex Index in the list of VTX sensors.
       * @return chipID: VxdID + chip number/side encoded in segment part.
       */
      VxdID getChipIDFromVTXIndex(int vtxChipIndex) const
      {
        return m_listOfVTXChips[vtxChipIndex];
      }

      /** Decode sensor side from a VTX ChipID.
       * @param chipID VxdID with chip number encoded in Segment part.
       * @return true if the chip is on u-side.
       */
      bool isVTXSideU() const
      {
        // Always false if c_nVTXChipsU =0 (corrected for cppcheck)
        // return (chipID.getSegmentNumber() < c_nVTXChipsU);
        return false;
      }

      /** Decode (1-based) chip  number from a VTX ChipID.
       * @param chipID VxdID with chip number encoded in Segment part.
       * @return Chip number on the respective sensor side.
       */
      unsigned short getVTXChipNumber(VxdID chipID) const
      {
        unsigned short chipNo = chipID.getSegmentNumber();
        // Always return chipNo + 1 if c_nVTXChipsU =0 (corrected for cppcheck)
        // return (chipNo < c_nVTXChipsU ? chipNo + 1 : (chipNo + 1 - c_nVTXChipsU));
        return chipNo + 1;
      }
    ...
    private:
      ...
      /** Create list of VTX chips. */
      void createListOfVTXChips();
      ...
      /** List of all VTX chips */
      std::vector<VxdID> m_listOfVTXChips;
      ...
      /** Number of VTX chips per sensor in u (DCD) on Belle II */
      const unsigned short c_nVTXChipsU = 0;
      /** Number of VTX chips per sensor in v (Switchers) on Belle II */
      const unsigned short c_nVTXChipsV = 0;
    };
  } // VXD namespace
} // Belle2 namespace
