    ...
    void GeoCache::addSensor(SensorInfoBase* sensorinfo)
    {
      ...
      switch (sensorinfo->getType()) {
        ...
        case SensorInfoBase::TEL:
          ...
          break;
        case SensorInfoBase::VTX:
          m_vtxLayers.insert(layerID);
          break;
        default:
          B2FATAL("Cannot use anything else as SensorTypes PXD, SVD, VTX or TEL when creating VXD Sensors");
      }
    ...
    const set<VxdID> GeoCache::getLayers(SensorInfoBase::SensorType type)
    {
      switch (type) {
        ...
        default:
          ...
          allLayers.insert(m_vtxLayers.begin(), m_vtxLayers.end());
          return allLayers;
      }
    }
