...
void GeoTools::createListOfVTXChips()
{
  // We set segment number to the number of chip.
  std::vector<unsigned short> sensorChips(c_nVTXChipsU + c_nVTXChipsV);
  std::iota(sensorChips.begin(), sensorChips.end(), 0);
  for (auto sensorID : m_listOfSensors) {
    std::vector<VxdID> sensorIDs;
    std::transform(
    ...
    m_listOfVTXChips.insert(m_listOfVTXChips.end(), sensorIDs.begin(), sensorIDs.end());
  }
}
