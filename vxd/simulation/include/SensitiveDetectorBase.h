  ...
  namespace VXD {
    ...
    public:
      ...
      explicit SensitiveDetectorBase(SensorInfoBase* info):
        Simulation::SensitiveDetectorBase((info->getType() == SensorInfoBase::PXD ? "PXD " : info->getType() == SensorInfoBase::SVD ?
                                           "SVD " : "VTX ") + (std::string)info->getID(),
                                          info->getType() == SensorInfoBase::PXD ? Const::PXD : info->getType() == SensorInfoBase::SVD ? Const::SVD : Const::VTX),
        m_info(info) {};
